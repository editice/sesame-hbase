...
public abstract class TriGParserTestCase {
	...
	public TestSuite createTestSuite() throws Exception {
		...
		String query = "SELECT testName, inputURL, outputURL "
				+ "FROM {} mf:name {testName}; "
				+ "        mf:result {outputURL}; "
				+ "        mf:action {} qt:data {inputURL} "
				+ "USING NAMESPACE "
				+ "  mf = <http://www.w3.org/2001/sw/DataAccess/tests/test-manifest#>, "
				+ "  qt = <http://www.w3.org/2001/sw/DataAccess/tests/test-query#>";

		TupleQueryResult queryResult = con.prepareTupleQuery(
		...
		while (queryResult.hasNext()) {
			...
			suite.addTest(new PositiveParserTest(testName, inputURL, outputURL,
					baseURL));
		}
		...
		query = "SELECT testName, inputURL "
				+ "FROM {} mf:name {testName}; "
				+ "        mf:action {} qt:data {inputURL} "
				+ "USING NAMESPACE "
				+ "  mf = <http://www.w3.org/2001/sw/DataAccess/tests/test-manifest#>, "
				+ "  qt = <http://www.w3.org/2001/sw/DataAccess/tests/test-query#>";
		queryResult = con.prepareTupleQuery(QueryLanguage.SERQL, query)
				.evaluate();
	...
	private class PositiveParserTest extends TestCase {
		...
		public PositiveParserTest(String testName, String inputURL,
				String outputURL, String baseURL) throws MalformedURLException {
			super(testName);
		...
		protected void runTest() throws Exception {
			...
			StatementCollector inputCollector = new StatementCollector(
					inputCollection);
			...
			StatementCollector outputCollector = new StatementCollector(
					outputCollection);
			...
			if (!ModelUtil.equals(inputCollection, outputCollection)) {
				Collection<? extends Statement> missingStatements = RepositoryUtil
						.difference(outputCollection, inputCollection);
				Collection<? extends Statement> unexpectedStatements = RepositoryUtil
						.difference(inputCollection, outputCollection);
	...
	private class NegativeParserTest extends TestCase {
		...
		public NegativeParserTest(String caseURI, String inputURL,
				String baseURL) throws MalformedURLException {
			super(caseURI);
			this.inputURL = url(inputURL);
			this.baseURL = baseURL;
		}
