...
public abstract class RDFXMLParserTestCase {
	...
	private static String OPENRDF_MANIFEST_FILE = LOCAL_TESTS_DIR
			+ "openrdf/Manifest.rdf";
	...
	public TestSuite createTestSuite() throws Exception {
		...
		con
				.add(localManifest, base(localManifest.toString()),
						RDFFormat.RDFXML);
		...
		query = "select TESTCASE, INPUT "
				+ "from {TESTCASE} rdf:type {test:NegativeParserTest}; "
				+ "                test:inputDocument {INPUT}; "
				+ "                test:status {\"APPROVED\"} "
				+ "using namespace test = <http://www.w3.org/2000/10/rdf-tests/rdfcore/testSchema#>";
	...
	private static URL resolveURL(String urlString)
		...
		if (urlString.startsWith(W3C_TESTS_DIR)) {
			...
			urlString = LOCAL_TESTS_DIR + "w3c-approved/"
					+ urlString.substring(W3C_TESTS_DIR.length());
		}
	...
	private class PositiveParserTest extends TestCase {
		...
		public PositiveParserTest(String caseURI, String inputURL,
				String outputURL) {
			super(caseURI);
	...
	private static class CanonXMLValueFactory extends ValueFactoryImpl {
		...
		public Literal createLiteral(String value, URI datatype) {
			if (RDF.XMLLITERAL.equals(datatype)) {
				...
				try {
					value = new String(c14n.canonicalize(value
