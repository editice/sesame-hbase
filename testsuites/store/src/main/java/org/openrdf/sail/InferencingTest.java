...
public class InferencingTest extends TestCase {
	 ...
	 * @param inputData
	 *            The URL of the (N-Triples) data containing the triples that
	 *            should be added to the RdfSchemaRepository.
	 * @param outputData
	 *            The URL of the (N-Triples) data containing the triples that
	 *            should or should not (depending on the value of
	 *            <tt>isPositiveTest</tt> be present in the statements returned
	 *            by the RdfSchemaRepository.
	 ...
	 */
	public InferencingTest(String name, Sail sailStack, String inputData,
			String outputData, boolean isPositiveTest) {
		super(name);
		int slashLoc = name.lastIndexOf('/');
		this.name = name.substring(0, slashLoc) + "-"
				+ name.substring(slashLoc + 1);
	...
	protected void runTest() throws Exception {
		...
		try {
			...
			entailedStatements = Iterations.addAll(con.getStatements(null,
					null, null, true), new HashSet<Statement>());
		} finally {
			stream.close();
			con.close();
		}
		...
		try {
			...
			expectedStatements = Iterations.addAll(con.getStatements(null,
					null, null, false), new HashSet<Statement>());
		} finally {
			stream.close();
			con.close();
		...
		// closure set.
		boolean outputEntailed = ModelUtil.isSubset(expectedStatements,
				entailedStatements);

		if (isPositiveTest && !outputEntailed) {
			File dumpFile = dumpStatements(RepositoryUtil.difference(
					expectedStatements, entailedStatements));
			...
			fail("Erroneous entailment, unexpected statements dumped to file "
					+ dumpFile);
		}
	}
	...
	public static void addTests(TestSuite suite, Sail sailStack) {
		...
		suite
				.addTest(createTestCase(sailStack, "subclassof", "error001",
						false));
		suite.addTest(createTestCase(sailStack, "subpropertyof", "test001",
				true));
		suite.addTest(createTestCase(sailStack, "subpropertyof", "test002",
				true));
		suite.addTest(createTestCase(sailStack, "subpropertyof", "test003",
				true));
		suite.addTest(createTestCase(sailStack, "subpropertyof", "error001",
				false));
	...
	private static TestCase createTestCase(Sail sailStack, String subdir,
			String testName, boolean isPositiveTest) {
		return new InferencingTest(subdir + "/" + testName, sailStack,
				TEST_DIR_PREFIX + "/" + subdir + "/" + testName + "-in.nt",
				TEST_DIR_PREFIX + "/" + subdir + "/" + testName + "-out.nt",
				isPositiveTest);
	}
}
