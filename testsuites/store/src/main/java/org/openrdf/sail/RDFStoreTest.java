...
public abstract class RDFStoreTest extends TestCase {
	...
	public void testEmptyRepository() throws Exception {
		...
		assertEquals("Empty repository should not return any statements", 0,
				countAllElements());

		assertEquals("Named context should be empty", 0,
				countContext1Elements());

		assertEquals(
				"Empty repository should not return any context identifiers",
				0, countElements(con.getContextIDs()));
	...
	private void testValueRoundTrip(Resource subj, URI pred, Value obj)
		...
		ParsedTupleQuery tupleQuery = QueryParserUtil.parseTupleQuery(
				QueryLanguage.SERQL,
				"SELECT S, P, O FROM {S} P {O} WHERE P = <"
						+ pred.stringValue() + ">", null);

		CloseableIteration<? extends BindingSet, QueryEvaluationException> iter;
		iter = con.evaluate(tupleQuery.getTupleExpr(), null, EmptyBindingSet
				.getInstance(), false);
	...
	public void testCreateURI1() throws Exception {
		...
		assertEquals(
				"createURI(Sring) and createURI(String, String) should create equal URIs",
				1, con.size());
	}
	...
	public void testSize() throws Exception {
		...
		assertEquals("Size of unknown context should be 0", 0, con
				.size(unknownContext));

		URIImpl uriImplContext1 = new URIImpl(context1.toString());

		assertEquals("Size of named context (defined as URIImpl) should be 3",
				3, con.size(uriImplContext1));
	}
	...
	public void testAddData() throws Exception {
		...
		assertEquals("Named context should contain 3 statements", 3,
				countContext1Elements());

		assertEquals("Repository should have 1 context identifier", 1,
		...
		assertEquals(
				...
				2, countQueryResults("select * from {ex:picasso} P {O}"));
		...
		assertEquals(
				...
				4, countQueryResults("select * from {S} rdf:type {O}"));

		assertEquals(
				"Repository should contain 2 statements matching (_, _, Class)",
				2, countQueryResults("select * from {S} P {rdfs:Class}"));
	...
	public void testAddWhileQuerying() throws Exception {
		...
		ParsedTupleQuery tupleQuery = QueryParserUtil.parseTupleQuery(
				QueryLanguage.SERQL, "SELECT C FROM {} rdf:type {C}", null);
		...
		assertEquals(3, countElements(con.getStatements(null, RDF.TYPE,
				RDFS.CLASS, false)));

		tupleQuery = QueryParserUtil.parseTupleQuery(QueryLanguage.SERQL,
				"SELECT P FROM {} P {}", null);
		iter = con.evaluate(tupleQuery.getTupleExpr(), null, EmptyBindingSet
				.getInstance(), false);
		...
		assertEquals(2, countElements(con.getStatements(null, RDF.TYPE,
				RDF.PROPERTY, false)));
	}
	...
	public void testRemoveAndClear() throws Exception {
		...
		assertEquals("Repository should contain 4 statements in total", 4,
				countAllElements());
		...
		assertEquals("Repository should contain 1 statement in total", 1,
				countAllElements());
		...
		assertEquals("Repository should no longer contain any statements", 0,
				countAllElements());
	}
	...
	public void testContexts() throws Exception {
		...
		assertEquals("context1 should contain 3 statements", 3,
				countContext1Elements());
		assertEquals("context2 should contain 3 statements", 3,
				countElements(con.getStatements(null, null, null, false,
						context2)));
		assertEquals("Repository should contain 8 statements", 8,
				countAllElements());
		...
		assertEquals(
				...
				5, countElements(con.getStatements(null, null, null, false,
						null, context1)));
		...
		assertEquals(
				...
				6, countElements(con.getStatements(null, null, null, false,
						context1, context2)));
		...
		assertEquals("context1 should contain 1 statements", 1,
				countContext1Elements());

		assertEquals("Repository should contain 6 statements", 6,
				countAllElements());

		assertEquals(
				"Statements without context and statements in context 1 together should total 3",
				3, countElements(con.getStatements(null, null, null, false,
						null, context1)));
		...
		assertEquals(
				...
				4, countElements(con.getStatements(null, null, null, false,
						context1, context2)));
	}
	...
	public void testMultiThreadedAccess() {
		...
		Runnable runnable = new Runnable() {
			...
			public void run() {
				...
				try {
					...
					Thread.sleep(500L);
					CloseableIteration<? extends Statement, SailException> result = sharedCon
					...
					while (result.hasNext()) {
						...
						assertTrue(st.getSubject().equals(painter)
								|| st.getSubject().equals(picasso));
						assertTrue(st.getPredicate().equals(RDF.TYPE));
						assertTrue(st.getObject().equals(RDFS.CLASS)
								|| st.getObject().equals(painter));
					}
					assertTrue(
							"we should have retrieved statements from both threads",
				...
				try {
					...
					// the connection should now be closed (by the other
					// thread),
		...
		try {
			con.addStatement(picasso, RDF.TYPE, painter);
			con.commit();
			// let this thread sleep to enable other thread to finish its
			// business.
			Thread.sleep(1000L);
			con.close();
		} catch (SailException e) {
	...
	public void testGetNamespaces() throws Exception {
		...
		CloseableIteration<? extends Namespace, SailException> namespaces = con
				.getNamespaces();
		try {
	...
	public void testDualConnections() throws Exception {
		...
		try {
			...
			ParsedTupleQuery tupleQuery = QueryParserUtil.parseTupleQuery(
					QueryLanguage.SERQL, query, null);
			assertEquals(5, countElements(con2
					.evaluate(tupleQuery.getTupleExpr(), null, EmptyBindingSet
							.getInstance(), false)));
	...
	protected int countQueryResults(String query) throws Exception {
		ParsedTupleQuery tupleQuery = QueryParserUtil.parseTupleQuery(
				QueryLanguage.SERQL, query + " using namespace ex = <"
						+ EXAMPLE_NS + ">", null);

		return countElements(con.evaluate(tupleQuery.getTupleExpr(), null,
	...
	private int verifyQueryResult(
		...
		while (resultIter.hasNext()) {
			...
			assertEquals("Wrong number of binding names for binding set",
					expectedBindingCount, resultBindings.getBindingNames()
							.size());
			...
			assertEquals("Wrong number of bindings in binding set",
					expectedBindingCount, bindingCount);
		}
