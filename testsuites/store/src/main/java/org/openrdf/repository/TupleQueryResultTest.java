...
public abstract class TupleQueryResultTest extends TestCase {
	...
	private void addData() throws IOException, UnsupportedRDFormatException,
			RDFParseException, RepositoryException {
		InputStream defaultGraph = TupleQueryResultTest.class
	...
	public void testGetBindingNames() throws Exception {
		TupleQueryResult result = con.prepareTupleQuery(QueryLanguage.SERQL,
				multipleResultQuery).evaluate();
		try {
			List<String> headers = result.getBindingNames();

			if (!headers.get(0).equals("P")) {
				fail("first header element should be 'P' but is '"
						+ headers.get(0) + "'");
			}
			if (!headers.get(1).equals("D")) {
				fail("second header element should be 'D' but is '"
						+ headers.get(1) + "'");
			}
		} finally {
			result.close();
	...
	/*
	 * deprecated public void testIsDistinct() throws Exception {
	 * TupleQueryResult result = con.prepareTupleQuery(QueryLanguage.SERQL,
	 * emptyResultQuery).evaluate();
	 * 
	 * try { if (result.isDistinct()) {
	 * fail("query result should not be distinct."); } } finally {
	 * result.close(); }
	 * 
	 * result = con.prepareTupleQuery(QueryLanguage.SERQL,
	 * singleResultQuery).evaluate();
	 * 
	 * try { if (!result.isDistinct()) {
