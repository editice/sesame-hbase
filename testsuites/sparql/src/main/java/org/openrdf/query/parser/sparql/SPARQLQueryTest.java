...
public abstract class SPARQLQueryTest extends TestCase {
	...
	public SPARQLQueryTest(String testURI, String name, String queryFileURL,
	...
	private void compareTupleQueryResults(TupleQueryResult queryResult,
		...
		if (laxCardinality) {
			resultsEqual = QueryResultUtil.isSubset(queryResultTable,
					expectedResultTable);
		} else {
	...
	private void upload(URI graphURI, Resource context) throws Exception {
		...
		try {
			RDFFormat rdfFormat = Rio.getParserFormatForFileName(graphURI
					.toString(), RDFFormat.TURTLE);
			RDFParser rdfParser = Rio.createParser(rdfFormat, dataRep
					.getValueFactory());
	...
	public interface Factory {
		...
		SPARQLQueryTest createSPARQLQueryTest(String testURI, String name,
	...
	public static TestSuite suite(String manifestFileURL, Factory factory,
		...
		query
				.append(" SELECT DISTINCT testURI, testName, resultFile, action, queryFile, defaultGraph, ordered ");
		...
		query
				.append("                             [ mf:checkOrder {ordered} ]; ");
		query
				.append("                             [ mf:requires {Requirement} ];");
		query
				.append("                             mf:action {action} qt:query {queryFile}; ");
		query
				.append("                                               [qt:data {defaultGraph}]; ");
		query
				.append("                                               [sd:entailmentRegime {Regime} ]");
		...
		query
				.append(" AND (NOT BOUND(Requirement) OR (Requirement != mf:BasicFederation)) ");
		...
		query
				.append(" FROM {testURI} mf:resultCardinality {mf:LaxCardinality}");
		query
				.append(" USING NAMESPACE mf = <http://www.w3.org/2001/sw/DataAccess/tests/test-manifest#>");
		TupleQuery laxCardinalityQuery = con.prepareTupleQuery(
				QueryLanguage.SERQL, query.toString());
		...
		while (testCases.hasNext()) {
			...
			TupleQueryResult laxCardinalityResult = laxCardinalityQuery
					.evaluate();
			try {
			...
			SPARQLQueryTest test = factory.createSPARQLQueryTest(testURI
					.toString(), testName, queryFile, resultFile, dataset,
