...
public abstract class SPARQL11SyntaxTest extends TestCase {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(SPARQL11SyntaxTest.class);
	...
	static {
		...
		sb
				.append("WHERE Type = mf:PositiveSyntaxTest11 or Type = mf:NegativeSyntaxTest11 or Type = mf:PositiveUpdateSyntaxTest11 or Type = mf:NegativeUpdateSyntaxTest11 ");
		sb.append("USING NAMESPACE");
		sb
				.append("  mf = <http://www.w3.org/2001/sw/DataAccess/tests/test-manifest#>,");
		sb
	...
	public SPARQL11SyntaxTest(String testURI, String name, String queryFileURL,
	...
	protected void runTest() throws Exception {
		...
		String query = IOUtil
				.readString(new InputStreamReader(stream, "UTF-8"));
		stream.close();
	...
	public interface Factory {
		...
		SPARQL11SyntaxTest createSPARQLSyntaxTest(String testURI,
	...
	public static Test suite(Factory factory) throws Exception {
		...
		if (REMOTE) {
			...
			URL url = SPARQL11SyntaxTest.class
					.getResource("/testcases-dawg-sparql-1.1/");
			if ("jar".equals(url.getProtocol())) {
				try {
					tmpDir = FileUtil.createTempDir("sparql-syntax");
					JarURLConnection con = (JarURLConnection) url
							.openConnection();
		...
		TupleQueryResult subManifests = con.prepareTupleQuery(
				QueryLanguage.SERQL, SUBMANIFEST_QUERY).evaluate();
		...
		for (String subManifest : subManifestList) {
			...
			while (tests.hasNext()) {
				...
				boolean positiveTest = type
						...
						|| type
								.equals("http://www.w3.org/2001/sw/DataAccess/tests/test-manifest#PositiveUpdateSyntaxTest11");

				subSuite.addTest(factory.createSPARQLSyntaxTest(testURI,
						testName, testAction, positiveTest));
			}
			tests.close();
