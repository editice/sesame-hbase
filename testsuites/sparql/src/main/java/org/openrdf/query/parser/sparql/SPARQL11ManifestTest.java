...
public class SPARQL11ManifestTest {
	...
	static final Logger logger = LoggerFactory
			.getLogger(SPARQL11ManifestTest.class);
	...
	public static TestSuite suite(SPARQLUpdateConformanceTest.Factory factory)
		...
		while (manifestResults.hasNext()) {
			...
			if (includeSubManifest(subManifestFile)) {
				suite.addTest(SPARQLUpdateConformanceTest.suite(
						subManifestFile, factory, APPROVED_TESTS_ONLY));
			}
		}
	...
	private static String getManifestFile() {
		...
		if (REMOTE) {
			...
			URL url = null;
			if (LOCAL_DAWG_TESTS) {
				url = SPARQL11ManifestTest.class
						.getResource("/testcases-dawg-sparql-1.1/manifest-all.ttl");
			} else {
				url = SPARQL11ManifestTest.class
