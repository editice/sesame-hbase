...
public abstract class SPARQLUpdateTest {
	...
	static final Logger logger = LoggerFactory
			.getLogger(SPARQLUpdateTest.class);
	...
	public void testInsertWhere() throws Exception {
		...
		assertFalse(con.hasStatement(bob, RDFS.LABEL, f.createLiteral("Bob"),
				true));
		assertFalse(con.hasStatement(alice, RDFS.LABEL, f
				.createLiteral("Alice"), true));

		operation.execute();

		assertTrue(con.hasStatement(bob, RDFS.LABEL, f.createLiteral("Bob"),
				true));
		assertTrue(con.hasStatement(alice, RDFS.LABEL,
	...
	public void testInsertWhereWithBindings2() throws Exception {
		...
		assertFalse(con.hasStatement(bob, RDFS.LABEL,
				f.createLiteral("Bobbie"), true));
		assertFalse(con.hasStatement(alice, RDFS.LABEL, null, true));

		operation.execute();

		assertTrue(con.hasStatement(bob, RDFS.LABEL, f.createLiteral("Bobbie"),
				true));
		assertFalse(con.hasStatement(alice, RDFS.LABEL, f
				.createLiteral("Alice"), true));
	}
	...
	public void testInsertNonMatchingWhere() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("INSERT { ?x rdfs:label ?y . } WHERE { ?x rdfs:comment ?y }");
	...
	public void testInsertWhereWithBindings() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("INSERT { ?x rdfs:comment ?z . } WHERE { ?x foaf:name ?y }");
	...
	public void testInsertWhereWithOptional() throws Exception {
		...
		update
				.append(" OPTIONAL {?s ex:age ?age . BIND ((?age + 1) as ?incAge)  } ");
		update.append(" } ");
		...
		RepositoryResult<Statement> result = con.getStatements(bob, age, null,
				true);
		...
		assertTrue(con.hasStatement(bob, age, f.createLiteral("43",
				XMLSchema.INTEGER), true));
	...
	public void testInsertWhereWithBlankNode() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append(" INSERT { ?s ex:complexAge [ rdf:value ?age; rdfs:label \"old\" ] . } ");
		...
		RepositoryResult<Statement> sts = con.getStatements(bob, complexAge,
				null, true);
	...
	public void testDeleteInsertWhere() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("DELETE { ?x foaf:name ?y } INSERT {?x rdfs:label ?y . } WHERE {?x foaf:name ?y }");
		...
		assertFalse(con.hasStatement(bob, FOAF.NAME, f.createLiteral("Bob"),
				true));
		assertFalse(con.hasStatement(alice, FOAF.NAME,
				f.createLiteral("Alice"), true));
	...
	public void testDeleteWhereOptional() throws Exception {
		...
		update
				.append(" OPTIONAL { ?x foaf:mbox ?mbox. FILTER (str(?mbox) = \"bob@example.org\") } }");
		...
		assertTrue(con.hasStatement(bob, FOAF.NAME, f.createLiteral("Bob"),
				true));
		assertTrue(con.hasStatement(alice, FOAF.NAME, f.createLiteral("Alice"),
				true));
	...
	public void testDeleteInsertWhereWithBindings2() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("DELETE { ?x foaf:name ?y } INSERT {?x rdfs:label ?z . } WHERE {?x foaf:name ?y }");
		...
		assertTrue(con.hasStatement(bob, RDFS.LABEL, f.createLiteral("person"),
				true));
		assertTrue(con.hasStatement(alice, RDFS.LABEL, f
				.createLiteral("person"), true));
	...
	public void testAutoCommitHandling() throws Exception {
		...
		try {
			...
			assertFalse(con2.hasStatement(bob, RDFS.LABEL, f
					.createLiteral("Bob"), true));
			assertFalse(con2.hasStatement(alice, RDFS.LABEL, f
					.createLiteral("Alice"), true));

			assertTrue(con2.hasStatement(bob, FOAF.NAME,
					f.createLiteral("Bob"), true));
			assertTrue(con2.hasStatement(alice, FOAF.NAME, f
					.createLiteral("Alice"), true));

			con.commit();

			// after commit, update should be visible to separate connection.
			assertTrue(con2.hasStatement(bob, RDFS.LABEL, f
					.createLiteral("Bob"), true));
			assertTrue(con2.hasStatement(alice, RDFS.LABEL, f
					.createLiteral("Alice"), true));

			assertFalse(con2.hasStatement(bob, FOAF.NAME, f
					.createLiteral("Bob"), true));
			assertFalse(con2.hasStatement(alice, FOAF.NAME, f
					.createLiteral("Alice"), true));
	...
	public void testConsecutiveUpdatesInSameTransaction() throws Exception {
		...
		con.setAutoCommit(false);
		Update operation = con.prepareUpdate(QueryLanguage.SPARQL, update1
				.toString());
	...
	public void testInsertTransformedWhere() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("INSERT {?x rdfs:label [] . } WHERE {?y ex:containsPerson ?x.  }");
	...
	public void testInsertWhereGraph() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("INSERT {GRAPH ?g {?x rdfs:label ?y . }} WHERE {GRAPH ?g {?x foaf:name ?y }}");
		...
		assertTrue(message, con.hasStatement(bob, RDFS.LABEL, f
				.createLiteral("Bob"), true, graph1));
		assertFalse(message, con.hasStatement(bob, RDFS.LABEL, f
				.createLiteral("Bob"), true, graph2));
		assertTrue(message, con.hasStatement(alice, RDFS.LABEL, f
				.createLiteral("Alice"), true, graph2));
		assertFalse(message, con.hasStatement(alice, RDFS.LABEL, f
				.createLiteral("Alice"), true, graph1));
	}
	...
	public void testInsertWhereUsing() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("INSERT {?x rdfs:label ?y . } USING ex:graph1 WHERE {?x foaf:name ?y }");
	...
	public void testInsertWhereWith() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("WITH ex:graph1 INSERT {?x rdfs:label ?y . } WHERE {?x foaf:name ?y }");
	...
	public void testDeleteWhereShortcut() throws Exception {
		...
		assertFalse(msg, con.hasStatement(bob, FOAF.NAME, f
				.createLiteral("Bob"), true));
		assertFalse(msg, con.hasStatement(alice, FOAF.NAME, f
				.createLiteral("Alice"), true));
	...
	public void testDeleteTransformedWhere() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("DELETE {?y foaf:name ?n } WHERE {?x ex:containsPerson ?y . ?y foaf:name ?n . }");
		...
		assertTrue(msg, con.hasStatement(graph1, f.createURI(EX_NS,
				"containsPerson"), bob, true));
		assertTrue(msg, con.hasStatement(graph2, f.createURI(EX_NS,
				"containsPerson"), alice, true));
	...
	public void testInsertData() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("INSERT DATA { ex:book1 dc:title \"book 1\" ; dc:creator \"Ringo\" . } ");
		...
		assertFalse(con.hasStatement(book1, DC.TITLE,
				f.createLiteral("book 1"), true));
		assertFalse(con.hasStatement(book1, DC.CREATOR, f
				.createLiteral("Ringo"), true));

		operation.execute();

		String msg = "two new statements about ex:book1 should have been inserted";
		assertTrue(msg, con.hasStatement(book1, DC.TITLE, f
				.createLiteral("book 1"), true));
		assertTrue(msg, con.hasStatement(book1, DC.CREATOR, f
				.createLiteral("Ringo"), true));
	}
	...
	public void testInsertDataBlankNode() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("INSERT DATA { _:foo dc:title \"book 1\" ; dc:creator \"Ringo\" . } ");

		Update operation = con.prepareUpdate(QueryLanguage.SPARQL, update
				.toString());

		assertFalse(con.hasStatement(null, DC.TITLE, f.createLiteral("book 1"),
				true));
		assertFalse(con.hasStatement(null, DC.CREATOR,
				f.createLiteral("Ringo"), true));

		operation.execute();

		RepositoryResult<Statement> titleStatements = con.getStatements(null,
				DC.TITLE, f.createLiteral("book 1"), true);
		assertNotNull(titleStatements);

		RepositoryResult<Statement> creatorStatements = con.getStatements(null,
				DC.CREATOR, f.createLiteral("Ringo"), true);
	...
	public void testInsertDataMultiplePatterns() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("INSERT DATA { ex:book1 dc:title \"book 1\". ex:book1 dc:creator \"Ringo\" . ex:book2 dc:creator \"George\". } ");
		...
		assertFalse(con.hasStatement(book1, DC.CREATOR, f
				.createLiteral("Ringo"), true));
		assertFalse(con.hasStatement(book2, DC.CREATOR, f
				.createLiteral("George"), true));
		...
		assertTrue(msg, con.hasStatement(book1, DC.CREATOR, f
				.createLiteral("Ringo"), true));
		assertTrue(msg, con.hasStatement(book2, DC.CREATOR, f
				.createLiteral("George"), true));
	}
	...
	public void testInsertDataInGraph() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("INSERT DATA { GRAPH ex:graph1 { ex:book1 dc:title \"book 1\" ; dc:creator \"Ringo\" . } } ");
		...
		assertFalse(con.hasStatement(book1, DC.TITLE,
				f.createLiteral("book 1"), true, graph1));
		assertFalse(con.hasStatement(book1, DC.CREATOR, f
				.createLiteral("Ringo"), true, graph1));

		operation.execute();

		String msg = "two new statements about ex:book1 should have been inserted in graph1";
		assertTrue(msg, con.hasStatement(book1, DC.TITLE, f
				.createLiteral("book 1"), true, graph1));
		assertTrue(msg, con.hasStatement(book1, DC.CREATOR, f
				.createLiteral("Ringo"), true, graph1));
	}
	...
	public void testInsertDataInGraph2() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("INSERT DATA { GRAPH ex:graph1 { ex:Human rdfs:subClassOf ex:Mammal. ex:Mammal rdfs:subClassOf ex:Animal. ex:george a ex:Human. ex:ringo a ex:Human. } } ");
		...
		assertTrue(con.hasStatement(human, RDFS.SUBCLASSOF, mammal, true,
				graph1));
		assertTrue(con
				.hasStatement(mammal, RDFS.SUBCLASSOF, null, true, graph1));
	...
	public void testDeleteDataMultiplePatterns() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("DELETE DATA { ex:alice foaf:knows ex:bob. ex:alice foaf:mbox \"alice@example.org\" .} ");

		Update operation = con.prepareUpdate(QueryLanguage.SPARQL, update
				.toString());

		assertTrue(con.hasStatement(alice, FOAF.KNOWS, bob, true));
		assertTrue(con.hasStatement(alice, FOAF.MBOX, f
				.createLiteral("alice@example.org"), true));
		operation.execute();

		String msg = "statement should have been deleted.";
		assertFalse(msg, con.hasStatement(alice, FOAF.KNOWS, bob, true));
		assertFalse(msg, con.hasStatement(alice, FOAF.MBOX, f
				.createLiteral("alice@example.org"), true));
	}
	...
	public void testDeleteDataFromGraph() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("DELETE DATA { GRAPH ex:graph1 {ex:alice foaf:knows ex:bob. } } ");
	...
	public void testDeleteDataFromWrongGraph() throws Exception {
		...
		update
				.append("DELETE DATA { GRAPH ex:graph2 {ex:alice foaf:knows ex:bob. } } ");
	...
	public void testCopyToDefault() throws Exception {
		...
		assertTrue(con.hasStatement(graph1, DC.PUBLISHER, null, false,
				(Resource) null));
		assertTrue(con.hasStatement(graph2, DC.PUBLISHER, null, false,
				(Resource) null));
		operation.execute();
		assertFalse(con.hasStatement(graph1, DC.PUBLISHER, null, false,
				(Resource) null));
		assertFalse(con.hasStatement(graph2, DC.PUBLISHER, null, false,
				(Resource) null));
		assertTrue(con.hasStatement(bob, FOAF.NAME, null, false,
				(Resource) null));
	...
	public void testCopyToNewNamed() throws Exception {
		...
		operation.execute();
		assertTrue(con.hasStatement(bob, FOAF.NAME, null, false, f.createURI(
				EX_NS, "graph3")));
		assertTrue(con.hasStatement(bob, FOAF.NAME, null, false, graph1));
	}
	...
	public void testCopyFromDefault() throws Exception {
		...
		assertTrue(con.hasStatement(graph2, DC.PUBLISHER, null, false,
				(Resource) null));
		assertTrue(con.hasStatement(graph1, DC.PUBLISHER, null, false, f
				.createURI(EX_NS, "graph3")));
		assertTrue(con.hasStatement(graph2, DC.PUBLISHER, null, false, f
				.createURI(EX_NS, "graph3")));
	...
	public void testUpdateSequenceDeleteInsert() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("INSERT {?x foaf:name \"foo\" } WHERE {?y ex:containsPerson ?x} ");
		...
		assertTrue(msg, con.hasStatement(bob, FOAF.NAME,
				f.createLiteral("foo"), true));
		assertTrue(msg, con.hasStatement(alice, FOAF.NAME, f
				.createLiteral("foo"), true));
	}
	...
	public void testUpdateSequenceInsertDelete2() throws Exception {
		...
		update.append(getNamespaceDeclarations());
		update
				.append("INSERT { GRAPH ex:graph2 { ?s ?p ?o } } WHERE { GRAPH ex:graph1 { ?s ?p ?o . FILTER (?s = ex:bob) } }; ");
		update
				.append("WITH ex:graph1 DELETE { ?s ?p ?o } WHERE {?s ?p ?o . FILTER (?s = ex:bob) } ");
		...
		assertTrue(msg, con.hasStatement(bob, FOAF.NAME,
				f.createLiteral("Bob"), true, graph2));
	...
	public void testUpdateSequenceInsertDeleteExample9() throws Exception {
		...
		update
				.append("INSERT  { GRAPH <http://example/bookStore2> { ?book ?p ?v } } ");
		update.append(" WHERE ");
		update.append(" { GRAPH  <http://example/bookStore> ");
		update.append("   { ?book dc:date ?date . ");
		update
				.append("       FILTER ( ?date < \"2000-01-01T00:00:00-02:00\"^^xsd:dateTime ) ");
		...
		assertTrue(msg, con.hasStatement(book1, RDF.TYPE, null, true,
				bookStore2));
		assertTrue(msg, con
				.hasStatement(book1, DC.DATE, null, true, bookStore2));
		assertTrue(msg, con.hasStatement(book1, DC.TITLE, null, true,
				bookStore2));
	}

	/*
	 * @Test public void testLoad() throws Exception { String update =
	 * "LOAD <http://www.daml.org/2001/01/gedcom/royal92.daml>";
	 * 
	 * String ns = "http://www.daml.org/2001/01/gedcom/gedcom#";
	 * 
	 * Update operation = con.prepareUpdate(QueryLanguage.SPARQL, update);
	 * 
	 * operation.execute(); assertTrue(con.hasStatement(null, RDF.TYPE,
	 * f.createURI(ns, "Family"), true)); }
	 * 
	 * @Test public void testLoadIntoGraph() throws Exception { String ns =
	 * "http://www.daml.org/2001/01/gedcom/gedcom#";
	 * 
	 * String update =
	 * "LOAD <http://www.daml.org/2001/01/gedcom/royal92.daml> INTO GRAPH <" +
	 * ns + "> ";
	 * 
	 * Update operation = con.prepareUpdate(QueryLanguage.SPARQL, update);
	 * 
	 * operation.execute(); assertFalse(con.hasStatement((Resource)null,
	 * RDF.TYPE, f.createURI(ns, "Family"), true, (Resource)null));
	 * assertTrue(con.hasStatement((Resource)null, RDF.TYPE, f.createURI(ns,
	 * "Family"), true, f.createURI(ns))); }
	 */

	/* protected methods */

	protected void loadDataset(String datasetFile) throws RDFParseException,
			RepositoryException, IOException {
		logger.debug("loading dataset...");
		InputStream dataset = SPARQLUpdateTest.class
