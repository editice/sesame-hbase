...
public abstract class ComplexSPARQLQueryTest {
	...
	static final Logger logger = LoggerFactory
			.getLogger(ComplexSPARQLQueryTest.class);
	...
	public void testSameTermRepeatInUnion() throws Exception {
		...
		try {
			tq = conn.prepareTupleQuery(QueryLanguage.SPARQL, query.toString());
			tq.setBinding("william", conn.getValueFactory().createURI(
	...
	public void testPropertyPathInTree() throws Exception {
		...
		query
				.append(" WHERE { ?node ex:hasParent+ ex:b . ?node ex:name ?name . }");
	...
	public void testFilterRegexBoolean() throws Exception {
		...
		query
				.append("            FILTER(REGEX(?name, \"Bo\") && REGEX(?mbox, \"bob\")) ");
	...
	public void testArbitraryLengthPathWithBinding1() throws Exception {
		...
		query
				.append("WHERE { ?child a owl:Class . ?child rdfs:subClassOf+ ?parent . }");
	...
	public void testArbitraryLengthPathWithFilter1() throws Exception {
		...
		query.append("SELECT ?parent ?child ");
		query
				.append("WHERE { ?child a owl:Class . ?child rdfs:subClassOf+ ?parent . FILTER (?parent = owl:Thing) }");
	...
	public void testArbitraryLengthPathWithFilter2() throws Exception {
		...
		query.append("SELECT ?parent ?child ");
		query
				.append("WHERE { ?child rdfs:subClassOf+ ?parent . FILTER (?parent = owl:Thing) }");
	...
	public void testArbitraryLengthPathWithFilter3() throws Exception {
		...
		query.append("SELECT ?parent ?child ");
		query
				.append("WHERE { ?child rdfs:subClassOf+ ?parent . FILTER (?child = <http://example.org/PO_0025117>) }");
	...
	protected void loadTestData(String dataFile) throws RDFParseException,
			RepositoryException, IOException {
		logger.debug("loading dataset {}", dataFile);
		InputStream dataset = ComplexSPARQLQueryTest.class
