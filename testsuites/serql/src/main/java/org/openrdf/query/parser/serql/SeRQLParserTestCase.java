...
public abstract class SeRQLParserTestCase extends TestCase {
	...
	static final Logger logger = LoggerFactory
			.getLogger(SeRQLParserTestCase.class);
	...
	public static Test suite(Factory factory) throws Exception {
		...
		while (tests.hasNext()) {
			...
			if (MFX_CORRECT.equals(result)) {
				positiveTests.addTest(factory.createTest(testName, queryFile,
						result));
			} else if (MFX_PARSE_ERROR.equals(result)) {
				negativeTests.addTest(factory.createTest(testName, queryFile,
						result));
			} else {
				logger.warn("Unexpected result value for test \"" + testName
						+ "\": " + result);
			}
		}
