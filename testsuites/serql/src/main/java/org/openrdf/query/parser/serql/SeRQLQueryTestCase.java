...
public abstract class SeRQLQueryTestCase extends TestCase {
	...
	static final Logger logger = LoggerFactory
			.getLogger(SeRQLQueryTestCase.class);
	...
	public interface Factory {
		Test createTest(String name, String dataFile, List<String> graphNames,
				String queryFile, String resultFile, String entailment);
	}

	/**
	 * Creates a new SeRQL Query test.
	 */
	public SeRQLQueryTestCase(String name, String dataFile,
	...
	protected void runTest() throws Exception {
		...
		// Add unnamed graph
		dataCon.add(url(dataFile), base(dataFile), RDFFormat
				.forFileName(dataFile));

		// add named graphs
		for (String graphName : graphNames) {
			dataCon.add(url(graphName), base(graphName), RDFFormat
					.forFileName(graphName), new URIImpl(graphName));
		}

		// Evaluate the query on the query data
		GraphQueryResult result = dataCon.prepareGraphQuery(getQueryLanguage(),
				query).evaluate();
		...
		erCon.add(url(resultFile), base(resultFile), RDFFormat
				.forFileName(resultFile));

		Collection<Statement> expectedStatements = Iterations.addAll(erCon
		...
		if (!ModelUtil.equals(actualStatements, expectedStatements)) {
			...
			Collection<? extends Statement> diff = RepositoryUtil.difference(
			...
			diff = RepositoryUtil.difference(expectedStatements,
					actualStatements);
			if (diff.size() != 0) {
	...
	public static Test suite(Factory factory) throws Exception {
		...
		RDFFormat format = RDFFormat.forFileName(MANIFEST_FILE,
				RDFFormat.TURTLE);
		con.add(manifestURL, base(manifestURL.toExternalForm()), format);

		String query = "SELECT testName, entailment, input, query, result "
				+ "FROM {} mf:name {testName};"
				+ "        mf:result {result}; "
				+ "        tck:entailment {entailment}; "
		...
		TupleQueryResult tests = con.prepareTupleQuery(QueryLanguage.SERQL,
				query).evaluate();
		while (tests.hasNext()) {
			BindingSet testBindings = tests.next();
			String testName = ((Literal) testBindings.getValue("testName"))
					.getLabel();
			...
			String entailment = ((Literal) testBindings.getValue("entailment"))
					.getLabel();

			query = "SELECT graph "
					+ "FROM {} mf:name {testName}; "
					+ "        mf:action {} qt:graphData {graph} "
					+ "WHERE testName = \""
					+ SeRQLUtil.encodeString(testName)
					+ "\" "
					+ "USING NAMESPACE"
					+ "  mf = <http://www.w3.org/2001/sw/DataAccess/tests/test-manifest#>,"
					+ "  qt = <http://www.w3.org/2001/sw/DataAccess/tests/test-query#>";
			...
			TupleQueryResult graphs = con.prepareTupleQuery(
			...
			if (testName.startsWith("test-029:")) {
				logger.error("test-029 SKIPPED in {}", SeRQLQueryTestCase.class
						.getName());
				continue;
			}
			suite.addTest(factory.createTest(testName, inputFile, graphNames,
