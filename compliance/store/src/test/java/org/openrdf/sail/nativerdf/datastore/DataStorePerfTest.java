...
public class DataStorePerfTest {
	...
	public static void main(String[] args) throws Exception {
		...
		System.out
				.println("Performance test results, average times in micro seconds");
		System.out.println("#str\tstore\tgetID\tgetData");
		for (long[] timeData : timeDataList) {
			System.out.printf("%d\t%d\t%d\t%d", timeData[0],
					timeData[1] / 1000, timeData[2] / 1000, timeData[3] / 1000);
			System.out.println();
		}
	}

	private static long[] runPerformanceTest(int stringCount) throws Exception {
		System.out.println("Running performance test with " + stringCount
				+ " strings...");
		...
		try {
			System.out.println("Initializing data store in directory "
					+ dataDir);
			...
			System.out.println("Strings stored in " + (endTime - startTime)
					/ 1E6 + " ms");
			...
			for (int i = 1; i <= stringCount; i++) {
				...
				if (sID == -1) {
					throw new RuntimeException("Failed to get ID for string \""
							+ i + "\"");
				}
			}
			...
			System.out.println("All IDs fetched in " + (endTime - startTime)
					/ 1E6 + " ms");
			...
			for (int id = 1; id <= stringCount; id++) {
				...
				if (s == null) {
					throw new RuntimeException("Failed to get data for ID "
							+ id);
				}
			}
			...
			System.out.println("All data fetched in " + (endTime - startTime)
					/ 1E6 + " ms");
