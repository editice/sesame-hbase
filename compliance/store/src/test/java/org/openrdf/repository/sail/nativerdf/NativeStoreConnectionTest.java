...
public class NativeStoreConnectionTest extends RepositoryConnectionTest {
	...
	public void testSES715() throws Exception {
		...
		for (int j = 0; j < 1000; j++) {
			testCon.add(vf.createURI("http://my.subject" + j), predicate,
					object, context1);
			testCon.add(vf.createURI("http://my.subject" + j), predicate,
					object, context2);
		}
		assertEquals(1000, testCon.getStatements(null, null, null, false,
				context1).asList().size());
		assertEquals(1000, testCon.getStatements(null, null, null, false,
				context2).asList().size());

		// remove all triples from context 1
		testCon.clear(context1);
		assertEquals(0, testCon
				.getStatements(null, null, null, false, context1).asList()
				.size());
		assertEquals(1000, testCon.getStatements(null, null, null, false,
				context2).asList().size());
		testCon.commit();
		...
		assertEquals(0, testCon2.getStatements(null, null, null, false,
				context1).asList().size());
		assertEquals(1000, testCon2.getStatements(null, null, null, false,
				context2).asList().size());
