...
public class ArbitraryLengthPathTest extends TestCase {
	...
	public void test10() throws Exception {
		...
		assertTrue(con.prepareBooleanQuery(QueryLanguage.SPARQL, sparql)
				.evaluate());
	}
	...
	private void populate(int n) throws RepositoryException {
		...
		for (int i = 0; i < n; i++) {
			con.add(vf.createURI("urn:test:root"), vf
					.createURI("urn:test:hasChild"), vf
					.createURI("urn:test:node" + i));
		}
		con.add(vf.createURI("urn:test:root"), vf
				.createURI("urn:test:hasChild"), vf
				.createURI("urn:test:node-end"));
	}
