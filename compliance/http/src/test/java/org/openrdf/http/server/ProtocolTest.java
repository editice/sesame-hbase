...
public class ProtocolTest extends TestCase {
	...
	public void testRepository_PUT() throws Exception {
		putFile(Protocol.getStatementsLocation(TestServer.REPOSITORY_URL),
	...
	public void testNullContext_PUT() throws Exception {
		String location = Protocol
				.getStatementsLocation(TestServer.REPOSITORY_URL);
		location += "?" + Protocol.CONTEXT_PARAM_NAME + "="
	...
	public void testNamedContext_PUT() throws Exception {
		String location = Protocol
				.getStatementsLocation(TestServer.REPOSITORY_URL);
		String encContext = Protocol.encodeValue(new URIImpl(
				"urn:x-local:graph1"));
	...
	public void testSeRQLselect() throws Exception {
		TupleQueryResult queryResult = evaluate(TestServer.REPOSITORY_URL,
				"select * from {X} P {Y}", QueryLanguage.SERQL);
		QueryResultIO.write(queryResult, TupleQueryResultFormat.SPARQL,
				System.out);
	}
	...
	private void putFile(String location, String file) throws Exception {
		...
		conn
				.setRequestProperty("Content-Type", dataFormat
						.getDefaultMIMEType());
		...
		if (responseCode != HttpURLConnection.HTTP_OK && // 200 OK
				responseCode != HttpURLConnection.HTTP_NO_CONTENT) // 204 NO
																	// CONTENT
		{
			String response = "location " + location + " responded: "
	...
	private TupleQueryResult evaluate(String location, String query,
			QueryLanguage queryLn) throws Exception {
		location += "?query=" + URLEncoder.encode(query, "UTF-8") + "&queryLn="
				+ queryLn.getName();
		...
		conn.setRequestProperty("Accept", TupleQueryResultFormat.SPARQL
				.getDefaultMIMEType());
		...
		try {
			...
			if (responseCode == HttpURLConnection.HTTP_OK) {
				...
				return QueryResultIO.parse(conn.getInputStream(),
