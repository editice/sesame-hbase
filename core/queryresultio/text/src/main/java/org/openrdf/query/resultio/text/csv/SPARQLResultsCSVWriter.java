...
public class SPARQLResultsCSVWriter implements TupleQueryResultWriter {
	...
	private void writeLiteral(Literal literal) throws IOException {
		...
		if (datatype != null
				&& (XMLDatatypeUtil.isIntegerDatatype(datatype)
						|| XMLDatatypeUtil.isDecimalDatatype(datatype) || XMLSchema.DOUBLE
						.equals(datatype))) {
			try {
				...
				// not a valid numeric datatyped literal. ignore error and write
				// as
