...
public class TupleQueryResultSerializationTest extends TestCase {
	...
	public void test3x2Result() throws Exception {
		MutableTupleQueryResult tqr = createQueryResult();
		testResultSerialization(tqr, TupleQueryResultFormat.BINARY);
	}

	public void testEmptyBindingSetResult() throws Exception {
		MutableTupleQueryResult tqr = createEmptyBindingSetResult();
		testResultSerialization(tqr, TupleQueryResultFormat.BINARY);
	}

	public void testEmptyQueryResult() throws Exception {
		MutableTupleQueryResult tqr = new MutableTupleQueryResult(Collections
				.<String> emptyList());
		testResultSerialization(tqr, TupleQueryResultFormat.BINARY);
	}

	private void testResultSerialization(MutableTupleQueryResult input,
			TupleQueryResultFormat format) throws Exception {
