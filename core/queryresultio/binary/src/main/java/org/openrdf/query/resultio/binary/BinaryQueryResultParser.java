...
public class BinaryQueryResultParser extends TupleQueryResultParserBase {
	...
	public synchronized void parse(InputStream in) throws IOException,
		...
		if (!Arrays.equals(magicNumber, MAGIC_NUMBER)) {
			throw new QueryResultParseException(
					"File does not contain a binary RDF table result");
		}
		...
		if (formatVersion > FORMAT_VERSION && formatVersion < 1) {
			throw new QueryResultParseException("Incompatible format version: "
					+ formatVersion);
		}
		...
		if (columnCount < 0) {
			throw new QueryResultParseException(
					"Illegal column count specified: " + columnCount);
		}
		...
		List<Value> previousTuple = Collections.nCopies(columnCount,
				(Value) null);
		...
		while (recordTypeMarker != TABLE_END_RECORD_MARKER) {
			if (recordTypeMarker == ERROR_RECORD_MARKER) {
				...
				case DATATYPE_LITERAL_RECORD_MARKER:
					...
					break;
				default:
					throw new IOException("Unkown record type: "
							+ recordTypeMarker);
				}
				...
				if (currentTuple.size() == columnCount) {
					...
					handler.handleSolution(new ListBindingSet(columnHeaders,
							previousTuple));
				}
			}
	...
	private void processError() throws IOException, QueryResultParseException {
		...
		if (errTypeFlag == MALFORMED_QUERY_ERROR) {
			...
			throw new QueryResultParseException("Unkown error type: "
					+ errTypeFlag);
		}
	...
	private void processNamespace() throws IOException {
		...
		if (namespaceID >= namespaceArray.length) {
			...
			System.arraycopy(namespaceArray, 0, newArray, 0,
	...
	private Literal readLiteral(int recordTypeMarker) throws IOException,
		...
		if (recordTypeMarker == DATATYPE_LITERAL_RECORD_MARKER) {
			...
			default:
				throw new QueryResultParseException(
						"Illegal record type marker for literal's datatype");
			}
