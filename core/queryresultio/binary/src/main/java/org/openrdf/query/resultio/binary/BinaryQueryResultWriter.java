...
public class BinaryQueryResultWriter implements TupleQueryResultWriter {
	 ...
	 */
	private Map<String, Integer> namespaceTable = new HashMap<String, Integer>(
			32);
	...
	public void startQueryResult(List<String> bindingNames)
		...
		try {
			...
			List<Value> nullTuple = Collections.nCopies(this.bindingNames
					.size(), (Value) null);
		...
		try {
			if (bindingSet.size() == 0) {
				...
				for (String bindingName : bindingNames) {
					...
					if (value == null) {
						writeNull();
					} else if (value.equals(previousBindings
