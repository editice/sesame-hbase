...
public class BooleanQueryResultFormat extends FileFormat {
	 ...
	 */
	public static final BooleanQueryResultFormat SPARQL = new BooleanQueryResultFormat(
			"SPARQL/XML", "application/sparql-results+xml", Charset
					.forName("UTF-8"), "srx");

	/**
	 * Plain text encoding using values "true" and "false" (case-insensitive).
	 */
	public static final BooleanQueryResultFormat TEXT = new BooleanQueryResultFormat(
			"TEXT", "text/boolean", Charset.forName("US-ASCII"), "txt");
	 ...
	 */
	private static List<BooleanQueryResultFormat> VALUES = new ArrayList<BooleanQueryResultFormat>(
			8);
	...
	public static BooleanQueryResultFormat register(String name,
		...
		BooleanQueryResultFormat format = new BooleanQueryResultFormat(name,
				mimeType, fileExt);
		register(format);
		return format;
	}
	...
	/**
	 * Tries to determine the appropriate boolean file format based on the a
	 * MIME type that describes the content type.
	 * 
	 * @param mimeType
	...
	/**
	 * Tries to determine the appropriate boolean file format based on the a
	 * MIME type that describes the content type. The supplied fallback format
	 * will be returned when the MIME type was not recognized.
	 * 
	 * @param mimeType
	 *            a MIME type, e.g. "application/sparql-results+xml"
	 * @param fallback
	 ...
	 *            for SPARQL/XML.
	 */
	public BooleanQueryResultFormat(String name, String mimeType,
	 ...
	 */
	public BooleanQueryResultFormat(String name, Collection<String> mimeTypes,
