...
public class SPARQLResultsXMLParser extends TupleQueryResultParserBase {
	...
	/**
	 * Creates a new parser for the SPARQL Query Results XML Format that will
	 * use an instance of {@link ValueFactoryImpl} to create Value objects.
	 */
	...
	/**
	 * Creates a new parser for the SPARQL Query Results XML Format that will
	 * use the supplied ValueFactory to create Value objects.
	 */
	...
	private class SPARQLResultsParser extends SimpleSAXAdapter {
		...
		@Override
		public void startTag(String tagName, Map<String, String> atts,
			...
			if (BINDING_TAG.equals(tagName)) {
				...
				if (currentBindingName == null) {
					throw new SAXException(BINDING_NAME_ATT
							+ " attribute missing for " + BINDING_TAG
							+ " element");
				}
				...
				if (varName == null) {
					throw new SAXException(VAR_NAME_ATT + " missing for "
							+ VAR_TAG + " element");
				}
		...
		public void endTag(String tagName) throws SAXException {
			if (BINDING_TAG.equals(tagName)) {
				if (currentValue == null) {
					throw new SAXException("Value missing for " + BINDING_TAG
							+ " element");
				}
