...
public class ListBindingSet implements BindingSet {
	 ...
	 * supplying it to this solution. The number of supplied values must be
	 * equal to the number of the binding names.
	 * 
	 * @param names
	 *            The binding names.
	 * @param values
	...
	private class ListBindingSetIterator implements Iterator<Binding> {
		...
		public Binding next() {
			Binding result = new BindingImpl(bindingNames.get(index), values
					.get(index));
			findNextElement();
			return result;
		}
