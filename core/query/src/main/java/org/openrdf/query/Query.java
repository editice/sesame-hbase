...
public interface Query extends Operation {
	 ...
	 * be interrupted when it exceeds the time limit. Any consecutive requests
	 * to fetch query results will result in {@link QueryInterruptedException}s.
	 * 
	 * @param maxQueryTime
	 *            The maximum query time, measured in seconds. A negative or
	 *            zero value indicates an unlimited query time (which is the
	 *            default).
	 */
