...
public class QueryResultUtil {
	...
	public static void report(GraphQueryResult gqr, RDFHandler rdfHandler)
		...
		try {
			...
			for (Map.Entry<String, String> entry : gqr.getNamespaces()
					.entrySet()) {
	...
	public static boolean equals(GraphQueryResult result1,
			GraphQueryResult result2) throws QueryEvaluationException {
	...
	private static boolean matchBindingSets(
			List<? extends BindingSet> queryResult1,
			Iterable<? extends BindingSet> queryResult2) {
		return matchBindingSets(queryResult1, queryResult2,
				new HashMap<BNode, BNode>(), 0);
	}
	...
	private static boolean matchBindingSets(
		...
		if (idx < queryResult1.size()) {
			...
			List<BindingSet> matchingBindingSets = findMatchingBindingSets(bs1,
					queryResult2, bNodeMapping);

			for (BindingSet bs2 : matchingBindingSets) {
				// Map bNodes in bs1 to bNodes in bs2
				Map<BNode, BNode> newBNodeMapping = new HashMap<BNode, BNode>(
						bNodeMapping);
				...
				// Enter recursion
				result = matchBindingSets(queryResult1, queryResult2,
						newBNodeMapping, idx + 1);
	...
	private static boolean bindingSetsMatch(BindingSet bs1, BindingSet bs2,
			...
			if (value1 instanceof BNode && value2 instanceof BNode) {
				...
				if (value1 instanceof Literal && value2 instanceof Literal) {
					...
					if (dt1 != null
							&& dt2 != null
							&& dt1.equals(dt2)
							&& XMLDatatypeUtil.isValidValue(leftLit.getLabel(),
									dt1)
							&& XMLDatatypeUtil.isValidValue(
									rightLit.getLabel(), dt2)) {
						Integer compareResult = null;
						if (dt1.equals(XMLSchema.DOUBLE)) {
							compareResult = Double.compare(leftLit
									.doubleValue(), rightLit.doubleValue());
							...
							XMLGregorianCalendar right = rightLit
									.calendarValue();
	 ...
	 * are compatible if they have equal values for each binding name that
	 * occurs in both binding sets.
	 */
