...
public class DAWGTestResultSetParser extends RDFHandlerBase {
	...
	public void endRDF() throws RDFHandlerException {
		try {
			Resource resultSetNode = GraphUtil.getUniqueSubject(graph,
					RDF.TYPE, RESULTSET);

			List<String> bindingNames = getBindingNames(resultSetNode);
			tqrHandler.startQueryResult(bindingNames);

			Iterator<Value> solIter = GraphUtil.getObjectIterator(graph,
					resultSetNode, SOLUTION);
			while (solIter.hasNext()) {
				...
				if (solutionNode instanceof Resource) {
					...
					throw new RDFHandlerException("Value for " + SOLUTION
							+ " is not a resource: " + solutionNode);
				}
			}
	...
	private List<String> getBindingNames(Resource resultSetNode)
		...
		Iterator<Value> varIter = GraphUtil.getObjectIterator(graph,
				resultSetNode, RESULTVARIABLE);
		...
		while (varIter.hasNext()) {
			...
			if (varName instanceof Literal) {
				...
				throw new RDFHandlerException("Value for " + RESULTVARIABLE
						+ " is not a literal: " + varName);
			}
		}
	...
	private void reportSolution(Resource solutionNode, List<String> bindingNames)
		...
		Iterator<Value> bindingIter = GraphUtil.getObjectIterator(graph,
				solutionNode, BINDING);
		while (bindingIter.hasNext()) {
			...
			if (bindingNode instanceof Resource) {
				...
				throw new RDFHandlerException("Value for " + BINDING
						+ " is not a resource: " + bindingNode);
			}
		}
	...
	private Binding getBinding(Resource bindingNode) throws GraphUtilException {
		Literal name = GraphUtil.getUniqueObjectLiteral(graph, bindingNode,
				VARIABLE);
