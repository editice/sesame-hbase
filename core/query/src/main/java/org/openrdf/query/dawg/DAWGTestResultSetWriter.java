...
public class DAWGTestResultSetWriter implements TupleQueryResultHandler {
	...
	public void handleSolution(BindingSet bindingSet)
		...
		try {
			...
			for (Binding binding : bindingSet) {
				...
				reportStatement(bindingNode, VARIABLE, vf.createLiteral(binding
						.getName()));

				Value value = binding.getValue();

				// Map bnodes to new bnodes to prevent collisions with the
				// bnodes
	...
	private void reportStatement(Resource subject, URI predicate, Value object)
		...
		rdfHandler.handleStatement(vf.createStatement(subject, predicate,
				object));
	}
}
