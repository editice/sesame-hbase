...
/**
 * TimerTask that keeps a weak reference to the supplied iteration and when
 * activated, interrupts it.
 * 
...
class InterruptTask<E, X extends Exception> extends TimerTask {
	...
	public InterruptTask(TimeLimitIteration<E, X> iteration) {
		this.iterationRef = new WeakReference<TimeLimitIteration<E, X>>(
				iteration);
	}
