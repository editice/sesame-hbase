...
/**
 * An Iterator that converts an iterator over objects of type <tt>S</tt> (the
 * source type) to an iterator over objects of type <tt>T</tt> (the target
 * type).
 */
public abstract class ConvertingIterator<S, T> implements Iterator<T> {
	 ...
	 * 
	 * @param iter
	 *            The source type itertor for this <tt>ConvertingIterator</tt>,
	 ...
	 *             operation.
	 * @throws IllegalStateException
	 *             If the itertor has been closed, or if {@link #next} has not
