...
public class ResourceUtil {
	...
	public static Set<URL> getURLs(String resourceName) throws IOException {
		...
		// most likely to succeed
		addAll(result, Thread.currentThread().getContextClassLoader()
	...
	public static InputStream getInputStream(String resourceName) {
		...
		if (result == null) {
			...
			if (result == null) {
				result = caller.getClassLoader().getResourceAsStream(
						resourceName);
				if (result == null) {
					// last resort: the system classloader
					result = ClassLoader
	 ...
	 * @throws IOException
	 *             if there was a problem reading the resource or writing to the
	 *             file
	 */
	...
	private static Class<?> getCaller() {
		...
		if (callStack.length > 0) {
			int index = 0;
			// look for the first class on the stack that isn't this class or
			// the
			// inner utility class (it's most likely stack[2], but we don't want
			// to
	...
	private static final class CallerResolver extends SecurityManager {
		...
		static {
			try {
				...
				throw new RuntimeException("Could not create CallerResolver: "
						+ se);
			}
		}
