...
public class FileUtil {
	...
	public static String getRelativePath(File subj, File relativeTo) {
		...
		StringTokenizer subjPathTok = new StringTokenizer(subjPath,
				File.separator);
		StringTokenizer relativeToPathTok = new StringTokenizer(relativeToPath,
				File.separator);
	 ...
	 * @param excludes
	 *            The File objects to be excluded; if a directory is excluded,
	 *            all files under it are excluded as well!
	 * @return Whether moving was succesfull
	 */
	public static boolean moveRecursive(File from, File to,
		...
		if (from.isFile()) {
			...
			for (int i = list.length; i-- > 0;) {
				...
				if (excludes != null && excludes.contains(listItem)) {
					...
					if (!moveRecursive(listItem, new File(to, listItem
							.getName()), excludes)) {
						result = false;
					}
				}
