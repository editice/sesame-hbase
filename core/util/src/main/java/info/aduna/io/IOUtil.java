...
public class IOUtil {
	...
	/**
	 * Fills the supplied character array with characters read from the
	 * specified Reader. This method will only stop reading when the character
	 * array has been filled completely, or the end of the stream has been
	 * reached.
	 * 
	 * @param r
	...
	public static int readChars(Reader r, char[] charArray) throws IOException {
		...
		while (charsRead >= 0) {
			...
			charsRead = r.read(charArray, totalCharsRead, charArray.length
					- totalCharsRead);
		}
	 ...
	 *             If the file size exceeds the maximum array length (larger
	...
	public static byte[] readBytes(File file) throws IOException {
		...
		if (fileSize > Integer.MAX_VALUE) {
			throw new IllegalArgumentException(
					"File size exceeds maximum array length (" + fileSize
							+ " > " + Integer.MAX_VALUE + ")");
		}
	...
	public static int readBytes(InputStream in, byte[] byteArray)
		...
		while (bytesRead >= 0) {
			...
			bytesRead = in.read(byteArray, totalBytesRead, byteArray.length
					- totalBytesRead);
		}
	 ...
	 * 
	 * @param in
	 *            the stream to read from. The stream will be closed by this
	 *            method.
	 ...
	 * 
	 * @param in
	 *            The <tt>InputStream</tt> containing the data to write to the
	 *            file.
	 * @param file
	 *            The file to write the data to.
