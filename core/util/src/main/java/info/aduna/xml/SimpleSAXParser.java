...
public class SimpleSAXParser {
	 ...
	 * parsing the XML. One must set a <tt>SimpleSAXListener</tt> on this object
	 * before calling one of the <tt>parse()</tt> methods.
	 * 
	 * @throws SAXException
	...
	/**
	 * Gets the listener that currently will receive any events from this
	 * parser.
	 * 
	...
	/**
	 * Checks whether leading and trailing whitespace characters in text
	 * elements are preserved. Defaults to <tt>false</tt>.
	 */
	...
	class SimpleSAXDefaultHandler extends DefaultHandler {
		...
		public void startElement(String namespaceURI, String localName,
			...
			if (attCount == 0) {
				...
				deferredAttributes = new LinkedHashMap<String, String>(
						attCount * 2);

				for (int i = 0; i < attCount; i++) {
					deferredAttributes.put(attributes.getQName(i), attributes
							.getValue(i));
				}
			}
