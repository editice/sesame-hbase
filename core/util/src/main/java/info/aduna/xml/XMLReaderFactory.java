...
/**
 * Factory class for creating an XMLReader. This factory tries to use the system
 * property 'org.xml.sax.driver', if that fails it falls back on
 * javax.xml.sax.parsers.SAXParserFactory. If the SAXParserFactory class can not
 * be found (this can happen when using a Java 1.3 or older), or the
 * initialization using SAXParserFactory fails otherwise, the factory falls back
 * on the Xerces 2 SAX Parser.
 */
public class XMLReaderFactory {
	...
	/**
	 * creates an org.xml.sax.XMLReader object. The method first tries to create
	 * the XMLReader object specified in the 'org.xml.sax.driver' system
	 * property. If that fails, it tries to use
	 * java.xml.parsers.SAXParserFactory. If that also fails, it tries to
	 * initialize the Xerces 2 SAX parser. If that also fails, a SAXException is
	 * thrown.
	 * 
	 * @return an XMLReader
	 * @throws SAXException
	...
	public static XMLReader createXMLReader() throws SAXException {
		...
		if (xmlReaderName != null) {
			...
			logger.debug("XMLReader initialized using system property: "
					+ xmlReaderName);
		}
		...
		if (reader == null) {
			try {
				...
				String message = XERCES_SAXPARSER
						+ " is not a valid XMLReader.";
				logger.error(message);
				throw new SAXException(message);
			} catch (Exception e) {
				String message = "Could not create instance of "
						+ XERCES_SAXPARSER;
				logger.error(message);
				throw new SAXException(message);
			}
			logger
					.debug("XMLReader initialized using default Xerces SAX parser "
							+ XERCES_SAXPARSER);
		}
		return reader;
	}
	 ...
	 * @return an XMLReader
	 * @throws SAXException
	 *             when the supplied XMLReader class name can not be found or
	 *             instantiated.
	 */
