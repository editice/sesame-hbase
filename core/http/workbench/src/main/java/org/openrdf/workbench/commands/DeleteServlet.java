...
public class DeleteServlet extends TransformationServlet {
	...
	private void dropRepository(String id) throws Exception {
		 ...
		 * RepositoryConnection con = systemRepo.getConnection(); try { Resource
		 * context = findContext(id, con); manager.getRepository(id).shutDown();
		 * con.clear(context); } finally { con.close(); }
		 */
	}
