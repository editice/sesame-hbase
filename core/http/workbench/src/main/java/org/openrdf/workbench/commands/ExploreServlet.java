...
public class ExploreServlet extends TupleServlet {
	...
	protected void service(WorkbenchRequest req, TupleResultBuilder builder,
		...
		if (value instanceof Resource) {
			limit -= export(con, builder, limit, null, null, null,
					(Resource) value);
		}
	}

	private int export(RepositoryConnection con, TupleResultBuilder builder,
			int limit, Resource subj, URI pred, Value obj, Resource... ctx)
