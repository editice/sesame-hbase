...
public class HttpServerUtil {
	...
	/**
	 * Extracts the MIME type from the specified content type string. This
	 * method parses the content type string and returns just the MIME type,
	 * ignoring any parameters that are included.
	 * 
	 * @param contentType
	 *            A content type string, e.g.
	 *            <tt>application/xml; charset=utf-8</tt> .
	...
	public static String selectPreferredMIMEType(Iterator<String> mimeTypes,
		...
		List<HeaderElement> acceptElements = getHeaderElements(request,
				"Accept");
		...
		while (mimeTypes.hasNext()) {
			...
			HeaderElement acceptType = matchAcceptHeader(mimeType,
					acceptElements);
			...
			if (acceptType != null) {
				...
				if (qualityStr != null) {
					try {
						...
						// Illegal quality value, assume it has a different
						// meaning
						// and ignore it
					}
				}
	 ...
	 */
	public static List<HeaderElement> getHeaderElements(
	...
	/**
	 * Splits the supplied string into sub parts using the specified splitChar
	 * as a separator, ignoring occurrences of this character inside quoted
	 * strings.
	 * 
	 * @param s
	 ...
	 * 
	 * @param mimeTypeSpec
	 *            The MIME type to determine the quality for, e.g. "text/plain"
	...
	private static boolean matchesAcceptHeader(HeaderElement mimeTypeElem,
			HeaderElement acceptElem) {
		...
		for (int i = 0; i < mimeTypeElem.getParameterCount(); i++) {
			if (!mimeTypeElem.getParameter(i)
					.equals(acceptElem.getParameter(i))) {
				return false;
			}
		}
	...
	public static boolean isTrue(String string) {
		boolean result = false;
		if (!isEmpty(string)
				&& (string.equalsIgnoreCase("true") || string
						.equalsIgnoreCase("on"))) {
			result = true;
		}
		return result;
	...
	public static boolean isFalse(String string) {
		boolean result = false;
		if (!isEmpty(string)
				&& (string.equalsIgnoreCase("false") || string
