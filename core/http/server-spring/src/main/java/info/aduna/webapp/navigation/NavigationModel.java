...
public class NavigationModel extends Group {
	 ...
	 *            groups, where the part after the last / is interpreted as the
	...
	/**
	 * Add another NavigationModel to this one. This is done by adding all
	 * groups and view from the other model to this one.
	 * 
	 * @param other
	...
	private void createNavigationModel() {
		...
		for (String navigationModelLocation : navigationModelLocations) {
			...
			if (first) {
				parser.parseInto(this, ResourceUtil
						.getURL(navigationModelLocation));
				first = false;
			} else {
				addModel(parser.parse(ResourceUtil
						.getURL(navigationModelLocation)));
			}
		}
	}
