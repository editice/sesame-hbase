...
public class RepositoryController extends AbstractController {
	...
	protected ModelAndView handleRequestInternal(HttpServletRequest request,
		...
		if (METHOD_DELETE.equals(reqMethod)) {
			...
			logger
					.info("DELETE request invoked for repository '" + repId
							+ "'");

			if (queryStr != null) {
				logger
						.warn("query supplied on repository delete request, aborting delete");
			...
			if (SystemRepository.ID.equals(repId)) {
				...
				throw new HTTPException(HttpStatus.SC_FORBIDDEN,
						"SYSTEM Repository can not be deleted");
			}
			...
			try {
				...
				// the interceptor.
				RepositoryConnection connection = RepositoryInterceptor
				...
				if (success) {
					...
					logger
							.error("error while attempting to delete repository '"
									+ repId + "'");
					throw new HTTPException(HttpStatus.SC_BAD_REQUEST,
							"could not locate repository configuration for repository '"
									+ repId + "'.");
				}
			} catch (OpenRDFException e) {
				logger.error("error while attempting to delete repository '"
						+ repId + "'", e);
				throw new ServerHTTPException("Repository delete error: "
						+ e.getMessage(), e);
			}
		}
		...
		if (queryStr != null) {
			Query query = getQuery(repository, repositoryCon, queryStr,
					request, response);
			...
			try {
				...
				throw new ServerHTTPException(SC_SERVICE_UNAVAILABLE,
				...
				if (e.getCause() != null
					...
					throw new ServerHTTPException("Query evaluation error: "
							+ e.getMessage());
				}
			}

			Object factory = ProtocolUtil.getAcceptableService(request,
					response, registry);
			...
			throw new ClientHTTPException(SC_BAD_REQUEST, "Missing parameter: "
					+ QUERY_PARAM_NAME);
		}
	}

	private Query getQuery(Repository repository,
			RepositoryConnection repositoryCon, String queryStr,
		...
		String[] defaultGraphURIs = request
				.getParameterValues(DEFAULT_GRAPH_PARAM_NAME);
		String[] namedGraphURIs = request
				.getParameterValues(NAMED_GRAPH_PARAM_NAME);
		...
		try {
			...
			ErrorInfo errInfo = new ErrorInfo(
					ErrorType.UNSUPPORTED_QUERY_LANGUAGE, queryLn.getName());
