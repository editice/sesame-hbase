...
public class NamespaceController extends AbstractController {
	...
	protected ModelAndView handleRequestInternal(HttpServletRequest request,
		...
		if (METHOD_GET.equals(reqMethod)) {
			...
			throw new ServerHTTPException("Unexpected request method: "
					+ reqMethod);
		}
	}

	private ModelAndView getExportNamespaceResult(
			RepositoryConnection repositoryCon, String prefix)
			throws ServerHTTPException, ClientHTTPException {
		try {
			String namespace = repositoryCon.getNamespace(prefix);

			if (namespace == null) {
				throw new ClientHTTPException(SC_NOT_FOUND,
	...
	private ModelAndView getUpdateNamespaceResult(
			RepositoryConnection repositoryCon, String prefix,
	...
	private ModelAndView getRemoveNamespaceResult(
			RepositoryConnection repositoryCon, String prefix)
