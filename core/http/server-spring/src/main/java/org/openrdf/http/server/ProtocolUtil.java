...
public class ProtocolUtil {
	...
	public static Value parseValueParam(HttpServletRequest request,
			String paramName, ValueFactory vf) throws ClientHTTPException {
	...
	public static Resource parseResourceParam(HttpServletRequest request,
			String paramName, ValueFactory vf) throws ClientHTTPException {
		String paramValue = request.getParameter(paramName);
		try {
	...
	public static URI parseURIParam(HttpServletRequest request,
			String paramName, ValueFactory vf) throws ClientHTTPException {
		String paramValue = request.getParameter(paramName);
		try {
	...
	public static URI parseGraphParam(HttpServletRequest request,
			ValueFactory vf) throws ClientHTTPException {
		...
		try {
			...
			throw new ClientHTTPException(SC_BAD_REQUEST,
					"Invalid value for parameter '" + Protocol.GRAPH_PARAM_NAME
							+ "': " + paramValue);
		}
	}

	public static Resource[] parseContextParam(HttpServletRequest request,
			String paramName, ValueFactory vf) throws ClientHTTPException {
	...
	public static boolean parseBooleanParam(HttpServletRequest request,
	...
	public static <FF extends FileFormat, S> S getAcceptableService(
		...
		if (mimeType == null) {
			...
			mimeType = HttpServerUtil.selectPreferredMIMEType(mimeTypes
					.iterator(), request);
		...
		if (hasAcceptParam) {
			ErrorInfo errInfo = new ErrorInfo(
					ErrorType.UNSUPPORTED_FILE_FORMAT, mimeType);
			throw new ClientHTTPException(SC_BAD_REQUEST, errInfo.toString());
		} else {
			// No acceptable format was found, send 406 as required by RFC 2616
			throw new ClientHTTPException(SC_NOT_ACCEPTABLE,
