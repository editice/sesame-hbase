...
public abstract class ServerInterceptor extends HandlerInterceptorAdapter {
	 ...
	 *             if it was impossible to determine a name due to an internal
	 *             error
	 */
	protected abstract String getThreadName() throws ServerHTTPException;

	/**
	 * Set attributes for this request. Called before the request is forwarded
	 * to a handler. By default, this method does nothing.
	 * 
	 * @param request
	 *            the request
	 * @throws ClientHTTPException
	 *             if it was impossible to set one or more attributes due to a
	 *             bad request on the part of the client
	 * @throws ServerHTTPException
	...
	/**
	 * Clean up resources used in handling this request. Called after the
	 * request is handled and a the view is rendered (or an exception has
	 * occurred). By default, this method does nothing.
	 * 
	 * @throws ServerHTTPException
	 *             if some resources could not be cleaned up because of an
	 *             internal error
	 */
