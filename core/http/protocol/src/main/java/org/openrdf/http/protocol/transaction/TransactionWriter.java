...
public class TransactionWriter {
	...
	private void serialize(SetNamespaceOperation op, XMLWriter xmlWriter)
			throws IOException {
		xmlWriter.setAttribute(TransactionXMLConstants.PREFIX_ATT, op
				.getPrefix());
	...
	private void serialize(URI uri, XMLWriter xmlWriter) throws IOException {
		if (uri != null) {
			xmlWriter.textElement(TransactionXMLConstants.URI_TAG, uri
					.toString());
		} else {
			serializeNull(xmlWriter);
		}
	...
	private void serialize(BNode bnode, XMLWriter xmlWriter) throws IOException {
		if (bnode != null) {
			xmlWriter.textElement(TransactionXMLConstants.BNODE_TAG, bnode
					.getID());
		} else {
			serializeNull(xmlWriter);
		}
	...
	private void serialize(Literal literal, XMLWriter xmlWriter)
		...
		if (literal != null) {
			if (literal.getLanguage() != null) {
				xmlWriter.setAttribute(TransactionXMLConstants.LANG_ATT,
						literal.getLanguage());
			}
			...
			xmlWriter.textElement(TransactionXMLConstants.LITERAL_TAG, literal
					.getLabel());
		} else {
			serializeNull(xmlWriter);
		}
