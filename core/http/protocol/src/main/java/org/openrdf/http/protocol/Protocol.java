...
public abstract class Protocol {
	...
	/**
	 * Get the location of the contexts lists resource for a specific
	 * repository.
	 * 
	 * @param repositoryLocation
	 *            the location of a repository implementing this REST protocol.
	...
	/**
	 * Get the location of the namespace with the specified prefix for a
	 ...
	 *         specific repository on the specified server
	 */
	public static final String getNamespacePrefixLocation(
			String repositoryLocation, String prefix) {
	...
	public static Resource[] decodeContexts(String[] encodedValues,
		...
		if (encodedValues == null) {
			...
			for (int index = 0; index < encodedValues.length; index++) {
				result[index] = decodeContext(encodedValues[index],
						valueFactory);
			}
		}
