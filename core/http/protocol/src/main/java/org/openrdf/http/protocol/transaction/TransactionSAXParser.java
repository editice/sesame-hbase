...
class TransactionSAXParser extends SimpleSAXAdapter {
	...
	public void endTag(String tagName) throws SAXException {
		...
		} else if (TransactionXMLConstants.REMOVE_STATEMENTS_TAG
				.equals(tagName)) {
	...
	private TransactionOperation createAddStatementOperation()
		...
		if (parsedValues.size() < 3) {
			throw new SAXException(
					"At least three values required for AddStatementOperation, found: "
							+ parsedValues.size());
		}
		...
		try {
			...
			return new AddStatementOperation(subject, predicate, object,
					contexts);
		} catch (ClassCastException e) {
			throw new SAXException(
					"Invalid argument(s) for AddStatementOperation", e);
		}
	}
	...
	private TransactionOperation createRemoveStatementsOperation()
		...
		if (parsedValues.size() < 3) {
			throw new SAXException(
					"At least three values required for RemoveStatementsOperation, found: "
							+ parsedValues.size());
		}
		...
		try {
			...
			throw new SAXException(
					"Invalid argument(s) for RemoveStatementsOperation", e);
		}
	}
	...
	private Resource[] createContexts(int startIdx) throws SAXException {
		...
		for (int i = startIdx; i < parsedValues.size(); i++) {
			...
			if (contextCandidate == null
					|| contextCandidate instanceof Resource) {
				contexts.add((Resource) contextCandidate);
			} else {
				throw new SAXException("Invalid context value: "
