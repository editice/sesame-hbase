...
public class TriXParser extends RDFParserBase {
	...
	private void parse(Object inputStreamOrReader) throws IOException,
			RDFParseException, RDFHandlerException {
		try {
			rdfHandler.startRDF();
			...
			if (wrappedExc == null) {
				...
				reportFatalError(wrappedExc, e.getLineNumber(), e
						.getColumnNumber());
			}
	...
	private class TriXSAXHandler extends SimpleSAXAdapter {
		...
		public void startTag(String tagName, Map<String, String> atts,
			...
			try {
				if (tagName.equals(URI_TAG)) {
					...
					if (datatype == null) {
						reportError(DATATYPE_ATT
								+ " attribute missing for typed literal");
					...
					if (parsingContext) {
						try {
							...
							if (valueList.size() > 1) {
								...
								try {
									currentContext = (Resource) valueList
											.get(0);
		...
		private void reportStatement() throws RDFParseException,
				RDFHandlerException {
			try {
				if (valueList.size() != 3) {
