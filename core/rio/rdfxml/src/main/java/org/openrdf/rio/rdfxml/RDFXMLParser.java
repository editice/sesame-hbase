...
//		System.out.println("ff");
	...
	void startElement(String namespaceURI, String localName, String qName,
			Atts atts) throws RDFParseException, RDFHandlerException {
		if (topIsProperty()) {
		...
		if (topElement instanceof NodeElement) {
			...
			if (predicate.parseCollection()) {
				...
				if (lastListResource == null) {
					...
					reportStatement(subject.getResource(), predicate.getURI(),
							RDF.NIL);
	...
	private void processNodeElt(String namespaceURI, String localName,
			String qName, Atts atts, boolean isEmptyElt)
			throws RDFParseException, RDFHandlerException {
		if (verifyData()) {
	...
	private void processPropertyElt(String namespaceURI, String localName,
			String qName, Atts atts, boolean isEmptyElt)
			throws RDFParseException, RDFHandlerException {
		if (verifyData()) {
		...
		if (namespaceURI.equals("")) {
			// no namespace URI
			reportError("unqualified property element <" + qName
					+ "> not allowed");
		...
		if (propURI.equals(RDF.LI)) {
			...
			propURI = createURI(RDF.NAMESPACE + "_"
					+ subject.getNextLiCounter());
		}
	 ...
	 */
	private void handleReification(Value value) throws RDFParseException,
			RDFHandlerException {
		PropertyElement predicate = (PropertyElement) peekStack(0);
		...
		if (predicate.isReified()) {
			...
			reifyStatement(reifRes, subject.getResource(), predicate.getURI(),
					value);
		}
	}

	private void reifyStatement(Resource reifNode, Resource subj, URI pred,
			Value obj) throws RDFParseException, RDFHandlerException {
	...
	private boolean topIsProperty() {
		return elementStack.isEmpty()
				|| peekStack(0) instanceof PropertyElement;
	}

	/**
	 * Checks whether the node element name is from the RDF namespace and, if
	 * so, if it is allowed to be used in a node element. If the name is equal
	 * to one of the disallowed names (RDF, ID, about, parseType, resource,
	 * nodeID, datatype and li), an error is generated. If the name is not
	 * defined in the RDF namespace, but it claims that it is from this
	 * namespace, a warning is generated.
	 */
	private void checkNodeEltName(String namespaceURI, String localName,
			String qName) throws RDFParseException {
		if (RDF.NAMESPACE.equals(namespaceURI)) {

			if (localName.equals("Description") || localName.equals("Seq")
					|| localName.equals("Bag") || localName.equals("Alt")
					|| localName.equals("Statement")
					|| localName.equals("Property") || localName.equals("List")
					|| localName.equals("subject")
					|| localName.equals("predicate")
					|| localName.equals("object") || localName.equals("type")
					|| localName.equals("value") || localName.equals("first")
					|| localName.equals("rest") || localName.equals("nil")
					|| localName.startsWith("_")) {
				// These are OK
			} else if (localName.equals("li") || localName.equals("RDF")
			...
			} else if (localName.equals("bagID")
	...
	/**
	 * Checks whether the property element name is from the RDF namespace and,
	 * if so, if it is allowed to be used in a property element. If the name is
	...
	private void checkPropertyEltName(String namespaceURI, String localName,
		...
		if (RDF.NAMESPACE.equals(namespaceURI)) {
			...
			if (localName.equals("li") || localName.equals("Seq")
					|| localName.equals("Bag") || localName.equals("Alt")
					|| localName.equals("Statement")
					|| localName.equals("Property") || localName.equals("List")
					|| localName.equals("subject")
				...
				// These are OK
					...
					|| localName.equals("RDF") || localName.equals("ID")
	...
	private void checkRDFAtts(Atts atts) throws RDFParseException {
		...
		while (iter.hasNext()) {
			...
			if (RDF.NAMESPACE.equals(att.getNamespace())) {
				...
				if (localName.equals("Seq") || localName.equals("Bag")
					...
					// These are OK
						...
						|| localName.equals("li") || localName.equals("RDF")
						|| localName.equals("ID") || localName.equals("about")
						|| localName.equals("parseType")
						|| localName.equals("resource")
						|| localName.equals("nodeID")
						|| localName.equals("datatype")) {
					reportError("'" + att.getQName()
							+ "' not allowed as attribute name");
					iter.remove();
				} else if (localName.equals("bagID")
						|| localName.equals("aboutEach")
						|| localName.equals("aboutEachPrefix")) {
					reportError(att.getQName()
							+ " is no longer a valid RDF name");
				} else {
	...
	private void reportStatement(Resource subject, URI predicate, Value object)
			throws RDFParseException, RDFHandlerException {
//		System.err.println((subject==null)+"--"+(predicate==null)+"--"+(object==null));
...
//		System.out.println(st.getClass().getName());
	...
	protected void reportWarning(String msg) {
		...
		if (locator != null) {
			reportWarning(msg, locator.getLineNumber(), locator
					.getColumnNumber());
		} else {
	...
	protected void reportFatalError(String msg) throws RDFParseException {
		Locator locator = saxFilter.getLocator();
		if (locator != null) {
			reportFatalError(msg, locator.getLineNumber(), locator
					.getColumnNumber());
		} else {
	...
	protected void reportFatalError(Exception e) throws RDFParseException {
		Locator locator = saxFilter.getLocator();
		if (locator != null) {
			reportFatalError(e, locator.getLineNumber(), locator
					.getColumnNumber());
		} else {
