...
public class NTriplesParser extends RDFParserBase {
	...
	private int parseTriple(int c) throws IOException, RDFParseException,
			RDFHandlerException {
		c = parseSubject(c);
	...
	private int parseObject(int c) throws IOException, RDFParseException {
		...
		if (c == '<') {
			...
			object = createLiteral(sb.toString(), lang.toString(), datatype
					.toString());
		} else if (c == -1) {
			throwEOFException();
		} else {
	...
	private int parseUriRef(int c, StringBuilder uriRef) throws IOException,
			RDFParseException {
	...
	private int parseNodeID(int c, StringBuilder name) throws IOException,
			RDFParseException {
	...
	private int parseLiteral(int c, StringBuilder value, StringBuilder lang,
			StringBuilder datatype) throws IOException, RDFParseException {
		assert c == '"' : "Supplied char should be a '\"', is: " + c;
