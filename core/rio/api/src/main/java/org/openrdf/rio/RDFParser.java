...
public interface RDFParser {
	...
	/**
	 * Sets the ValueFactory that the parser will use to create Value objects
	 * for the parsed RDF data.
	 * 
	 * @param valueFactory
	 ...
	 * <em>normalize</em> . If set to <em>ignore</em>, no special action will be
	 * taken to handle datatyped literals. If set to <em>verify</em>, any
	 * literals with known (XML Schema built-in) datatypes are checked to see if
	 * their values are valid. If set to <em>normalize</em>, the literal values
	 * are not only checked, but also normalized to their canonical
