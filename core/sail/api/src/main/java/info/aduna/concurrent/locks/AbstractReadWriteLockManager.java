 ...
 * @author James Leigh
 */
public abstract class AbstractReadWriteLockManager implements
	 ...
	 * 
	 * @param trackLocks
	 *            Controls whether the lock manager will keep track of active
	 *            locks. Enabling lock tracking will add some overhead, but can
	...
	/**
	 * Blocks current thread until after the writer lock is released (if
	 * active).
	 * 
	 ...
	 * The lock is tracked if lock tracking is enabled. This method is not
	 * thread safe itself, the calling method is expected to handle
