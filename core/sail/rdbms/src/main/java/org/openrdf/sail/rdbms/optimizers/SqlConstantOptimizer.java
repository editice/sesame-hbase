...
public class SqlConstantOptimizer extends
	...
	public void meet(SqlAnd node) throws RuntimeException {
		...
		} else if (right instanceof SqlNot
				&& ((SqlNot) right).getArg().equals(left)) {
			replace(node, new FalseValue());
		} else if (left instanceof SqlNot
				&& ((SqlNot) left).getArg().equals(right)) {
			replace(node, new FalseValue());
		}
	}
	...
	public void meet(SqlIsNull node) throws RuntimeException {
		...
		if (arg instanceof SqlNull) {
			...
			for (Entry entry : scase.getEntries()) {
				...
				if (rep == null) {
					rep = and(condition.clone(), isNull(entry.getResult()
							.clone()));
					prev = not(condition.clone());
				} else {
					rep = or(rep, and(and(prev.clone(), condition.clone()),
							isNull(entry.getResult().clone())));
	...
	public void meet(SqlNot node) throws RuntimeException {
		...
		if (arg instanceof TrueValue) {
			...
			SqlOr or = (SqlOr) arg;
			replace(node, and(not(or.getLeftArg().clone()), not(or
					.getRightArg().clone())));
		}
	}
	...
	private SqlExpr other(SqlExpr node, SqlExpr compare,
			Class<? extends BinarySqlOperator> type) {
		if (type.isInstance(node)) {
