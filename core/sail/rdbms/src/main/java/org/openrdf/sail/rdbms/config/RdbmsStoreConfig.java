...
public class RdbmsStoreConfig extends SailImplConfigBase {
	...
	public Resource export(Graph graph) {
		...
		graph.add(implNode, MAX_TRIPLE_TABLES, vf
				.createLiteral(maxTripleTables));
	...
	public void parse(Graph graph, Resource implNode)
		...
		try {
			Literal jdbcDriverLit = getOptionalObjectLiteral(graph, implNode,
					JDBC_DRIVER);
			...
			Literal passwordLit = getOptionalObjectLiteral(graph, implNode,
					PASSWORD);
			if (passwordLit != null) {
				setPassword(passwordLit.getLabel());
			}

			Literal maxTripleTablesLit = getOptionalObjectLiteral(graph,
					implNode, MAX_TRIPLE_TABLES);
			if (maxTripleTablesLit != null) {
				try {
					setMaxTripleTables(maxTripleTablesLit.intValue());
				} catch (NumberFormatException e) {
					throw new SailConfigException(
							"Invalid value for maxTripleTables: "
									+ maxTripleTablesLit);
				}
			}
