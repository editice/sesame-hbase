...
public class RdbmsConnectionFactory {
	...
	public void init() throws SailException {
		...
		try {
			...
			if (sequenced) {
				...
				hashTable = tables.createHashTable(hashLookups, hashManager
						.getQueue());
			...
			NamespacesTable nsTable = tables
					.createNamespacesTable(nsAndTableIndexes);
			nsTable.initialize();
			...
			bnodeTable = tables.createBNodeTable(resourceInserts, bnodeManager
					.getQueue());
			uriTable = tables.createURITable(resourceInserts, uriManager
					.getQueue());
			literalManager.setHashManager(hashManager);
			literalManager.setIdSequence(ids);
			literalTable = tables.createLiteralTable(literalInserts,
					literalManager.getQueue());
	...
	public SailConnection createConnection() throws SailException {
		try {
			...
			s.setIdSequence(ids);
			DefaultSailChangedEvent sailChangedEvent = new DefaultSailChangedEvent(
					sail);
