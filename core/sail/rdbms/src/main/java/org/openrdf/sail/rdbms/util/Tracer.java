...
public class Tracer implements InvocationHandler {
	...
	public static DataSource traceDataSource(DataSource ds) {
		try {
			...
			File traceFile = File.createTempFile("sqltrace-"
					+ Long.toHexString(System.currentTimeMillis()), ".java");
	...
	private static Object trace(String var, Object delegate, Class<?> type,
			PrintWriter out) {
		ClassLoader cl = Thread.currentThread().getContextClassLoader();
		Class<?>[] types = new Class<?>[] { type };
		return Proxy
				.newProxyInstance(cl, types, new Tracer(delegate, var, out));
	}

	private static String getVariableName(Class<?> type) {
		return type.getSimpleName().replaceAll("[a-z]", "").toLowerCase()
				+ (++count);
	}
	...
	private String buildMethodCall(String returnVar, Method method,
			Object[] args) {
