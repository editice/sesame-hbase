 ...
 */
public class KVEvaluationStrategyImpl implements EvaluationStrategy {
	...
	public KVEvaluationStrategyImpl(TripleSource tripleSource) {
		this(tripleSource, null);
	}

	public KVEvaluationStrategyImpl(TripleSource tripleSource, Dataset dataset) {
...
//			 logger.trace("bindings: "+bindings.getBindingNames());
	...
	private class ValuePair {
		...
		private KVEvaluationStrategyImpl getOuterType() {
			return KVEvaluationStrategyImpl.this;
		}
...
//				System.err.println("context is 0 len");
	...
	public CloseableIteration<BindingSet, QueryEvaluationException> evaluate(
		...
		if (expr instanceof Projection) {
			 logger.error("projection-zj-p"+expr);
//			 here we must guarantee only one projection,no nested projection
...
//		 logger.trace("step 1");
		...
		//here go to entrance pattern
		result = this.entranceUnaryTupleOperator(projection.getArg(), bindings);
...
//			logger.trace("not this tag");
	...
	/**
	 * here we use to define begin to evaluate
	 * @author zj
	 * @param expr
	 * @param bindings
	 * @return
	 * @throws QueryEvaluationException
	 */
	public CloseableIteration<BindingSet, QueryEvaluationException> entranceUnaryTupleOperator(
			TupleExpr expr, BindingSet bindings)
			throws QueryEvaluationException {
		// logger.trace("-->origin evaluate()");
		if (expr instanceof StatementPattern) {
...
//			 logger.trace("bindings: "+bindings.getBindingNames());
			return evaluate((UnaryTupleOperator) expr, bindings);
			 ...
			 logger.trace("BinaryTupleOperator--"+(BinaryTupleOperator)expr);
			return entranceBinaryTupleOperator((BinaryTupleOperator) expr, bindings);
		} else if (expr instanceof SingletonSet) {
			 logger.trace("singletonset");
			return evaluate((SingletonSet) expr, bindings);
		} else if (expr instanceof EmptySet) {
	...
	private CloseableIteration<BindingSet, QueryEvaluationException> entranceBinaryTupleOperator(
			BinaryTupleOperator expr, BindingSet bindings) throws QueryEvaluationException {
		if (expr instanceof Join) {
			// logger.trace("we need do join");
			return entranceJoin((Join) expr, bindings);
		} else if (expr instanceof LeftJoin) {
			return evaluate((LeftJoin) expr, bindings);
		} else if (expr instanceof Union) {
			return evaluate((Union) expr, bindings);
	...
	private CloseableIteration<BindingSet, QueryEvaluationException> entranceJoin(
			Join join, BindingSet bindings) throws QueryEvaluationException {
		if (join.getRightArg() instanceof Service) {
//			logger.trace("this tag");
			CloseableIteration<BindingSet, QueryEvaluationException> leftIter = evaluate(
...
//			logger.trace("this tag");
			...
			//true define that it's entrance join
			return new KVJoinIterator(this, join, bindings,true);
		}
	}
