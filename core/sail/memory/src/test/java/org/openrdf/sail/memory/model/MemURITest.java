...
public class MemURITest extends TestCase {
	...
	private void compareURIs(String uri) throws Exception {
		...
		MemURI memURI = new MemURI(this, uriImpl.getNamespace(), uriImpl
				.getLocalName());

		assertEquals("MemURI not equal to URIImpl for: " + uri, uriImpl, memURI);
		assertEquals("MemURI has different hash code than URIImpl for: " + uri,
				uriImpl.hashCode(), memURI.hashCode());
	}
}
