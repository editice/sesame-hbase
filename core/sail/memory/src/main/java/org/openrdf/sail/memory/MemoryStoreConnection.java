 ...
 * @author jeen
 */
public class MemoryStoreConnection extends NotifyingSailConnectionBase
	...
	protected CloseableIteration<? extends BindingSet, QueryEvaluationException> evaluateInternal(
		...
		try {
			...
			new QueryJoinOptimizer(new MemEvaluationStatistics()).optimize(
			...
			iter = new LockingIteration<BindingSet, QueryEvaluationException>(
					stLock, iter);
			releaseLock = false;
			return iter;
	...
	protected CloseableIteration<? extends Resource, SailException> getContextIDsInternal()
		...
		try {
			final int snapshot = transactionActive() ? store
					.getCurrentSnapshot() + 1 : store.getCurrentSnapshot();
			final ReadMode readMode = transactionActive() ? ReadMode.TRANSACTION
					: ReadMode.COMMITTED;
		...
		return new CloseableIteratorIteration<MemResource, SailException>(
				contextIDs.iterator());
	}

	private boolean isContextResource(MemResource memResource, int snapshot,
			ReadMode readMode) throws SailException {
		MemStatementList contextStatements = memResource
	...
	protected CloseableIteration<? extends Statement, SailException> getStatementsInternal(
		...
		try {
			...
			iter = store.createStatementIterator(SailException.class, subj,
					pred, obj, !includeInferred, snapshot, readMode, contexts);
			iter = new LockingIteration<MemStatement, SailException>(stLock,
					iter);
			releaseLock = false;
			return iter;
		} finally {
	...
	public boolean hasStatement(Resource subj, URI pred, Value obj,
		...
		try {
			...
			return store.hasStatement(subj, pred, obj, !includeInferred,
	...
	protected long sizeInternal(Resource... contexts) throws SailException {
		...
		try {
			CloseableIteration<? extends Statement, SailException> iter = getStatementsInternal(
	...
	protected CloseableIteration<? extends Namespace, SailException> getNamespacesInternal()
			throws SailException {
		return new CloseableIteratorIteration<Namespace, SailException>(store
				.getNamespaceStore().iterator());
	}
	...
	protected void startTransactionInternal() throws SailException {
		...
		try {
			// Prevent concurrent transactions by acquiring an exclusive txn
			// lock
	 ...
	 * @throws SailException
	 */
	protected boolean addStatementInternal(Resource subj, URI pred, Value obj,
	...
	public boolean removeInferredStatement(Resource subj, URI pred, Value obj,
		...
		try {
			...
			try {
				autoStartTransaction();
				return removeStatementsInternal(subj, pred, obj, false,
						contexts);
			} finally {
				updateLock.unlock();
			}
	 ...
	 * @param pred
	 *            The predicate for the pattern, or <tt>null</tt> for a
	 *            wildcard.
	 * @param obj
	...
	protected boolean removeStatementsInternal(Resource subj, URI pred,
		...
		CloseableIteration<MemStatement, SailException> stIter = store
				.createStatementIterator(SailException.class, subj, pred, obj,
						explicit, store.getCurrentSnapshot() + 1,
						ReadMode.TRANSACTION, contexts);

		return removeIteratorStatements(stIter, explicit);
	}

	protected boolean removeIteratorStatements(
			CloseableIteration<MemStatement, SailException> stIter,
			boolean explicit) throws SailException {
	...
	protected class MemTripleSource implements TripleSource {
		...
		public MemTripleSource(boolean includeInferred, int snapshot,
				ReadMode readMode) {
		...
		public CloseableIteration<MemStatement, QueryEvaluationException> getStatements(
				Resource subj, URI pred, Value obj, Resource... contexts) {
			return store.createStatementIterator(
					QueryEvaluationException.class, subj, pred, obj,
	...
	/**
	 * Uses the MemoryStore's statement sizes to give cost estimates based on
	 * the size of the expected results. This process could be improved with
		...
		protected class MemCardinalityCalculator extends CardinalityCalculator {
			...
			public double getCardinality(StatementPattern sp) {
				...
				if (!(pred instanceof URI)) {
					// can happen when a previous optimizer has inlined a
					// comparison operator. See SES-970 / SES-998
					pred = null;
				}
				...
				// Perform look-ups for value-equivalents of the specified
				// values
				MemResource memSubj = valueFactory
						.getMemResource((Resource) subj);
				MemURI memPred = valueFactory.getMemURI((URI) pred);
				MemValue memObj = valueFactory.getMemValue(obj);
				MemResource memContext = valueFactory
						.getMemResource((Resource) context);

				if (subj != null && memSubj == null || pred != null
						&& memPred == null || obj != null && memObj == null
				...
				if (listSizes.isEmpty()) {
					...
					//
					// // Subtract 1 from var count as this was used for the
					// list
					// size
