...
public class MemoryStoreConfig extends SailImplConfigBase {
	...
	public Resource export(Graph graph) {
		...
		if (persist) {
			graph.add(implNode, PERSIST, graph.getValueFactory().createLiteral(
					persist));
		}

		if (syncDelay != 0) {
			graph.add(implNode, SYNC_DELAY, graph.getValueFactory()
					.createLiteral(syncDelay));
		}
			...
			if (persistValue != null) {
				try {
					...
					throw new SailConfigException("Boolean value required for "
							+ PERSIST + " property, found " + persistValue);
				}
			}

			Literal syncDelayValue = GraphUtil.getOptionalObjectLiteral(graph,
					implNode, SYNC_DELAY);
			if (syncDelayValue != null) {
				try {
					...
					throw new SailConfigException(
							"Long integer value required for " + SYNC_DELAY
