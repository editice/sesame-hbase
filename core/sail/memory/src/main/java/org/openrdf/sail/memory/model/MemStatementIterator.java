 ...
 * MemoryStore.
 */
public class MemStatementIterator<X extends Exception> extends
		LookAheadIteration<MemStatement, X> {
	...
	protected MemStatement getNextElement() {
		...
		for (; statementIdx < statementList.size(); statementIdx++) {
			...
			if (st.isInSnapshot(snapshot)
					&& (subject == null || subject == st.getSubject())
				...
				// skipped due to explicitOnly, contexts and readMode
				// requirements

				if (contexts != null && contexts.length > 0) {
					boolean matchingContext = false;
					for (int i = 0; i < contexts.length && !matchingContext; i++) {
						matchingContext = ObjectUtil.nullEquals(
								st.getContext(), contexts[i]);
					}
					if (!matchingContext) {
						// statement does not appear in one of the specified
						// contexts,
						// skip it.
						continue;
					}
