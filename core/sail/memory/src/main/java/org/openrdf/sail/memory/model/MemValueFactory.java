...
public class MemValueFactory extends ValueFactoryBase {
	...
	/**
	 * Returns a previously created MemValue that is equal to the supplied
	 * value, or <tt>null</tt> if the supplied value is a new value or is equal
	 * to <tt>null</tt>.
	 * 
	 * @param value
	 *            The MemValue equivalent of the supplied value, or
	 *            <tt>null</tt>.
	...
	public MemValue getMemValue(Value value) {
		if (value instanceof Resource) {
			...
			throw new IllegalArgumentException(
					"value is not a Resource or Literal: " + value);
		}
	}
	...
	public MemResource getMemResource(Resource resource) {
		if (resource instanceof URI) {
			...
			throw new IllegalArgumentException(
					"resource is not a URI or BNode: " + resource);
		}
	}
	...
	private boolean isOwnMemValue(Value value) {
		return value instanceof MemValue
				&& ((MemValue) value).getCreator() == this;
	}
	...
	public synchronized MemLiteral getOrCreateMemLiteral(Literal literal) {
		...
		if (memLiteral == null) {
			...
			if (datatype != null) {
				try {
					if (XMLDatatypeUtil.isIntegerDatatype(datatype)) {
						memLiteral = new IntegerMemLiteral(this, label, literal
								.integerValue(), datatype);
					} else if (datatype.equals(XMLSchema.DECIMAL)) {
						memLiteral = new DecimalMemLiteral(this, label, literal
								.decimalValue(), datatype);
					} else if (datatype.equals(XMLSchema.FLOAT)) {
						memLiteral = new NumericMemLiteral(this, label, literal
								.floatValue(), datatype);
					} else if (datatype.equals(XMLSchema.DOUBLE)) {
						memLiteral = new NumericMemLiteral(this, label, literal
								.doubleValue(), datatype);
					} else if (datatype.equals(XMLSchema.BOOLEAN)) {
						memLiteral = new BooleanMemLiteral(this, label, literal
								.booleanValue());
					} else if (datatype.equals(XMLSchema.DATETIME)) {
						memLiteral = new CalendarMemLiteral(this, label,
								datatype, literal.calendarValue());
					} else {
			...
			assert wasNew : "Created a duplicate MemLiteral for literal "
					+ literal;
		}
	...
	public synchronized URI createURI(String namespace, String localName) {
		...
		if (URIUtil.isCorrectURISplit(namespace, localName)) {
			if (namespace.indexOf(':') == -1) {
				throw new IllegalArgumentException(
						"Not a valid (absolute) URI: " + namespace + localName);
			}
	...
	protected synchronized Literal createIntegerLiteral(Number n, URI datatype) {
		MemLiteral newLiteral = new IntegerMemLiteral(this, BigInteger
				.valueOf(n.longValue()), datatype);
		return getSharedLiteral(newLiteral);
	}
