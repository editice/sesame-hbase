 ...
 */
class ForwardChainingRDFSInferencerConnection extends
		InferencerConnectionWrapper implements SailConnectionListener {
	...
	public void flushUpdates() throws SailException {
		...
		if (statementsRemoved) {
			logger
					.debug("statements removed, starting inferencing from scratch");
			clearInferred();
			addAxiomStatements();

			newStatements = new GraphImpl();
			Iterations.addAll(getWrappedConnection().getStatements(null, null,
					null, true), newStatements);
	...
	protected void addAxiomStatements() throws SailException {
		...
		addInferredStatement(RDFS.CONTAINERMEMBERSHIPPROPERTY, RDFS.SUBCLASSOF,
				RDF.PROPERTY);
	...
	protected void doInferencing() throws SailException {
		...
		while (hasNewStatements()) {
			...
			logger.debug("iteration " + iteration + " done; inferred "
					+ nofInferred + " new statements");
		...
		for (int i = 0; i < RDFSRules.RULECOUNT; i++) {
			logger.debug("rule " + RDFSRules.RULENAMES[i] + ":\t#inferred="
					+ ruleCount[i]);
		}
	...
	private int applyRuleRdf1() throws SailException {
		...
		while (iter.hasNext()) {
			...
			boolean added = addInferredStatement(st.getPredicate(), RDF.TYPE,
					RDF.PROPERTY);
	...
	private int applyRuleRdfs2_1() throws SailException {
		...
		while (ntIter.hasNext()) {
			...
			t1Iter = getWrappedConnection().getStatements(aaa, RDFS.DOMAIN,
					null, true);
	...
	private int applyRuleRdfs2_2() throws SailException {
		...
		Iterator<Statement> ntIter = newThisIteration.match(null, RDFS.DOMAIN,
				null);
		...
		while (ntIter.hasNext()) {
			...
			if (aaa instanceof URI && zzz instanceof Resource) {
				CloseableIteration<? extends Statement, SailException> t1Iter;
				t1Iter = getWrappedConnection().getStatements(null, (URI) aaa,
						null, true);
	...
	private int applyRuleRdfs3_1() throws SailException {
		...
		while (ntIter.hasNext()) {
			...
			if (uuu instanceof Resource) {
				CloseableIteration<? extends Statement, SailException> t1Iter;
				t1Iter = getWrappedConnection().getStatements(aaa, RDFS.RANGE,
						null, true);
				...
				while (t1Iter.hasNext()) {
					...
					Value zzz = t1.getObject();
					if (zzz instanceof Resource) {
						boolean added = addInferredStatement((Resource) uuu,
								RDF.TYPE, zzz);
						if (added) {
							nofInferred++;
						}
	...
	private int applyRuleRdfs3_2() throws SailException {
		...
		Iterator<Statement> ntIter = newThisIteration.match(null, RDFS.RANGE,
				null);
	...
	private int applyRuleRdfs4a() throws SailException {
		...
		while (iter.hasNext()) {
			...
			boolean added = addInferredStatement(st.getSubject(), RDF.TYPE,
					RDFS.RESOURCE);
			if (added) {
				nofInferred++;
			}
	...
	private int applyRuleRdfs4b() throws SailException {
		...
		while (iter.hasNext()) {
			...
			if (uuu instanceof Resource) {
				boolean added = addInferredStatement((Resource) uuu, RDF.TYPE,
						RDFS.RESOURCE);
				if (added) {
					nofInferred++;
				}
	...
	private int applyRuleRdfs5_1() throws SailException {
		...
		Iterator<Statement> ntIter = newThisIteration.match(null,
		...
		while (ntIter.hasNext()) {
			...
			if (bbb instanceof Resource) {
				CloseableIteration<? extends Statement, SailException> t1Iter;
				t1Iter = getWrappedConnection().getStatements((Resource) bbb,
						RDFS.SUBPROPERTYOF, null, true);
				...
				while (t1Iter.hasNext()) {
					...
					if (ccc instanceof Resource) {
						boolean added = addInferredStatement(aaa,
								RDFS.SUBPROPERTYOF, ccc);
						if (added) {
							nofInferred++;
						}
					}
	...
	private int applyRuleRdfs5_2() throws SailException {
		...
		while (ntIter.hasNext()) {
			...
			if (ccc instanceof Resource) {
				CloseableIteration<? extends Statement, SailException> t1Iter;
				t1Iter = getWrappedConnection().getStatements(null,
						RDFS.SUBPROPERTYOF, bbb, true);
	...
	private int applyRuleRdfs6() throws SailException {
		...
		Iterator<Statement> iter = newThisIteration.match(null, RDF.TYPE,
				RDF.PROPERTY);
	...
	private int applyRuleRdfs7_1() throws SailException {
		...
		while (ntIter.hasNext()) {
			...
			CloseableIteration<? extends Statement, SailException> t1Iter;
			t1Iter = getWrappedConnection().getStatements(aaa,
					RDFS.SUBPROPERTYOF, null, true);
	...
	// xxx rdfs:subClassOf yyy (nt) && aaa rdf:type xxx (t1) --> aaa rdf:type
	// yyy
	private int applyRuleRdfs9_1() throws SailException {
		...
		while (ntIter.hasNext()) {
			...
			if (yyy instanceof Resource) {
				CloseableIteration<? extends Statement, SailException> t1Iter;
				t1Iter = getWrappedConnection().getStatements(null, RDF.TYPE,
						xxx, true);
	...
	// aaa rdf:type xxx (nt) && xxx rdfs:subClassOf yyy (t1) --> aaa rdf:type
	// yyy
	private int applyRuleRdfs9_2() throws SailException {
		int nofInferred = 0;

		Iterator<Statement> ntIter = newThisIteration.match(null, RDF.TYPE,
				null);
		...
		while (ntIter.hasNext()) {
			...
			if (xxx instanceof Resource) {
				CloseableIteration<? extends Statement, SailException> t1Iter;
				t1Iter = getWrappedConnection().getStatements((Resource) xxx,
	...
	private int applyRuleRdfs11_1() throws SailException {
		...
		while (ntIter.hasNext()) {
			...
			if (yyy instanceof Resource) {
				CloseableIteration<? extends Statement, SailException> t1Iter;
				t1Iter = getWrappedConnection().getStatements((Resource) yyy,
						RDFS.SUBCLASSOF, null, true);
				...
				while (t1Iter.hasNext()) {
					...
					if (zzz instanceof Resource) {
						boolean added = addInferredStatement(xxx,
								RDFS.SUBCLASSOF, zzz);
						if (added) {
							nofInferred++;
						}
	...
	private int applyRuleRdfs12() throws SailException {
		...
		while (iter.hasNext()) {
			...
			boolean added = addInferredStatement(xxx, RDFS.SUBPROPERTYOF,
					RDFS.MEMBER);
			if (added) {
				nofInferred++;
			}
	...
	private int applyRuleX1() throws SailException {
		...
		while (iter.hasNext()) {
			...
			if (predURI.startsWith(prefix)
					&& isValidPredicateNumber(predURI
							.substring(prefix.length()))) {
				boolean added = addInferredStatement(predNode, RDF.TYPE,
						RDFS.CONTAINERMEMBERSHIPPROPERTY);
				if (added) {
					nofInferred++;
				}
