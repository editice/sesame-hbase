...
public class HashFile {
			...
			if (nioFile.size() == 0L) {
				...
				if (buf.remaining() < HEADER_LENGTH) {
					throw new IOException(
							"File too short to be a compatible hash file");
				}
				...
				if (!Arrays.equals(MAGIC_NUMBER, magicNumber)) {
					throw new IOException(
							"File doesn't contain compatible hash file data");
				}

				if (version > FILE_FORMAT_VERSION) {
					throw new IOException(
							"Unable to read hash file; it uses a newer file format");
				} else if (version != FILE_FORMAT_VERSION) {
					throw new IOException(
							"Unable to read hash file; invalid file format version: "
									+ version);
				}
	...
	private long getOverflowBucketOffset(int bucketID) {
		return HEADER_LENGTH + ((long) bucketCount + (long) bucketID - 1L)
				* recordSize;
	}
	...
	private void increaseHashTable() throws IOException {
		...
		File tmpFile = new File(getFile().getParentFile(), "rehash_"
				+ getFile().getName());
		...
		nioFile
				.transferTo(oldTableSize, oldFileSize - oldTableSize,
						tmpChannel);
