...
public class NativeStore extends NotifyingSailBase {
	...
	protected void initializeInternal() throws SailException {
		...
		if (dataDir == null) {
			...
			throw new SailException(
					"The specified path does not denote a directory: "
							+ dataDir);
		} else if (!dataDir.canRead()) {
			throw new SailException(
					"Not allowed to read from the specified directory: "
							+ dataDir);
		}
		...
		try {
			...
			valueStore = new ValueStore(dataDir, forceSync, valueCacheSize,
			...
			if (valueStore != null) {
				try {
					...
					logger
							.warn(
									"Failed to close value store after native store initialization failure",
									e);
				}
				valueStore = null;
			}
	...
	protected CloseableIteration<Resource, IOException> getContextIDs(
...
//				System.err.println(subj.getClass()+subj.stringValue());
				return new EmptyIteration<Statement, IOException>();
			}
		}
//		else{
//			System.err.println(subj.getClass());
//		}
...
//				System.err.println(pred.getClass()+pred.stringValue());
				return new EmptyIteration<Statement, IOException>();
			}
		}
...
//				System.err.println(obj.getClass()+obj.stringValue());
				return new EmptyIteration<Statement, IOException>();
			}
		}
		...
		for (int contextID : contextIDList) {
			...
			if (includeInferred) {
				...
				btreeIter = tripleStore.getTriples(subjID, predID, objID,
						contextID, readTransaction);
			} else {
				// Only get explicit statements
				btreeIter = tripleStore.getTriples(subjID, predID, objID,
						contextID, true, readTransaction);
			}

			perContextIterList.add(new NativeStatementIterator(btreeIter,
					valueStore));
		}
	...
	protected double cardinality(Resource subj, URI pred, Value obj,
