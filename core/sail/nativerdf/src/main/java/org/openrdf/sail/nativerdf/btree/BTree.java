...
public class BTree {
	...
	/**
	 * Object used to determine whether one value is lower, equal or greater
	 * than another value. This determines the order of values in the BTree.
	 */
	 ...
	 */
	private final Map<Integer, Node> nodeCache = new HashMap<Integer, Node>(
			NODE_CACHE_SIZE);
	 ...
	 * class)
	 */
	private final Map<Integer, Node> mruNodes = new LinkedHashMap<Integer, Node>(
			NODE_CACHE_SIZE);
	 ...
	 * performance, the specified block size should be equal to the file
	 * system's block size.
	 */
	...
	/**
	 * The size of a node in bytes. Value derived from branchFactor and
	 * slotSize.
	 */
	 ...
	 * unknown, 0 for an empty BTree, 1 for a BTree with just a root node, and
	 * so on.
	 */
	 ...
	 * @param blockSize
	 *            The size (in bytes) of a file block for a single node.
	 *            Ideally, the size specified is the size of a block in the used
	 *            file system.
	 * @param valueSize
	 *            The size (in bytes) of the fixed-length values that are or
	 *            will be stored in the B-Tree.
	 * @throws IOException
	 *             In case the initialization of the B-Tree file failed.
	 * @see DefaultRecordComparator
	 */
	public BTree(File dataDir, String filenamePrefix, int blockSize,
	 ...
	 *            will be stored in the B-Tree.
	 * @param forceSync
	 *            Flag indicating whether updates should be synced to disk
	 *            forcefully by calling {@link FileChannel#force(boolean)}. This
	 *            may have a severe impact on write performance.
	 * @throws IOException
	 *             In case the initialization of the B-Tree file failed.
	 * @see DefaultRecordComparator
	 */
	public BTree(File dataDir, String filenamePrefix, int blockSize,
			int valueSize, boolean forceSync) throws IOException {
		this(dataDir, filenamePrefix, blockSize, valueSize,
				new DefaultRecordComparator(), forceSync);
	}
	 ...
	 */
	public BTree(File dataDir, String filenamePrefix, int blockSize,
			int valueSize, RecordComparator comparator) throws IOException {
	 ...
	 */
	public BTree(File dataDir, String filenamePrefix, int blockSize,
			int valueSize, RecordComparator comparator, boolean forceSync)
			throws IOException {
		if (dataDir == null) {
		...
		if (filenamePrefix == null) {
			throw new IllegalArgumentException(
					"filenamePrefix must not be null");
		}
		if (blockSize < HEADER_LENGTH) {
			throw new IllegalArgumentException("block size must be at least "
					+ HEADER_LENGTH + " bytes");
		}
		if (valueSize <= 0) {
			throw new IllegalArgumentException(
					"value size must be larger than 0");
		}
		...
		if (nioFile.size() == 0L) {
			...
			if (Arrays.equals(MAGIC_NUMBER, magicNumber)) {
				...
				logger.info("Updating file header for btree file '{}'", file
						.getAbsolutePath());
				writeFileHeader();
			} else {
				throw new IOException(
						"File doesn't contain (compatible) BTree data: " + file);
			}
	 ...
	 * Any pending changes will be synchronized to disk before closing. Once the
	...
	/**
	 * Closes any opened files and release any resources used by this B-Tree.
	 * Any pending changes are optionally synchronized to disk before closing.
	 * Once the B-Tree has been closed, it can no longer be used.
	 * 
	 * @param syncChanges
	 *            Flag indicating whether pending changes should be synchronized
	 *            to disk.
	 */
	 ...
	 * 
	 * @param key
	 *            A value that is equal to the value that should be retrieved,
	 *            at least as far as the RecordComparator of this BTree is
	 *            concerned.
		...
		try {
			...
			while (true) {
				...
				if (valueIdx >= 0) {
					...
					// Returned index references the first value that is larger
					// than
					// the key, search the child node just left of it (==same
					// index).
	...
	public RecordIterator iterateRangedValues(byte[] searchKey,
			byte[] searchMask, byte[] minValue, byte[] maxValue) {
	...
	private List<PathSegment> getPath(byte[] key) throws IOException {
		...
		if (currentNode != null) {
			while (true) {
				...
				path
						.add(new PathSegment(keyIndex, currentNode
								.getValueCount()));
	...
	private long getValueCountEstimate(List<PathSegment> minValuePath,
			List<PathSegment> maxValuePath) throws IOException {
		...
		// range
		long valueCount = (maxValueIndex - minValueIndex)
				* getTreeSizeEstimate(splitIdx + 2);
	...
	public byte[] insert(byte[] value) throws IOException {
		...
		try {
			...
			if (insertResult.overflowValue != null) {
				// Root node overflowed, create a new root node and insert
				// overflow
				// value-nodeID pair in it
				Node newRootNode = createNewNode();
				newRootNode.setChildNodeID(0, rootNode.getID());
				newRootNode
						.insertValueNodeIDPair(0, insertResult.overflowValue,
								insertResult.overflowNodeID);
		...
		if (valueIdx >= 0) {
			...
			if (node.isLeaf()) {
				...
				if (insertResult.overflowValue != null) {
					...
					insertResult = insertInNode(insertResult.overflowValue,
							insertResult.overflowNodeID, valueIdx, node);
	...
	private InsertResult insertInNode(byte[] value, int nodeID, int valueIdx,
		...
		if (node.isFull()) {
			...
			insertResult.overflowValue = node.splitAndInsert(value, nodeID,
					valueIdx, newNode);
	...
	private byte[] removeFromTree(byte[] key, Node node) throws IOException {
		...
		if (valueIdx >= 0) {
			...
			if (node.isLeaf()) {
				...
				// Replace the matching value with the largest value from the
				// left
				// child node
	...
	private byte[] removeLargestValueFromTree(Node node) throws IOException {
		...
		if (node.isLeaf()) {
			if (node.isEmpty()) {
				throw new IllegalArgumentException(
						"Trying to remove largest value from an empty node in "
								+ getFile());
			}
	...
	private void balanceChildNode(Node parentNode, Node childNode, int childIdx)
		...
		if (childNode.getValueCount() < minValueCount) {
			// Child node contains too few values, try to borrow one from its
			// right
			// sibling
			Node rightSibling = (childIdx < parentNode.getValueCount()) ? parentNode
					.getChildNode(childIdx + 1)
					: null;

			if (rightSibling != null
					&& rightSibling.getValueCount() > minValueCount) {
				// Right sibling has enough values to give one up
				parentNode.rotateLeft(childIdx, childNode, rightSibling);
			} else {
				// Right sibling does not have enough values to give one up, try
				// its
				// left sibling
				Node leftSibling = (childIdx > 0) ? parentNode
						.getChildNode(childIdx - 1) : null;

				if (leftSibling != null
						&& leftSibling.getValueCount() > minValueCount) {
					...
					if (leftSibling != null) {
						leftSibling.mergeWithRightSibling(parentNode
								.removeValueRight(childIdx - 1), childNode);
					} else {
						childNode.mergeWithRightSibling(parentNode
								.removeValueRight(childIdx), rightSibling);
					}
				}
	...
	private Node createNewNode() throws IOException {
		...
		synchronized (nodeCache) {
			if (nodeCache.size() >= NODE_CACHE_SIZE
					&& mruNodes.size() > MIN_MRU_CACHE_SIZE) {
	...
	private Node readNode(int id) throws IOException {
		if (id <= 0) {
			throw new IllegalArgumentException("id must be larger than 0, is: "
					+ id + " in " + getFile());
		}
	...
	private void releaseNode(Node node) throws IOException {
		...
		// synchronizes on nodeCache. This method should not be called directly
		// to
	...
	class Node {
		...
		public Node(int id) {
			if (id <= 0) {
				throw new IllegalArgumentException(
						"id must be larger than 0, is: " + id + " in "
								+ getFile());
			}
		...
		public void release() throws IOException {
			...
			synchronized (nodeCache) {
				assert usageCount > 0 : "Releasing node while usage count is "
						+ usageCount;
		...
		public byte[] getValue(int valueIdx) {
			...
			assert valueIdx < valueCount : "valueIdx out of range (" + valueIdx
					+ " >= " + valueCount + ")";

			return ByteArrayUtil
					.get(data, valueIdx2offset(valueIdx), valueSize);
		}
		...
		public int getChildNodeID(int nodeIdx) {
			...
			assert nodeIdx <= valueCount : "nodeIdx out of range (" + nodeIdx
					+ " > " + valueCount + ")";
		...
		public void setChildNodeID(int nodeIdx, int nodeID) {
			assert nodeIdx >= 0 : "nodeIdx must not be negative, is: "
					+ nodeIdx;
			assert nodeIdx <= valueCount : "nodeIdx out of range (" + nodeIdx
					+ " > " + valueCount + ")";
		...
		public int search(byte[] key) {
			...
			while (low <= high) {
				int mid = (low + high) >> 1;
				int diff = comparator.compareBTreeValues(key, data,
						valueIdx2offset(mid), valueSize);
		...
		public void insertValueNodeIDPair(int valueIdx, byte[] value, int nodeID) {
			assert valueIdx >= 0 : "valueIdx must be positive, is: " + valueIdx;
			assert valueIdx <= valueCount : "valueIdx out of range ("
					+ valueIdx + " > " + valueCount + ")";
			assert value != null : "value must not be null";
			assert nodeID >= 0 : "nodeID must not be negative, is: " + nodeID;
		 ...
		 * @throws IOException
		 */
		public byte[] splitAndInsert(byte[] newValue, int newNodeID,
				int newValueIdx, Node newNode) throws IOException {
			// First store the new value-node pair in data, then split it. This
			// can be done because data got one spare slot when it was
			// allocated.
			insertValueNodeIDPair(newValueIdx, newValue, newNodeID);

			assert valueCount == branchFactor : "Node contains " + valueCount
					+ " values, expected " + branchFactor;
			...
			System.arraycopy(data, splitOffset, newNode.data, 4, data.length
					- splitOffset);
			...
			// Return the median value; it should be inserted into the parent
			// node
			return medianValue;
		}
		...
		public void mergeWithRightSibling(byte[] medianValue, Node rightSibling)
			...
			System.arraycopy(rightSibling.data, 4, data,
					nodeIdx2offset(rightIdx),
		...
		public void rotateLeft(int valueIdx, Node leftChildNode,
				Node rightChildNode) throws IOException {
			leftChildNode.insertValueNodeIDPair(leftChildNode.getValueCount(),
					this.getValue(valueIdx), rightChildNode.getChildNodeID(0));
			setValue(valueIdx, rightChildNode.removeValueLeft(0));
			notifyRotatedLeft(valueIdx, leftChildNode, rightChildNode);
		}

		public void rotateRight(int valueIdx, Node leftChildNode,
				Node rightChildNode) throws IOException {
			rightChildNode.insertNodeIDValuePair(0, leftChildNode
					.getChildNodeID(leftChildNode.getValueCount()), this
					.getValue(valueIdx - 1));
			setValue(valueIdx - 1, leftChildNode.removeValueRight(leftChildNode
					.getValueCount() - 1));
		...
		private void notifyRotatedLeft(int index, Node leftChildNode,
				Node rightChildNode) throws IOException {
			synchronized (listeners) {
				Iterator<NodeListener> iter = listeners.iterator();
				...
				while (iter.hasNext()) {
					// Deregister if listener return true
					if (iter.next().rotatedLeft(this, index, leftChildNode,
							rightChildNode)) {
						iter.remove();
					}
				}
		...
		private void notifyRotatedRight(int index, Node leftChildNode,
				Node rightChildNode) throws IOException {
			synchronized (listeners) {
				Iterator<NodeListener> iter = listeners.iterator();
				...
				while (iter.hasNext()) {
					// Deregister if listener return true
					if (iter.next().rotatedRight(this, index, leftChildNode,
							rightChildNode)) {
						iter.remove();
					}
				}
		...
		private void notifyNodeSplit(Node rightNode, int medianIdx)
			...
			synchronized (listeners) {
				...
				while (iter.hasNext()) {
					boolean deregister = iter.next().nodeSplit(this, rightNode,
							medianIdx);
		...
		private void notifyNodeMerged(Node targetNode, int mergeIdx)
			...
			synchronized (listeners) {
				...
				while (iter.hasNext()) {
					boolean deregister = iter.next().nodeMergedWith(this,
							targetNode, mergeIdx);
		...
		public void read() throws IOException {
			...
			assert bytesRead == nodeSize : "Read operation didn't read the entire node ("
					+ bytesRead + " of " + nodeSize + " bytes)";
		...
		public void write() throws IOException {
			...
			assert bytesWritten == nodeSize : "Write operation didn't write the entire node ("
					+ bytesWritten + " of " + nodeSize + " bytes)";
		...
		private void shiftData(int startOffset, int endOffset, int shift) {
			System.arraycopy(data, startOffset, data, startOffset + shift,
	...
	private interface NodeListener {
		 ...
		 * @return Indicates whether the node listener should be deregistered as
		...
		public boolean rotatedLeft(Node node, int index, Node leftChildNode,
				Node rightChildNode) throws IOException;

		public boolean rotatedRight(Node node, int index, Node leftChildNode,
				Node rightChildNode) throws IOException;
		 ...
		 * @param newNode
		 *            The newly allocated node containing the "right" half of
		 *            the values.
		 * @param medianIdx
		 *            The index where the node has been split. The value at this
		 *            index has been moved to the node's parent.
		 * @return Indicates whether the node listener should be deregistered as
		 *         a result of this event.
		 */
		 ...
		 * All values from the source node have been appended to the value of
		 * the target node.
		 * 
		 * @param sourceNode
		 ...
		 *            <tt>targetNode</tt> .
		 * @return Indicates whether the node listener should be deregistered
		 *         with the <em>source node</em> as a result of this event.
		 */
		public boolean nodeMergedWith(Node sourceNode, Node targetNode,
				int mergeIdx) throws IOException;
	}
	...
	private class RangeIterator implements RecordIterator, NodeListener {
		...
		public RangeIterator(byte[] searchKey, byte[] searchMask,
		...
		public byte[] next() throws IOException {
			...
			try {
				...
				while (value != null) {
					if (maxValue != null
							&& comparator.compareBTreeValues(maxValue, value,
									0, value.length) < 0) {
						...
						break;
							...
							&& !ByteArrayUtil.matchesPattern(value, searchMask,
									searchKey)) {
		...
		private void findMinimum() throws IOException {
			...
			while (true) {
				...
				if (currentNode.isLeaf()) {
					...
					// [SES-725] must change stacks after node loading has
					// succeeded
		...
		public void set(byte[] value) {
			...
			try {
				if (currentNode == null
						|| currentIdx > currentNode.getValueCount()) {
					throw new IllegalStateException();
				}
		...
		public boolean rotatedLeft(Node node, int valueIndex,
				Node leftChildNode, Node rightChildNode) throws IOException {
			if (currentNode == node) {
				...
				if (currentIdx == 0) {
					// the value that would be visited next has been moved to
					// the
					// parent node
					popStacks();
					currentIdx = valueIndex;
				...
				for (int i = 0; i < parentNodeStack.size(); i++) {
					...
					if (stackNode == rightChildNode) {
						...
						if (stackIdx == 0) {
							// this node is no longer the parent, replace with
							// left
							// sibling
							...
							parentIndexStack.set(i, leftChildNode
									.getValueCount());
						}
		...
		public boolean rotatedRight(Node node, int valueIndex,
				Node leftChildNode, Node rightChildNode) throws IOException {
			for (int i = 0; i < parentNodeStack.size(); i++) {
				Node stackNode = parentNodeStack.get(i);
			...
			if (sourceNode == currentNode) {
				...
				for (int i = 0; i < parentNodeStack.size(); i++) {
					...
					if (sourceNode == parentNode) {
						...
						parentIndexStack.set(i, mergeIdx
	...
	public static void runPerformanceTest(String[] args) throws Exception {
		...
		// }
		// System.out.println("Removed all item in " +
	...
	public static void runDebugTest(String[] args) throws Exception {
		 ...
		 * System.out.println("Removing H..."); btree.remove("H".getBytes());
		 * btree.commitTransaction(); btree.print(System.out);
		 * System.out.println("Removing T..."); btree.remove("T".getBytes());
		 * btree.commitTransaction(); btree.print(System.out);
		 * System.out.println("Removing R..."); btree.remove("R".getBytes());
		 * btree.commitTransaction(); btree.print(System.out);
		 * System.out.println("Removing E..."); btree.remove("E".getBytes());
		 * btree.commitTransaction(); btree.print(System.out);
		 * System.out.println("Values from I to U:"); RecordIterator iter =
		 * btree.iterateRange("I".getBytes(), "V".getBytes()); byte[] value =
		 * iter.next(); while (value != null) { System.out.print(new
		 * String(value) + " "); value = iter.next(); } System.out.println();
		 */
	}
