 ...
 */
public class NativeStoreConnection extends NotifyingSailConnectionBase
	...
	protected CloseableIteration<? extends BindingSet, QueryEvaluationException> evaluateInternal(
		...
		try {
			...
			NativeTripleSource tripleSource = new NativeTripleSource(
					nativeStore, includeInferred, transactionActive());
			...
			// bindings);
			new QueryJoinOptimizer(new NativeEvaluationStatistics(nativeStore))
					.optimize(tupleExpr, dataset, bindings);
...
//			System.exit(0);
	...
	protected void replaceValues(TupleExpr tupleExpr) throws SailException {
		...
		tupleExpr.visit(new QueryModelVisitorBase<SailException>() {
			...
			public void meet(Var var) {
				if (var.hasValue()) {
					var.setValue(nativeStore.getValueStore().getNativeValue(
							var.getValue()));
				}
			}
		});
	...
	protected CloseableIteration<? extends Resource, SailException> getContextIDsInternal()
		...
		try {
			...
			return new ExceptionConvertingIteration<Resource, SailException>(
					contextIter) {
	...
	protected CloseableIteration<? extends Statement, SailException> getStatementsInternal(
		...
		try {
			CloseableIteration<? extends Statement, IOException> iter = nativeStore
					.createStatementIterator(subj, pred, obj, includeInferred,
							transactionActive(), contexts);

			return new ExceptionConvertingIteration<Statement, SailException>(
					iter) {
	...
	protected long sizeInternal(Resource... contexts) throws SailException {
		...
		try {
			...
			for (int contextID : contextIDs) {
				...
				RecordIterator iter = nativeStore.getTripleStore().getTriples(
						-1, -1, -1, contextID, true, transactionActive());
				try {
...
//		System.out.println(obj.stringValue());
		try {
			...
			for (Resource context : contexts) {
				...
				boolean wasNew = nativeStore.getTripleStore().storeTriple(
			...
			logger
					.error(
							"Encountered an unexpected problem while trying to add a statement",
							e);
			throw e;
		}
	...
	public boolean removeInferredStatement(Resource subj, URI pred, Value obj,
		...
		try {
			...
			try {
				autoStartTransaction();
				int removeCount = removeStatements(subj, pred, obj, false,
						contexts);
		...
		try {
			...
			for (int i = 0; i < contextIDList.size(); i++) {
				...
				if (hasConnectionListeners()) {
					...
					// can
					// be reported
					RecordIterator btreeIter = tripleStore.getTriples(subjID,
							predID, objID, contextID, explicit, true);

					NativeStatementIterator iter = new NativeStatementIterator(
							btreeIter, valueStore);

					removedStatements = Iterations.asList(iter);
				}

				removeCount += tripleStore.removeTriples(subjID, predID, objID,
						contextID, explicit);
