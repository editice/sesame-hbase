...
class AllocatedNodesList {
	...
	/**
	 * Magic number "Allocated Nodes File" to detect whether the file is
	 * actually an allocated nodes file. The first three bytes of the file
	...
	public AllocatedNodesList(File allocNodesFile, BTree btree)
		...
		if (allocNodesFile == null) {
			throw new IllegalArgumentException(
					"allocNodesFile must not be null");
		}
		if (btree == null) {
	...
	private void loadAllocatedNodesInfo() throws IOException {
		...
		if (nioFile.size() >= HEADER_LENGTH
				&& Arrays.equals(MAGIC_NUMBER, nioFile.readBytes(0,
						MAGIC_NUMBER.length))) {
			byte version = nioFile.readByte(MAGIC_NUMBER.length);
			if (version > FILE_FORMAT_VERSION) {
				throw new IOException(
						"Unable to read allocated nodes file; it uses a newer file format");
			} else if (version != FILE_FORMAT_VERSION) {
				throw new IOException(
						"Unable to read allocated nodes file; invalid file format version: "
								+ version);
			}

			data = nioFile.readBytes(HEADER_LENGTH,
					(int) (nioFile.size() - HEADER_LENGTH));
		} else {
