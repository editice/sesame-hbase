...
public class ValueStore extends ValueFactoryBase {
	...
	public ValueStore(File dataDir, boolean forceSync) throws IOException {
		this(dataDir, forceSync, VALUE_CACHE_SIZE, VALUE_ID_CACHE_SIZE,
				NAMESPACE_CACHE_SIZE, NAMESPACE_ID_CACHE_SIZE);
	}

	public ValueStore(File dataDir, boolean forceSync, int valueCacheSize,
			int valueIDCacheSize, int namespaceCacheSize,
	...
	public int storeValue(Value value) throws IOException {
		...
		NativeValue nv = isOwnValue ? (NativeValue) value
				: getNativeValue(value);
	...
	private boolean isOwnValue(Value value) {
		return value instanceof NativeValue
				&& ((NativeValue) value).getValueStoreRevision()
						.getValueStore() == this;
	}
...
//		System.out.println(literal.getLabel()+literal.getLanguage()+literal.getDatatype());
	...
	private NativeLiteral data2literal(int id, byte[] data) throws IOException {
		...
		// Get label
		String label = new String(data, 6 + langLength, data.length - 6
				- langLength, "UTF-8");
