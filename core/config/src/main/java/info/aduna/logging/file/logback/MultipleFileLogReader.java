...
public class MultipleFileLogReader extends LogReaderBase implements LogReader {
	...
	public void setAppender(Appender<?> appender) {
		...
		if (appender instanceof RollingFileAppender) {
			RollingPolicy rp = ((RollingFileAppender<?>) appender)
					.getRollingPolicy();
			if (rp instanceof TimeBasedRollingPolicy) {
				fileNamePattern = ((TimeBasedRollingPolicy) rp)
						.getFileNamePattern();
			} else {
				throw new UnsupportedOperationException(
						"Must be TimeBasedRollingPolicy!");
			}
		} else {
	...
	public void init() throws Exception {
		if (this.getAppender() == null) {
			throw new RuntimeException(
					"Appender must be set before initialization!");
		}
		count = 0;
		...
		String[] files = dir.list(new DateRangeFilenameFilter(pattern, df,
				startCal, endCal));
		Arrays.sort(files);
	...
	public class DateRangeFilenameFilter implements FilenameFilter {
		 ...
		 * @param endCal
		 */
		public DateRangeFilenameFilter(Pattern pattern, SimpleDateFormat df,
		...
		public boolean accept(File dir, String name) {
			...
			cal.setTime(d);
			return (((startCal == null) || (cal.compareTo(startCal) >= 0)) && ((endCal == null) || (cal
					.compareTo(endCal) <= 0)));
		}
