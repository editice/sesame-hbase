...
public class LogConfigurator extends JoranConfigurator {
	...
	public LogReader getLogReader(String appenderName) {
		if (appenderName != null) {
			...
			if (className != null) {
				try {
					LogReader logReader = (LogReader) OptionHelper
							.instantiateByClassName(className,
									info.aduna.logging.LogReader.class, context);
					logReader.setAppender(appenders.get(appenderName));
					return logReader;
				} catch (Exception ex) {
					System.err.println("Could not create logreader of type "
							+ className + " !");
					ex.printStackTrace();
				}
			} else {
				System.err.println("Could not find logreader for appender "
						+ appenderName + " !");
			}
		}
		return null;
	...
	public void addInstanceRules(RuleStore rs) {
		...
		rs.addRule(new Pattern("configuration/appender/logreader"),
				new LogReaderAction());
	}
