	...
	public int launch() throws CommandNotExistsException {
		...
		try {
			if (this.commandArray != null) {
				this.subProcess = Runtime.getRuntime().exec(this.commandArray,
						null, this.baseDir);
			} else {
				this.subProcess = Runtime.getRuntime().exec(this.commandLine,
						null, this.baseDir);
			}
			...
			stderr.join(10000);
			if (exitValue != 0) {
				logger.info("WARNING: exit value " + exitValue
			...
			throw new CommandNotExistsException(
					"Command probably does not exist: " + ioe);
		} catch (Exception e) {
			logger.error("Exception while running/launching \""
					+ getCommandLine() + "\".", e);
		} finally {
	...
	private class BackgroundPrinter extends Thread {
		...
		public void run() {
			try {
				...
				logger.warn(
						"Exception while reading from stream from subprocess.",
						e);
			}
		}
		...
		public void close() {
			try {
				...
				logger
						.warn(
								"Closing background stream for launched process caused exception.",
								e);
			}
		}
	}
