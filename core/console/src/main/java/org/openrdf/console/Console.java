...
public class Console {
	...
	private static final AppVersion VERSION = AppVersion.parse(Sesame
			.getVersion());
	...
	private final AppConfiguration appConfig = new AppConfiguration(APP_NAME,
			APP_NAME, VERSION);
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			String serverURL = commandLine.getOptionValue(serverURLOption
					.getOpt());
	...
	private static void printUsage(Options options) {
		System.out
				.println("Sesame Console, an interactive shell based utility to communicate with Sesame repositories.");
		...
		// writeln(" -s, --serverURL=URL URL of Sesame server to connect to,
		// e.g.
		// http://localhost/openrdf-sesame/");
		// writeln(" -d, --dataDir=DIR Sesame data dir to 'connect' to");
		System.out.println();
		System.out
				.println("For bug reports and suggestions, see http://www.openrdf.org/");
	}
	...
	private boolean executeCommand(String command) throws IOException {
		...
		if ("quit".equals(operation) || "exit".equals(operation)) {
			...
			evaluateQuery(QueryLanguage.SERQL, command.substring("serql"
					.length()));
		} else if ("sparql".equals(operation)) {
			evaluateQuery(QueryLanguage.SPARQL, command.substring("sparql"
					.length()));
	...
	private boolean connectDefault() {
		return installNewManager(new LocalRepositoryManager(appConfig
				.getDataDir()), "default data directory");
	}
	...
	private boolean connectLocal(String path) {
		...
		return installNewManager(new LocalRepositoryManager(dir), dir
				.toString());
	}
	...
	private boolean connectRemote(final String url, final String user,
			String pass) {
		if (pass == null) {
			pass = "";
		}
	...
	private boolean installNewManager(RepositoryManager newManager,
			String newManagerID) {
		...
		try {
			...
			if (templateFile.exists()) {
				if (!templateFile.canRead()) {
					writeError("Not allowed to read template file: "
							+ templateFile);
					return;
				}
				...
				templateStream = Console.class
						.getResourceAsStream(templateFileName);

				if (templateStream == null) {
					writeError("No template called " + templateName
							+ " found in " + templatesDir);
					return;
				}
			}
			...
			String template;
			try {
				template = IOUtil.readString(new InputStreamReader(
			...
			Map<String, List<String>> variableMap = configTemplate
					.getVariableMap();
			...
			if (RepositoryConfigUtil.hasRepositoryConfig(systemRepo, repConfig
					.getID())) {
			...
			try {
				RepositoryConfigUtil.updateRepositoryConfigs(systemRepo,
						repConfig);
	...
	private void dropRepository(String[] tokens) throws IOException {
		...
		try {
			boolean proceed = askProceed(
					"WARNING: you are about to drop repository '" + id + "'.",
					true);
			if (proceed) {
			...
			writeError("Unable to drop repository '" + id + "': "
					+ e.getMessage());
			...
			writeError("Failed to update configuration in system repository: "
					+ e.getMessage());
			logger.warn("Failed to update configuration in system repository",
					e);
		}
	}
		...
		try {
			...
			if (repIDs.isEmpty()) {
				...
				for (String repID : repIDs) {
					...
					try {
						RepositoryInfo repInfo = manager
								.getRepositoryInfo(repID);
	...
	private void showNamespaces() {
		...
		try {
			...
			try {
				CloseableIteration<? extends Namespace, RepositoryException> namespaces = con
						.getNamespaces();
	...
	private void showContexts() {
		...
		try {
			...
			try {
				CloseableIteration<? extends Resource, RepositoryException> contexts = con
						.getContextIDs();
		...
		try {
			...
			if (context != null) {
				...
				if (context.startsWith("_:")) {
					contextURI = repository.getValueFactory().createBNode(
							context.substring(2));
				} else {
					contextURI = repository.getValueFactory()
							.createURI(context);
				}
			...
			writeln("Data has been added to the repository ("
	...
	private void verify(String[] tokens) {
		...
		try {
			...
			if (warnings + errors > 0) {
				writeln("Found " + warnings + " warnings and " + errors
						+ " errors");
			} else {
	...
	private void clear(String[] tokens) {
		...
		for (int i = 1; i < tokens.length; i++) {
			...
			if (contextID.equalsIgnoreCase("null")) {
				...
				contexts[i - 1] = valueFactory.createBNode(contextID
						.substring(2));
			} else {
				try {
	...
	private void evaluateQuery(QueryLanguage ql, String queryString) {
		try {
			...
			ParsedOperation query = QueryParserUtil.parseOperation(ql,
					queryString, null);
	...
	private String addQueryPrefixes(QueryLanguage ql, String queryString) {
		...
		if (repository != null && queryPrefix) {
			...
			// advance.
			if ((SERQL.equals(ql) && queryString.toLowerCase().indexOf(
					"using namespace ") == -1)
					|| SPARQL.equals(ql)
					&& !queryString.toLowerCase().startsWith("prefix")) {
				try {
					RepositoryConnection con = repository.getConnection();
					try {
						Collection<Namespace> namespaces = con.getNamespaces()
								.asList();
						...
						if (!namespaces.isEmpty()) {
							...
							if (SERQL.equals(ql)) {
								...
								for (Namespace namespace : namespaces) {
									namespaceClause.append(namespace
											.getPrefix());
									namespaceClause.append(" = ");
									namespaceClause.append("<");
									namespaceClause.append(SeRQLUtil
											.encodeString(namespace.getName()));
									namespaceClause.append(">, ");
								}

								// Remove trailing ", "
								namespaceClause.setLength(namespaceClause
										.length() - 2);
								...
								for (Namespace namespace : namespaces) {
									...
									namespaceClause.append(": ");
									namespaceClause.append("<");
									namespaceClause.append(SPARQLUtil
											.encodeString(namespace.getName()));
									namespaceClause.append("> ");
								}
					...
					writeError("Error connecting to repository: "
							+ e.getMessage());
	...
	private void evaluateTupleQuery(QueryLanguage ql, String queryString)
			throws UnsupportedQueryLanguageException, MalformedQueryException,
		...
		try {
			...
			Collection<Namespace> namespaces = con.getNamespaces().addTo(
					new ArrayList<Namespace>());

			TupleQueryResult tupleQueryResult = con.prepareTupleQuery(ql,
			...
			try {
				...
				if (bindingNames.isEmpty()) {
					...
					int columnWidth = (consoleWidth - 1) / bindingNames.size()
							- 3;
					...
					for (String bindingName : bindingNames) {
						...
						StringUtil.appendN(' ', columnWidth
								- bindingName.length(), sb);
					}
					sb.append("|");
					...
					while (tupleQueryResult.hasNext()) {
						...
						for (String bindingName : bindingNames) {
							...
							String valueStr = getStringRepForValue(value,
									namespaces);
				...
				long endTime = System.nanoTime();
				writeln(resultCount + " result(s) (" + (endTime - startTime)
						/ 1000000 + " ms)");
			} finally {
	...
	private void evaluateGraphQuery(QueryLanguage ql, String queryString)
		...
		try {
			...
			GraphQueryResult queryResult = con.prepareGraphQuery(ql,
					queryString).evaluate();
			...
			try {
				...
				long endTime = System.nanoTime();
				writeln(resultCount + " results (" + (endTime - startTime)
						/ 1000000 + " ms)");
			} finally {
				queryResult.close();
			}
	...
	private void evaluateBooleanQuery(QueryLanguage ql, String queryString)
		...
		try {
			...
			boolean booleanQueryResult = con.prepareBooleanQuery(ql,
					queryString).evaluate();

			writeln("Answer: " + booleanQueryResult);

			long endTime = System.nanoTime();
			writeln("Query evaluated in " + (endTime - startTime) / 1000000
					+ " ms");
		} finally {
			con.close();
		}
	...
	private void executeUpdate(QueryLanguage ql, String queryString)
		...
		try {
			...
			long endTime = System.nanoTime();
			writeln("Update executed in " + (endTime - startTime) / 1000000
					+ " ms");
		} finally {
			con.close();
		}
	...
	private String getStringRepForValue(Value value,
			Collection<Namespace> namespaces) {
		if (value == null) {
			return "";
			...
			String prefix = getPrefixForNamespace(uri.getNamespace(),
					namespaces);
	...
	private boolean tryToRemoveLock(RepositoryReadOnlyException e,
		...
		if (lockManager.isLocked()) {
			if (askProceed(
					"WARNING: The lock from another process on this repository needs to be removed",
					true)) {
				repo.shutDown();
	...
	private boolean tryToRemoveLock(RepositoryLockedException e)
		...
		if (e.getCause() instanceof SailLockedException) {
			...
			if (lockManager != null && lockManager.isLocked()) {
				if (askProceed("WARNING: The lock from process '"
						+ sle.getLockedBy()
	...
	private boolean askProceed(String msg, boolean defaultValue)
		...
		while (true) {
			...
			if ("no".equalsIgnoreCase(reply) || "no.".equalsIgnoreCase(reply)) {
				return false;
			} else if ("yes".equalsIgnoreCase(reply)
					|| "yes.".equalsIgnoreCase(reply)) {
				return true;
	...
	private String readln(String message) throws IOException {
		if (message != null) {
			write(message + " ");
		}
		return in.readLine();
	...
	private String readPassword(String message) throws IOException {
	...
	private void writeParseError(String prefix, int lineNo, int colNo,
			String msg) {
		...
		String locationString = RDFParseException.getLocationString(lineNo,
				colNo);
	...
	class VerificationListener extends RDFHandlerBase implements
			ParseErrorListener {
