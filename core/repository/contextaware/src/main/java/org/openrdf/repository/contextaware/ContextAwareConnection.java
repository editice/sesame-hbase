...
public class ContextAwareConnection extends RepositoryConnectionWrapper {
	...
	protected boolean isDelegatingRemove() throws RepositoryException {
		return getArchiveContexts().length == 0
				&& getRemoveContexts().length < 2;
	}
	...
	/**
	 * The default context(s) to get the data from. Note that this parameter is
	 * a vararg and as such is optional. If no contexts are supplied the method
	...
	public void add(Iterable<? extends Statement> statements,
		...
		if (isNilContext(contexts)) {
			add(new IteratorIteration<Statement, RuntimeException>(statements
					.iterator()));
		} else {
	...
	public <E extends Exception> void add(
		...
		if (isNilContext(contexts)) {
			super.add(new ConvertingIteration<Statement, Statement, E>(
					statementIter) {

				protected Statement convert(Statement st) {
					if (st.getContext() == null)
						return new ContextStatementImpl(st.getSubject(), st
								.getPredicate(), st.getObject(), insertContext);
					return st;
				}
			});
		} else {
	...
	public void exportStatements(Resource subj, URI pred, Value obj,
			...
			throws RepositoryException, RDFHandlerException {
		if (isAllContext(contexts)) {
			super.exportStatements(subj, pred, obj, isIncludeInferred(),
					handler, getReadContexts());
		} else {
			super.exportStatements(subj, pred, obj, isIncludeInferred(),
					handler, contexts);
		}
	...
	public RepositoryResult<Statement> getStatements(Resource subj, URI pred,
		...
		if (isAllContext(contexts)) {
			return super.getStatements(subj, pred, obj, isIncludeInferred(),
					getReadContexts());
		} else {
			return super.getStatements(subj, pred, obj, isIncludeInferred(),
					contexts);
	...
	public boolean hasStatement(Resource subj, URI pred, Value obj,
			Resource... contexts) throws RepositoryException {
		if (isAllContext(contexts)) {
			return super.hasStatement(subj, pred, obj, isIncludeInferred(),
					getReadContexts());
		} else {
			return super.hasStatement(subj, pred, obj, isIncludeInferred(),
					contexts);
	...
	public boolean hasStatement(Statement st, Resource... contexts)
			throws RepositoryException {
		if (isAllContext(contexts) && st.getContext() == null) {
			return super.hasStatement(st, isIncludeInferred(),
					getReadContexts());
		} else {
	...
	public Query prepareQuery(String query) throws MalformedQueryException,
			RepositoryException {
	...
	public void remove(Iterable<? extends Statement> statements,
			Resource... contexts) throws RepositoryException {
		if (isAllContext(contexts)) {
			remove(new IteratorIteration<Statement, RuntimeException>(
					statements.iterator()));
		} else {
	...
	public <E extends Exception> void remove(
		...
		if (isAllContext(contexts) && removeContexts.length == 1) {
			super.remove(new ConvertingIteration<Statement, Statement, E>(
					statementIter) {
	...
	protected void removeWithoutCommit(Resource subject, URI predicate,
		...
		if (archiveContexts.length > 0) {
			...
			try {
				getDelegate().exportStatements(subject, predicate, object,
						true, handler, archiveContexts);
		...
		if (isAllContext(contexts)) {
			getDelegate().remove(subject, predicate, object,
					getRemoveContexts());
		} else {
	...
	private <O extends Operation> O initOperation(O op) {
		...
		if (readContexts.length > 0 || removeContexts.length > 0
				|| insertContext != null) {
	...
	private boolean isNilContext(Resource[] contexts) {
		return isAllContext(contexts) || contexts.length == 1
				&& contexts[0] == null;
	}
