...
public class HTTPRepositoryConfig extends RepositoryImplConfigBase {
	...
	public void validate() throws RepositoryConfigException {
		...
		if (url == null) {
			throw new RepositoryConfigException(
					"No URL specified for HTTP repository");
		}
	}
	...
	public Resource export(Graph graph) {
		...
		if (url != null) {
			graph.add(implNode, REPOSITORYURL, graph.getValueFactory()
					.createURI(url));
		}
		// if (username != null) {
	...
	public void parse(Graph graph, Resource implNode)
		...
		try {
			...
			Literal username = GraphUtil.getOptionalObjectLiteral(graph,
					implNode, USERNAME);
			if (username != null) {
				setUsername(username.getLabel());
			}
			Literal password = GraphUtil.getOptionalObjectLiteral(graph,
					implNode, PASSWORD);
			if (password != null) {
