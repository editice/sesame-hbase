...
public class HTTPRepository extends RepositoryBase {
			...
			try {
				while (repositoryList.hasNext()) {
					...
					if (uri != null && uri.stringValue().equals(repositoryURL)) {
						isWritable = LiteralUtil.getBooleanValue(bindingSet
	 ...
	 * 
	 * @param format
	 *            the preferred {@link TupleQueryResultFormat}. If set to 'null'
	 *            no explicit preference will be stated.
	 */
	 ...
	 * serialization any context info contained in the query result will be
	 * lost.
	 * 
	 * @param format
	 ...
	 * 
	 * @param username
	 *            the username. Setting this to null will disable
	 *            authentication.
	 * @param password
	 *            the password. Setting this to null will disable
	 *            authentication.
	 */
