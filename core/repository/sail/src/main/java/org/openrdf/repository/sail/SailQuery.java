...
public abstract class SailQuery extends AbstractQuery {
	...
	protected CloseableIteration<? extends BindingSet, QueryEvaluationException> enforceMaxQueryTime(
		...
		if (maxQueryTime > 0) {
			bindingsIter = new QueryInterruptIteration(bindingsIter,
					1000L * maxQueryTime);
		}
	...
	/**
	 * Gets the "active" dataset for this query. The active dataset is either
	 * the dataset that has been specified using {@link #setDataset(Dataset)} or
	 * the dataset that has been specified in the query, where the former takes
	...
	protected class QueryInterruptIteration extends
			TimeLimitIteration<BindingSet, QueryEvaluationException> {
