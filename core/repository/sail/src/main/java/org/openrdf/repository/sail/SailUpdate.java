...
public class SailUpdate extends AbstractOperation implements Update {
	...
	public void execute() throws UpdateExecutionException {
		...
		Map<UpdateExpr, Dataset> datasetMapping = parsedUpdate
				.getDatasetMapping();
		...
		for (UpdateExpr updateExpr : updateExprs) {
			...
			if (updateExpr instanceof Load) {
				...
				Value graph = load.getGraph() != null ? load.getGraph()
						.getValue() : null;
				...
				Dataset activeDataset = getMergedDataset(datasetMapping
						.get(updateExpr));

				try {
					conn.executeUpdate(updateExpr, activeDataset,
							getBindings(), true);
	...
	protected Dataset getMergedDataset(Dataset sparqlDefinedDataset) {
		if (sparqlDefinedDataset == null) {
			...
			if (dgs != null && dgs.size() > 0) {
				...
				// define
				// the default graphs by means of the update itself, the rest
				// can
				...
				mergedDataset.setDefaultInsertGraph(dataset
