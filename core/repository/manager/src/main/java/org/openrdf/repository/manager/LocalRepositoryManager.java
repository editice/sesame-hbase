...
public class LocalRepositoryManager extends RepositoryManager {
	 ...
	 * 
	 * @param baseDir
	 *            The base directory where data for repositories can be stored,
	 *            among other things.
	 */
	...
	protected Repository createRepository(String id)
		...
		try {
			...
			RepositoryConfig repConfig = RepositoryConfigUtil
					.getRepositoryConfig(systemRepository, id);
			if (repConfig != null) {
				repConfig.validate();

				repository = createRepositoryStack(repConfig
	...
	private Repository createRepositoryStack(RepositoryImplConfig config)
		...
		RepositoryFactory factory = RepositoryRegistry.getInstance().get(
				config.getType());
		if (factory == null) {
			throw new RepositoryConfigException("Unsupported repository type: "
					+ config.getType());
		}
		...
		try {
			...
			try {
				...
				throw new RepositoryException(
						"Location of repository does not resolve to a valid URL",
						mue);
			}
			...
			throw new RepositoryException(
					"Unable to read repository configuration", e);
		}
	}
	...
	class ConfigChangeListener extends RepositoryConnectionListenerAdapter {
		...
		private void registerModifiedContexts(RepositoryConnection conn,
				Resource... contexts) {
		...
		public void remove(RepositoryConnection conn, Resource subject,
				URI predicate, Value object, Resource... contexts) {
			if (object != null
					&& object.equals(RepositoryConfigSchema.REPOSITORY_CONTEXT)) {
				if (subject == null) {
					modifiedAllContextsByConnection.put(conn, true);
				} else {
		...
		public void commit(RepositoryConnection con) {
			...
			Boolean fullRefreshNeeded = modifiedAllContextsByConnection
					.remove(con);
			if (fullRefreshNeeded != null && fullRefreshNeeded.booleanValue()) {
				logger
						.debug("Reacting to commit on SystemRepository for all contexts");
				refresh();
			}
			// refresh only modified contexts that actually contain repository
			// configurations
			else {
				Set<Resource> modifiedContexts = modifiedContextsByConnection
						.remove(con);
				Set<Resource> removedContexts = removedContextsByConnection
						.remove(con);
				...
				if (modifiedContexts != null) {
					logger
							.debug(
									"React to commit on SystemRepository for contexts {}",
									modifiedContexts);
					try {
						...
						try {
							...
							for (Resource context : modifiedContexts) {
								logger.debug("Processing modified context {}.",
										context);
								try {
									if (isRepositoryConfigContext(cleanupCon,
											context)) {
										String repositoryID = getRepositoryID(
												cleanupCon, context);
										logger
												.debug(
														"Reacting to modified repository config for {}",
														repositoryID);
									...
									logger
											.error(
													"Failed to process repository configuration changes",
													re);
								}
							}
						} finally {
		...
		private boolean isRepositoryConfigContext(RepositoryConnection con,
				Resource context) throws RepositoryException {
			logger.debug("Is {} a repository config context?", context);
			return con.hasStatement(context, RDF.TYPE, REPOSITORY_CONTEXT,
					true, (Resource) null);
		}

		private String getRepositoryID(RepositoryConnection con,
				Resource context) throws RepositoryException {
			String result = null;

			RepositoryResult<Statement> idStatements = con.getStatements(null,
					REPOSITORYID, null, true, context);
	...
	protected void cleanUpRepository(String repositoryID) throws IOException {
		...
		if (dataDir.isDirectory()) {
			logger.debug("Cleaning up data dir {} for repository {}", dataDir
					.getAbsolutePath(), repositoryID);
