...
public abstract class RepositoryManager {
	 ...
	 * The base name may for example be a repository name entered by the user.
	 * The generated ID will contain a variant of this name that does not occur
	 * as a repository ID in this manager yet and is suitable for use as a file
	 * name (e.g. for the repository's data directory).
	 * 
	 * @param baseName
	 *            The String on which the returned ID should be based, must not
	 *            be <tt>null</tt>.
		...
		if (baseName != null) {
			...
			for (char c : baseName.toCharArray()) {
				if (Character.isLetter(c) || Character.isDigit(c) || c == '-'
						|| c == '_' || c == '.') {
					// Convert to lower case since file names are case
					// insensitive on
					// some/most platforms
	...
	public boolean hasRepositoryConfig(String repositoryID)
			throws RepositoryException, RepositoryConfigException {
		return RepositoryConfigUtil.hasRepositoryConfig(getSystemRepository(),
				repositoryID);
	}

	public RepositoryConfig getRepositoryConfig(String repositoryID)
			throws RepositoryConfigException, RepositoryException {
		return RepositoryConfigUtil.getRepositoryConfig(getSystemRepository(),
				repositoryID);
	}
	 ...
	 * 
	 * @param config
	 *            The repository configuration that should be added to or
	 *            updated in the system repository.
	 * @throws RepositoryException
	 *             If the manager failed to update it's system repository.
	 * @throws RepositoryConfigException
	 *             If the manager doesn't know how to update a configuration due
	 *             to inconsistent configuration data in the system repository.
	 *             For example, this happens when there are multiple existing
	 ...
	 * 
	 * @param repositoryID
	 *            The ID of the repository whose configuration needs to be
	 *            removed.
	 * @throws RepositoryException
	 *             If the manager failed to update it's system repository.
	 * @throws RepositoryConfigException
	 *             If the manager doesn't know how to remove a configuration due
	 *             to inconsistent configuration data in the system repository.
	 *             For example, this happens when there are multiple existing
	 *             configurations with the concerning ID.
	...
	public boolean removeRepositoryConfig(String repositoryID)
		...
		synchronized (initializedRepositories) {
			isRemoved = RepositoryConfigUtil.removeRepositoryConfigs(
					getSystemRepository(), repositoryID);

			if (isRemoved) {
				logger
						.debug(
								"Shutdown repository {} after removal of configuration.",
								repositoryID);
				Repository repository = initializedRepositories
						.remove(repositoryID);
				...
				try {
					...
					throw new RepositoryException(
							"Unable to clean up resources for removed repository "
									+ repositoryID, e);
				}
			}
		}
	...
	public Repository getRepository(String id)
		...
		synchronized (initializedRepositories) {
			...
			if (result == null) {
				// First call (or old object thrown away), create and initialize
				// the
				// repository.
	...
	/**
	 * Returns all configured repositories. This may be an expensive operation
	 * as it initializes repositories that have not been initialized yet.
	 * 
	 ...
	 *             When not able to retrieve existing configurations
	 */
	public abstract Collection<RepositoryInfo> getAllRepositoryInfos(
			boolean skipSystemRepo) throws RepositoryException;
	...
	public void refresh() {
		...
		try {
			RepositoryConnection cleanupCon = getSystemRepository()
					.getConnection();
			try {
				synchronized (initializedRepositories) {
					Iterator<Map.Entry<String, Repository>> iter = initializedRepositories
							.entrySet().iterator();
					...
					while (iter.hasNext()) {
						...
						if (!SystemRepository.ID.equals(repositoryID)) {
							...
							refreshRepository(cleanupCon, repositoryID,
									repository);
						}
					}
				}
	...
	void refreshRepository(RepositoryConnection con, String repositoryID,
	...
	void cleanupIfRemoved(RepositoryConnection con, String repositoryID) {
		try {
			if (RepositoryConfigUtil.getContext(con, repositoryID) == null) {
				logger
						.debug(
								"Cleaning up repository {}, its configuration has been removed",
								repositoryID);

				cleanUpRepository(repositoryID);
			} else {
				logger.debug("Repository {} should not be cleaned up.",
						repositoryID);
			}
		} catch (RepositoryException e) {
			logger.error("Failed to process repository configuration changes",
					e);
			...
			logger.warn("Unable to remove data dir for removed repository {} ",
					repositoryID);
		}
	}
