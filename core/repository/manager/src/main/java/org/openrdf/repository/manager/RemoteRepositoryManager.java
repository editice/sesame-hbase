...
public class RemoteRepositoryManager extends RepositoryManager {
	...
	protected Repository createSystemRepository() throws RepositoryException {
		HTTPRepository systemRepository = new HTTPRepository(serverURL,
				SystemRepository.ID);
	...
	public Collection<RepositoryInfo> getAllRepositoryInfos(
		...
		try {
			...
			try {
				TupleQueryResult responseFromServer = httpClient
						.getRepositoryList();
				while (responseFromServer.hasNext()) {
					BindingSet bindingSet = responseFromServer.next();
					RepositoryInfo repInfo = new RepositoryInfo();

					String id = LiteralUtil.getLabel(bindingSet.getValue("id"),
							null);
					...
					String description = LiteralUtil.getLabel(bindingSet
							.getValue("title"), null);
					boolean readable = LiteralUtil.getBooleanValue(bindingSet
							.getValue("readable"), false);
					boolean writable = LiteralUtil.getBooleanValue(bindingSet
					...
					if (uri instanceof URI) {
						try {
							...
							logger
									.warn(
											"Server reported malformed repository URL: {}",
											uri);
						}
					}
	...
	public boolean removeRepository(String repositoryID)
		...
		boolean existingRepo = RepositoryConfigUtil.hasRepositoryConfig(
