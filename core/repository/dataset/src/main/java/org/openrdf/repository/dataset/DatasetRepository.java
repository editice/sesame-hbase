...
public class DatasetRepository extends RepositoryWrapper {
	...
	public void setDelegate(Repository delegate) {
		if (delegate instanceof SailRepository) {
			...
			throw new IllegalArgumentException(
					"delegate must be a SailRepository, is: "
							+ delegate.getClass());
		}
	}
	...
	public RepositoryConnection getConnection() throws RepositoryException {
		return new DatasetRepositoryConnection(this, getDelegate()
				.getConnection());
	}
	...
	private synchronized void load(URL url, URLConnection urlCon, URI context)
		...
		RDFFormat format = RDFParserRegistry.getInstance()
				.getFileFormatForMIMEType(mimeType);

		// Fall back to using file name extensions
		if (format == null) {
			format = RDFParserRegistry.getInstance().getFileFormatForFileName(
					url.getPath());
		}
