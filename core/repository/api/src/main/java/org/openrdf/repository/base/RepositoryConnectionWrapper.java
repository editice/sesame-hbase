 ...
 */
public class RepositoryConnectionWrapper extends RepositoryConnectionBase
	...
	public void exportStatements(Resource subj, URI pred, Value obj,
		...
		if (isDelegatingRead()) {
			getDelegate().exportStatements(subj, pred, obj, includeInferred,
					handler, contexts);
		} else {
			exportStatements(getStatements(subj, pred, obj, includeInferred,
					contexts), handler);
		}
	}
	...
	public RepositoryResult<Statement> getStatements(Resource subj, URI pred,
			...
			throws RepositoryException {
		return getDelegate().getStatements(subj, pred, obj, includeInferred,
				contexts);
	}
	...
	public boolean hasStatement(Resource subj, URI pred, Value obj,
			...
			throws RepositoryException {
		if (isDelegatingRead()) {
			return getDelegate().hasStatement(subj, pred, obj, includeInferred,
					contexts);
		}
	 ...
	 */
	protected void exportStatements(RepositoryResult<Statement> stIter,
			RDFHandler handler) throws RepositoryException, RDFHandlerException {
		try {
			handler.startRDF();
