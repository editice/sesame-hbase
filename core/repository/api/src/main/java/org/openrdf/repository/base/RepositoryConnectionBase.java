...
public abstract class RepositoryConnectionBase implements RepositoryConnection {
	...
	private volatile ParserConfig parserConfig = new ParserConfig(true, false,
			false, DatatypeHandling.VERIFY);
	...
	public boolean hasStatement(Resource subj, URI pred, Value obj,
		...
		RepositoryResult<Statement> stIter = getStatements(subj, pred, obj,
	...
	public void setAutoCommit(boolean autoCommit) throws RepositoryException {
		...
		// if we are switching from non-autocommit to autocommit mode, commit
		// any
		// pending updates
		if (autoCommit) {
			commit();
	...
	public void add(URL url, String baseURI, RDFFormat dataFormat,
		...
		if (dataFormat != null) {
			...
			Set<RDFFormat> rdfFormats = RDFParserRegistry.getInstance()
					.getKeys();
	...
	private void addZip(InputStream in, String baseURI, RDFFormat dataFormat,
			Resource... contexts) throws IOException, RDFParseException,
			RepositoryException {
		...
		try {
			...
			try {
				for (ZipEntry entry = zipIn.getNextEntry(); entry != null; entry = zipIn
					...
					RDFFormat format = Rio.getParserFormatForFileName(entry
							.getName(), dataFormat);
					...
					try {
						...
						RDFParseException pe = new RDFParseException(msg, e
	...
	protected void addInputStreamOrReader(Object inputStreamOrReader,
		...
		RDFParser rdfParser = Rio.createParser(dataFormat, getRepository()
				.getValueFactory());
	...
	protected void addWithoutCommit(Statement st, Resource... contexts)
		...
		addWithoutCommit(st.getSubject(), st.getPredicate(), st.getObject(),
				contexts);
	}

	protected abstract void addWithoutCommit(Resource subject, URI predicate,
	...
	protected void removeWithoutCommit(Statement st, Resource... contexts)
		...
		removeWithoutCommit(st.getSubject(), st.getPredicate(), st.getObject(),
				contexts);
	}
