...
public class RepositoryUtil {
	 ...
	 * attached properties. Note that the method pulls the entire default
	...
	/**
	 * Compares the models of the default context of two repositories and
	 * returns true if rep1 is a subset of rep2. Note that the method pulls the
	...
	/**
	 * Compares two models defined by the default context of two repositories
	 * and returns the difference between the first and the second model (that
	 * is, all statements that are present in rep1 but not in rep2). Blank node
	 * IDs are not relevant for model equality, they are mapped from one model
	 * to the other by using the attached properties. Note that the method pulls
	 * the entire default context of both repositories into main memory. Use
	 * with caution.
	 * <p>
	 * <b>NOTE: this algorithm is currently broken; it doesn't actually map
	 * blank nodes between the two models.</b>
	 * 
	...
	public static Collection<? extends Statement> difference(Repository rep1,
			Repository rep2) throws RepositoryException {
		Collection<Statement> model1 = new HashSet<Statement>();
		Collection<Statement> model2 = new HashSet<Statement>();

		RepositoryConnection con1 = rep1.getConnection();
		try {
			Iterations.addAll(con1.getStatements(null, null, null, false),
					model1);
		} finally {
			con1.close();
		}
		...
		RepositoryConnection con2 = rep2.getConnection();
		try {
			Iterations.addAll(con2.getStatements(null, null, null, false),
					model2);
		} finally {
			con2.close();
		}
	 ...
	 * the difference between the first and the second model (that is, all
	 * statements that are present in model1 but not in model2). Blank node IDs
	 * are not relevant for model equality, they are mapped from one model to
	 * the other by using the attached properties. *
	 * <p>
	 * <b>NOTE: this algorithm is currently broken; it doesn't actually map
	 * blank nodes between the two models.</b>
	 * 
	 * @return The collection of statements that is the difference between
	 *         model1 and model2.
	 */
	public static Collection<? extends Statement> difference(
			Collection<? extends Statement> model1,
		...
		while (iter1.hasNext()) {
			...
			if (!removed) {
				// statement was not present in model2 and is part of the
				// difference
				result.add(st);
			}
			iter1.remove();
	...
	private static boolean statementsMatch(Statement st1, Statement st2,
