	...
	protected class JoinVisitor extends QueryModelVisitorBase<RuntimeException> {
		...
		public void meet(Join node) {
			...
			try {
				...
				// We order all remaining join arguments based on cardinality
				// and
				...
				if (joinArgs.size() > 0) {
					...
					for (TupleExpr tupleExpr : joinArgs) {
						cardinalityMap.put(tupleExpr, statistics
								.getCardinality(tupleExpr));
						if (tupleExpr instanceof ZeroLengthPath) {
							varsMap.put(tupleExpr, ((ZeroLengthPath) tupleExpr)
									.getVarList());
						} else {
							varsMap.put(tupleExpr,
					...
					// order all other join arguments based on available
					// statistics
				...
				if (orderedSubselects.size() > 0) {
					...
					for (int i = 1; i < orderedSubselects.size(); i++) {
						subselectJoins = new Join(subselectJoins,
								orderedSubselects.get(i));
					}
				}

				if (orderedJoinArgs.size() > 0) {
					// Note: generated hierarchy is right-recursive to help the
					// IterativeEvaluationOptimizer to factor out the left-most
					// join
					// argument
					int i = orderedJoinArgs.size() - 1;
					TupleExpr replacement = orderedJoinArgs.get(i);
					for (i--; i >= 0; i--) {
						replacement = new Join(orderedJoinArgs.get(i),
								replacement);
					}
		...
		protected List<TupleExpr> reorderSubselects(List<TupleExpr> subselects) {
			...
			for (int i = 0; i < subselects.size(); i++) {
				...
				for (int j = i + 1; j < subselects.size(); j++) {
					...
					TupleExpr[] tupleTuple = new TupleExpr[] { firstArg,
							secondArg };
					l.add(tupleTuple);
			...
			// Step 3: sort the rest of the list by selecting and adding an
			// element
			// at a time.
		...
		private TupleExpr getNextSubselect(List<TupleExpr> currentList,
				List<TupleExpr> joinArgs) {

			// determine union of names of all elements currently in the list:
			// this
			...
			// first, the highest join size with the current list, and second,
			// the
			// highest union size.
