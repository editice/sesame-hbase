...
public class QueryEvaluationUtil {
	 ...
	 * defined in the <a
	 * href="http://www.w3.org/TR/rdf-sparql-query/#ebv">SPARQL
	 * specification</a>:
	 * <ul>
	 ...
	 *             In case the application of the EBV algorithm results in a
	 *             type error.
	 */
	...
	public static boolean compare(Value leftVal, Value rightVal,
			CompareOp operator) throws ValueExprEvaluationException {
	...
	public static boolean compareLiterals(Literal leftLit, Literal rightLit,
			CompareOp operator) throws ValueExprEvaluationException {
		// type precendence:
		// - simple literal
		// - numeric
		...
		if (QueryEvaluationUtil.isSimpleLiteral(leftLit)
				&& XMLSchema.STRING.equals(rightDatatype)) {
			commonDatatype = XMLSchema.STRING;
		} else if (QueryEvaluationUtil.isSimpleLiteral(rightLit)
				&& XMLSchema.STRING.equals(leftDatatype)) {
			commonDatatype = XMLSchema.STRING;
		}
		...
		if (QueryEvaluationUtil.isSimpleLiteral(leftLit)
			...
			if (commonDatatype == null) {
				if (leftDatatype.equals(rightDatatype)) {
					...
					// left and right arguments have different datatypes, try to
					// find
					// a
					// more general, shared datatype
					if (leftDatatype.equals(XMLSchema.DOUBLE)
							|| rightDatatype.equals(XMLSchema.DOUBLE)) {
						commonDatatype = XMLSchema.DOUBLE;
					} else if (leftDatatype.equals(XMLSchema.FLOAT)
							|| rightDatatype.equals(XMLSchema.FLOAT)) {
			...
			if (commonDatatype != null) {
				try {
					if (commonDatatype.equals(XMLSchema.DOUBLE)) {
						compareResult = Double.compare(leftLit.doubleValue(),
								rightLit.doubleValue());
					} else if (commonDatatype.equals(XMLSchema.FLOAT)) {
						compareResult = Float.compare(leftLit.floatValue(),
								rightLit.floatValue());
					} else if (commonDatatype.equals(XMLSchema.DECIMAL)) {
						compareResult = leftLit.decimalValue().compareTo(
								rightLit.decimalValue());
					} else if (XMLDatatypeUtil
							.isIntegerDatatype(commonDatatype)) {
						compareResult = leftLit.integerValue().compareTo(
								rightLit.integerValue());
					} else if (commonDatatype.equals(XMLSchema.BOOLEAN)) {
						Boolean leftBool = Boolean.valueOf(leftLit
								.booleanValue());
						Boolean rightBool = Boolean.valueOf(rightLit
								.booleanValue());
						compareResult = leftBool.compareTo(rightBool);
					} else if (XMLDatatypeUtil
							.isCalendarDatatype(commonDatatype)) {
						XMLGregorianCalendar left = leftLit.calendarValue();
						XMLGregorianCalendar right = rightLit.calendarValue();

						compareResult = left.compare(right);

						// Note: XMLGregorianCalendar.compare() returns
						// compatible
						// values
						...
						compareResult = leftLit.getLabel().compareTo(
								rightLit.getLabel());
					}
				} catch (IllegalArgumentException e) {
					// One of the basic-type method calls failed, try syntactic
					// match
		...
		if (compareResult != null) {
			...
			if (!literalsEqual) {
				if (leftDatatype != null && rightDatatype != null
						&& isSupportedDatatype(leftDatatype)
						&& isSupportedDatatype(rightDatatype)) {
					// left and right arguments have incompatible but supported
					// datatypes

					// we need to check that the lexical-to-value mapping for
					// both
					// datatypes succeeds
					if (!XMLDatatypeUtil.isValidValue(leftLit.getLabel(),
							leftDatatype)) {
						throw new ValueExprEvaluationException(
								"not a valid datatype value: " + leftLit);
					}

					if (!XMLDatatypeUtil.isValidValue(rightLit.getLabel(),
							rightDatatype)) {
						throw new ValueExprEvaluationException(
								"not a valid datatype value: " + rightLit);
					}
				} else if (leftDatatype != null
						&& rightLit.getLanguage() == null
						|| rightDatatype != null
						&& leftLit.getLanguage() == null) {
					// For literals with unsupported datatypes we don't know if
					// their
					// values are equal
					throw new ValueExprEvaluationException(
	 ...
	 * literal" is either a simple literal, a plain literal with language tag,
	 * or a literal with datatype xsd:string.
	 * 
	...
	private static boolean isSupportedDatatype(URI datatype) {
		return (XMLSchema.STRING.equals(datatype)
				|| XMLDatatypeUtil.isNumericDatatype(datatype) || XMLDatatypeUtil
