...
public class OrderIterator extends
		DelayedIteration<BindingSet, QueryEvaluationException> {
	...
	protected Iteration<BindingSet, QueryEvaluationException> createIteration()
		...
		try {
			while (iter.hasNext()) {
				...
				if (size < limit || comparator.compare(next, map.lastKey()) < 0) {
					...
					if (list == null) {
						list = distinct ? new LinkedHashSet<BindingSet>()
								: new LinkedList<BindingSet>();
						map.put(next, list);
					}
		...
		return new LookAheadIteration<BindingSet, QueryEvaluationException>() {
			...
			private volatile Iterator<BindingSet> iterator = Collections
					.<BindingSet> emptyList().iterator();
