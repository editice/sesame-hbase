...
public class LeftJoinIterator extends
	...
	public LeftJoinIterator(EvaluationStrategy strategy, LeftJoin join,
		...
		try {
			while (rightIter.hasNext() || leftIter.hasNext()) {
				...
				if (!rightIter.hasNext()) {
					...
					rightIter.close();
					rightIter = strategy.evaluate(join.getRightArg(),
							leftBindings);
				}
				...
				while (rightIter.hasNext()) {
					...
					try {
						if (join.getCondition() == null) {
							...
							// Limit the bindings to the ones that are in scope
							// for
							// this filter
							QueryBindingSet scopeBindings = new QueryBindingSet(
									rightBindings);
							scopeBindings.retainAll(scopeBindingNames);

							if (strategy.isTrue(join.getCondition(),
									scopeBindings)) {
								return rightBindings;
							}
						}
