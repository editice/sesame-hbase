...
public class ValueComparator implements Comparator<Value> {
	...
	private int compareLiterals(Literal leftLit, Literal rightLit) {
		...
		// form."
		if (!(QueryEvaluationUtil.isPlainLiteral(leftLit) || QueryEvaluationUtil
				.isPlainLiteral(rightLit))) {
			try {
				boolean isSmaller = QueryEvaluationUtil.compareLiterals(
		...
		if (result == 0) {
			// datatypes are equal or both literals are untyped; sort by
			// language
		...
		if (result == 0) {
			// Literals are equal as fas as their datatypes and language tags
			// are
