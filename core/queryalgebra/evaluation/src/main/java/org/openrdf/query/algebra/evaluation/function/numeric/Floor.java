	...
	public Literal evaluate(ValueFactory valueFactory, Value... args)
		...
		if (args.length != 1) {
			throw new ValueExprEvaluationException(
					"FLOOR requires exactly 1 argument, got " + args.length);
		}
		...
		if (args[0] instanceof Literal) {
			...
			if (datatype != null && XMLDatatypeUtil.isNumericDatatype(datatype)) {
				if (XMLDatatypeUtil.isIntegerDatatype(datatype)) {
					...
					BigDecimal floor = literal.decimalValue().setScale(0,
							RoundingMode.FLOOR);
					return valueFactory.createLiteral(floor.toPlainString(),
							datatype);
				} else if (XMLDatatypeUtil.isFloatingPointDatatype(datatype)) {
					double floor = Math.floor(literal.doubleValue());
					return valueFactory.createLiteral(Double.toString(floor),
							datatype);
				} else {
					throw new ValueExprEvaluationException(
