	...
	public Literal evaluate(ValueFactory valueFactory, Value... args)
		...
		if (args.length != 1) {
			throw new ValueExprEvaluationException(
					"xsd:dateTime cast requires exactly 1 argument, got "
							+ args.length);
		}
		...
		if (args[0] instanceof Literal) {
			...
			if (QueryEvaluationUtil.isStringLiteral(literal)) {
				String dateTimeValue = XMLDatatypeUtil
						.collapseWhiteSpace(literal.getLabel());
				if (XMLDatatypeUtil.isValidDateTime(dateTimeValue)) {
					return valueFactory.createLiteral(dateTimeValue,
							XMLSchema.DATETIME);
				}
				...
				if (datatype.equals(XMLSchema.DATE)) {
					...
					// fn:year-from-date( SV )), let SMO be
					// eg:convertTo2CharString(
					// fn:month-from-date( SV )), let SDA be
					// eg:convertTo2CharString(
					// fn:day-from-date( SV )) and let STZ be
					// eg:convertTZtoString(
					// fn:timezone-from-date( SV )); TV is xs:dateTime(
					// fn:concat(
					...
					try {
						...
						if (DatatypeConstants.FIELD_UNDEFINED != year
							...
							return valueFactory.createLiteral(dtBuilder
									.toString(), XMLSchema.DATETIME);
						} else {
							throw new ValueExprEvaluationException(
									"not a valid date value: " + literal);
						}
					} catch (IllegalArgumentException e) {
						throw new ValueExprEvaluationException(
								"not a valid calendar value: " + literal);
					}
				}
			}
		}
		...
		throw new ValueExprEvaluationException(
				"Invalid argument for xsd:dateTime cast: " + args[0]);
	}
}
