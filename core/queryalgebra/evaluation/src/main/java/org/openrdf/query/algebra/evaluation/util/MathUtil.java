...
public class MathUtil {
	...
	/**
	 * The default expansion scale used in division operations resulting in a
	 * decimal value with non-terminating decimal expansion. The OpenRDF default
	 * is 24 digits, a value used in various other SPARQL implementations, to
	 * make comparison between these systems easy.
	 */
	...
	/**
	 * Computes the result of applying the supplied math operator on the
	 * supplied left and right operand.
	 * 
	 * @param leftLit
	 ...
	 * @return a numeric datatype literal containing the result of the
	 *         operation. The result will be datatype according to the most
	 *         specific data type the two operands have in common per the
	 *         SPARQL/XPath spec.
			...
			if (commonDatatype.equals(XMLSchema.DOUBLE)) {
				...
				case DIVIDE:
					...
					try {
						...
						result = left.setScale(getDecimalExpansionScale(),
								RoundingMode.HALF_UP).divide(right,
	 ...
	 *            The decimal expansion scale to set. Note that a mimimum of 18
	 *            is required to stay compliant with the XPath specification of
