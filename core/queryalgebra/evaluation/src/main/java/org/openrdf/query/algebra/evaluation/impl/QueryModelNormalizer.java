...
public class QueryModelNormalizer extends
	...
	public void meet(Join join) {
		...
		if (leftArg instanceof EmptySet || rightArg instanceof EmptySet) {
			...
			join.replaceWith(newUnion);
			newUnion.visit(this);
		} else if (leftArg instanceof LeftJoin
				&& isWellDesigned(((LeftJoin) leftArg))) {
			...
			leftJoin.visit(this);
		} else if (rightArg instanceof LeftJoin
				&& isWellDesigned(((LeftJoin) rightArg))) {
			// sort left join above normal joins
		...
		if (leftArg instanceof EmptySet) {
			...
			try {
				conditionValue = QueryEvaluationUtil
						.getEffectiveBooleanValue(((ValueConstant) condition)
								.getValue());
	...
	public void meet(Union union) {
		...
		if (leftArg instanceof EmptySet) {
			...
			union.replaceWith(leftArg);
				...
				&& rightArg instanceof SingletonSet) {
			union.replaceWith(leftArg);
		}
	}
	...
	private static class BindingCollector extends
