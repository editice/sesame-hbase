	...
	public Literal evaluate(ValueFactory valueFactory, Value... args)
		...
		if (args.length != 1) {
			throw new ValueExprEvaluationException(
					"SHA512 requires exactly 1 argument, got " + args.length);
		}
		...
		if (args[0] instanceof Literal) {
			...
			if (QueryEvaluationUtil.isSimpleLiteral(literal)
				...
				try {
					return valueFactory
							.createLiteral(hash(lexValue, "SHA-512"));
				...
				throw new ValueExprEvaluationException(
						"Invalid argument for SHA512: " + literal);
			}
		} else {
			throw new ValueExprEvaluationException(
					"Invalid argument for SHA512: " + args[0]);
		}
	}
