...
public class SameTermFilterOptimizer implements QueryOptimizer {
	...
	protected static class SameTermFilterVisitor extends
		...
		public void meet(Filter filter) {
			...
			if (filter.getCondition() instanceof SameTerm) {
				...
				if (isUnboundVar(leftArg, bindingNames)
						|| isUnboundVar(rightArg, bindingNames)) {
					// One or both var(s) are unbound, this expression will
					// never
					// return any results
		...
		private void renameVar(Var oldVar, Var newVar, Filter filter) {
			filter.getArg().visit(
					new VarRenamer(oldVar.getName(), newVar.getName()));
			...
			extension.addElement(new ExtensionElem(new Var(newVar.getName()),
					oldVar.getName()));
	...
	protected static class VarRenamer extends
			QueryModelVisitorBase<RuntimeException> {
	...
	protected static class VarBinder extends
			QueryModelVisitorBase<RuntimeException> {
