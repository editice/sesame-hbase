...
public class SPARQLFederatedService implements FederatedService {
	...
	private class BatchingServiceIteration extends JoinExecutorBase<BindingSet> {
		...
		protected void handleBindings() throws Exception {
			while (!closed && leftIter.hasNext()) {
				...
				ArrayList<BindingSet> blockBindings = new ArrayList<BindingSet>(
						blockSize);
				...
				CloseableIteration<BindingSet, QueryEvaluationException> materializedIter = new CollectionIteration<BindingSet, QueryEvaluationException>(
						blockBindings);
				addResult(evaluateInternal(service, materializedIter, service
						.getBaseURI()));
			}
		}
	}
	 ...
	 * 
	 * @param serviceUrl
	 *            the serviceUrl use to initialize the inner
	 *            {@link SPARQLRepository}
	 */
	 ...
	 * the type (ASK/SELECT) different evaluation is necessary:
	 * 
	 * SELECT: insert bindings into SELECT query and evaluate ASK: insert
	...
	public CloseableIteration<BindingSet, QueryEvaluationException> evaluate(
		...
		try {
			...
			if (type == QueryType.SELECT) {
				...
				TupleQuery query = getConnection().prepareTupleQuery(
						QueryLanguage.SPARQL, sparqlQueryString, baseUri);
				...
				BooleanQuery query = getConnection().prepareBooleanQuery(
						QueryLanguage.SPARQL, sparqlQueryString, baseUri);
				...
				throw new QueryEvaluationException("Unsupported QueryType: "
						+ type.toString());

		} catch (MalformedQueryException e) {
			throw new QueryEvaluationException(e);
		} catch (RepositoryException e) {
			throw new QueryEvaluationException("SPARQLRepository for endpoint "
					+ rep.toString() + " could not be initialized.", e);
		}
	}
	 ...
	 * at the initialized {@link SPARQLRepository} of this
	 * {@link FederatedService}.
	 * 
	...
	protected CloseableIteration<BindingSet, QueryEvaluationException> evaluateInternal(
		...
		// projection vars
		Set<String> projectionVars = new HashSet<String>(service
				.getServiceVars());
		...
		try {
			...
			if (allBindings.size() == 1) {
				...
				CloseableIteration<BindingSet, QueryEvaluationException> result = evaluate(
						queryString, allBindings.get(0), baseUri,
						QueryType.SELECT, service);
				result = service.isSilent() ? new SilentIteration(result)
						: result;
				return result;
			}

			// To be able to insert the input bindings again later on, we need
			// some
			// means to identify the row of each binding. hence, we use an
			// additional
			// projection variable, which is also passed in the BINDINGS clause
			// with the value of the actual row. The value corresponds to the
			// index
			...
			List<String> relevantBindingNames = getRelevantBindingNames(
					allBindings, service.getServiceVars());

			if (relevantBindingNames.size() != 0) {
				// append the BINDINGS clause to the query
				queryString += buildBindingsClause(allBindings,
			...
			try {
				...
				CloseableIteration<BindingSet, QueryEvaluationException> result = new ServiceFallbackIteration(
						service, preparedQuery, allBindings, this);
				result = service.isSilent() ? new SilentIteration(result)
						: result;
				return result;
			}
	 ...
	 *         to be formed)
	 */
	private List<String> getRelevantBindingNames(List<BindingSet> bindings,
	...
	/**
	 * Computes the BINDINGS clause for the set of relevant input bindings. The
	 * BINDINGS clause is attached to a subquery for block-nested-loop
	 * evaluation. Implementation note: we use a special binding to mark the
	 ...
	 * @throws QueryEvaluationException
	 */
	private String buildBindingsClause(List<BindingSet> bindings,
			List<String> relevantBindingNames) throws QueryEvaluationException {
