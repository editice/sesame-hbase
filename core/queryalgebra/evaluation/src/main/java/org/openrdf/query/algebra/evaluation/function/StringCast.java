	...
	public Literal evaluate(ValueFactory valueFactory, Value... args)
		...
		if (args.length != 1) {
			throw new ValueExprEvaluationException(
					"xsd:string cast requires exactly 1 argument, got "
							+ args.length);
		}
		...
		if (value instanceof URI) {
			...
			if (QueryEvaluationUtil.isSimpleLiteral(literal)) {
				...
				if (datatype.equals(XMLSchema.STRING)) {
					...
					// FIXME Slightly simplified wrt the spec, we just always
					// use the canonical value of the
					// source literal as the target lexical value. This is not
					// 100% compliant with handling of
					// some date-related datatypes.
					// 
					// See
					// http://www.w3.org/TR/xpath-functions/#casting-from-primitive-to-primitive
					if (XMLDatatypeUtil.isValidValue(literal.getLabel(),
							datatype)) {
						String normalizedValue = XMLDatatypeUtil.normalize(
								literal.getLabel(), datatype);
						return valueFactory.createLiteral(normalizedValue,
								XMLSchema.STRING);
					} else {
						return valueFactory.createLiteral(literal.getLabel(),
								XMLSchema.STRING);
		...
		throw new ValueExprEvaluationException(
				"Invalid argument for xsd:string cast: " + value);
	}
}
