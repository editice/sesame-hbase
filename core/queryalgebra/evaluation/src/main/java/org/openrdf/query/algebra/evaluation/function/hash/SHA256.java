	...
	public Literal evaluate(ValueFactory valueFactory, Value... args)
		...
		if (args.length != 1) {
			throw new ValueExprEvaluationException(
					"SHA256 requires exactly 1 argument, got " + args.length);
		}
		...
		if (args[0] instanceof Literal) {
			...
			if (QueryEvaluationUtil.isSimpleLiteral(literal)
				...
				try {
					return valueFactory
							.createLiteral(hash(lexValue, "SHA-256"));
				...
				throw new ValueExprEvaluationException(
						"Invalid argument for SHA256: " + literal);
			}
		} else {
			throw new ValueExprEvaluationException(
					"Invalid argument for SHA256: " + args[0]);
		}
	}
