...
/**
 * Iteration which forms the cross product of a list of materialized input
 * bindings with each result obtained from the inner iteration. See
 * {@link SPARQLFederatedService}.
 * 
 * Example: <source> inputBindings := {b1, b2, ...} resultIteration := {r1, r2,
 * ...}
 * 
 * getNextElement() returns (r1,b1), (r1, b2), ..., (r2, b1), (r2, b2), ...
 * 
 * i.e. compute the cross product per result binding (see
 * {@link #computedCrossProduct} </source>
 * 
 * 
 * @author Andreas Schwarte
 */
public class ServiceCrossProductIteration extends
	...
	protected BindingSet getNextElement() throws QueryEvaluationException {
		...
		if (inputBindingsIterator.hasNext()) {
			...
			QueryBindingSet res = new QueryBindingSet(next.size()
					+ currentInputBinding.size());
			res.addAll(next);
