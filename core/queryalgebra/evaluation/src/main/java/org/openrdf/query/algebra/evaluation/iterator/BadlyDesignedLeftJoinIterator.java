...
public class BadlyDesignedLeftJoinIterator extends LeftJoinIterator {
	...
	public BadlyDesignedLeftJoinIterator(EvaluationStrategy strategy,
			LeftJoin join, BindingSet inputBindings, Set<String> problemVars)
	...
	protected BindingSet getNextElement() throws QueryEvaluationException {
		...
		while (result != null
				&& !QueryResultUtil
						.bindingSetsCompatible(inputBindings, result)) {
			result = super.getNextElement();
		}

		if (result != null) {
			// Make sure the provided problemVars are part of the returned
			// results
			...
			for (String problemVar : problemVars) {
				if (!result.hasBinding(problemVar)) {
					...
					extendedResult.addBinding(problemVar, inputBindings
							.getValue(problemVar));
				}
			}
	...
	private static QueryBindingSet getFilteredBindings(BindingSet bindings,
