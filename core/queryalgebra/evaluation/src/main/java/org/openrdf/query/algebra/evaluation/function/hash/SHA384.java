	...
	public Literal evaluate(ValueFactory valueFactory, Value... args)
		...
		if (args.length != 1) {
			throw new ValueExprEvaluationException(
					"SHA384 requires exactly 1 argument, got " + args.length);
		}
		...
		if (args[0] instanceof Literal) {
			...
			if (QueryEvaluationUtil.isSimpleLiteral(literal)
				...
				try {
					return valueFactory
							.createLiteral(hash(lexValue, "SHA-384"));
				...
				throw new ValueExprEvaluationException(
						"Invalid argument for SHA384: " + literal);
			}
		} else {
			throw new ValueExprEvaluationException(
					"Invalid argument for SHA384: " + args[0]);
		}
	}
