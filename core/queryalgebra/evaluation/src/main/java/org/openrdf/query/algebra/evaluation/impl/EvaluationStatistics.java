...
public class EvaluationStatistics {
	...
	protected static class CardinalityCalculator extends
		...
		public void meet(Service node) {
			if (!node.getServiceRef().hasValue()) {
				// the URI is not available, may be computed in the course of
				// the
				// query
				// => use high cost to order the SERVICE node late in the query
				// plan
				cardinality = 100000;
			} else {
				...
				if (count == 1 && node.getServiceVars().size() > 1) {
					cardinality = 100 + node.getServiceVars().size(); // TODO
																		// (should
					// be higher
					// than other
					// simple
	...
	private static class ServiceNodeAnalyzer extends
			QueryModelVisitorBase<RuntimeException> {
