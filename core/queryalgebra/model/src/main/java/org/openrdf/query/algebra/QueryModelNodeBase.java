...
public abstract class QueryModelNodeBase implements QueryModelNode {
	...
	public void replaceChildNode(QueryModelNode current,
		...
		throw new IllegalArgumentException("Node is not a child node: "
				+ current);
	}
	 ...
	 * {@link IllegalArgumentException} indicating that <tt>current</tt> is not
	...
	public QueryModelNodeBase clone() {
		try {
			...
			throw new RuntimeException(
					"Query model nodes are required to be cloneable", e);
		}
	}

	protected <T extends QueryModelNode> boolean replaceNodeInList(
			List<T> list, QueryModelNode current, QueryModelNode replacement) {
