...
public class Service extends UnaryTupleOperator {
	...
	/*
	 * a string representation of the inner expression (e.g. extracted during
	 * parsing)
	 */
	...
	/*
	 * prepared queries, including prefix. Contains %PROJECTION_VARS% to be
	 * replaced at evaluation time. see
	 */
	...
	public Service(Var serviceRef, TupleExpr serviceExpr,
			...
			Map<String, String> prefixDeclarations, String baseURI,
			boolean silent) {
		super(serviceExpr);
	...
	/**
	 * Returns the query string using the provided projection vars. Two cases
	 * are considered: a) projectionVars available => SELECT query The variables
	 * are inserted into the preparedSelectQueryString in the SELECT clause. b)
	...
	public String getQueryString(Set<String> projectionVars) {
		...
		return preparedSelectQueryString.replace("%PROJECTION_VARS%", sb
				.toString());
	}
	...
	private String parseServiceExpression(String serviceExpression) {
		...
		if (serviceExpression.toLowerCase().startsWith("service")) {
			return serviceExpression.substring(
					serviceExpression.indexOf("{") + 1,
