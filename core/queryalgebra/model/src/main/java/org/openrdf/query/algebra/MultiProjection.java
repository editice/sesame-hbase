...
public class MultiProjection extends UnaryTupleOperator {
	...
	public Set<String> getAssuredBindingNames() {
		...
		if (projections.size() >= 1) {
			...
			bindingNames.addAll(projections.get(0).getTargetNamesFor(
					assuredSourceNames));

			for (int i = 1; i < projections.size(); i++) {
				bindingNames.retainAll(projections.get(i).getTargetNamesFor(
						assuredSourceNames));
			}
		}
	...
	public MultiProjection clone() {
		...
		clone.projections = new ArrayList<ProjectionElemList>(getProjections()
				.size());
