...
public class XMLDateTime implements Cloneable, Comparable<XMLDateTime> {
	...
	public XMLDateTime(String dateTimeString) {
		this.dateTimeString = XMLDatatypeUtil
				.collapseWhiteSpace(dateTimeString);
		parseDateTimeString();
		setNumericFields();
		validateFieldValues();
	}
	...
	private void parseDateTimeString() {
		if (dateTimeString.length() < 19) {
			throw new IllegalArgumentException(
					"String value too short to be a valid xsd:dateTime value: "
							+ dateTimeString);
		}
	...
	private void setNumericFields() {
		try {
			...
			if (fractionalSeconds != null) {
				...
				iFractionalSeconds = Double.parseDouble("0."
						+ fractionalSeconds);
			}
			...
			throw new IllegalArgumentException("All fields must be numbers: "
					+ dateTimeString);
		}
	}
	...
	private void validateFieldValues() {
		if (year.length() < 4) {
			throw new IllegalArgumentException(
					"Year field requires at least 4 digits: " + dateTimeString);
		}
		if (months.length() != 2) {
			throw new IllegalArgumentException(
					"Month field must be two digits: " + dateTimeString);
		}
		if (days.length() != 2) {
			throw new IllegalArgumentException(
					"Days field must be two digits: " + dateTimeString);
		}
		if (hours.length() != 2) {
			throw new IllegalArgumentException(
					"Hours field must be two digits: " + dateTimeString);
		}
		if (minutes.length() != 2) {
			throw new IllegalArgumentException(
					"Minutes field must be two digits: " + dateTimeString);
		}
		if (seconds.length() != 2) {
			throw new IllegalArgumentException(
					"Seconds field must be two digits: " + dateTimeString);
		}
		if (hoursTimezone != null) {
			if (hoursTimezone.length() != 2) {
				throw new IllegalArgumentException(
						"Timezone-hours field must be two digits: "
								+ dateTimeString);
			}
			if (minutesTimezone.length() != 2) {
				throw new IllegalArgumentException(
						"Timezone-minutes field must be two digits: "
								+ dateTimeString);
			}
		}
		...
		if (year.length() > 4 && year.charAt(0) == '0') {
			throw new IllegalArgumentException(
					"Leading zeros in years with more than 4 digits are prohibited: "
							+ dateTimeString);
		}
		if (iYear == 0) {
			throw new IllegalArgumentException("0000 is not a valid year: "
					+ dateTimeString);
		}
		if (iHours > 24) {
		...
		if (iHoursTimezone > 14 || iMinutesTimezone > 59
				|| iHoursTimezone == 14 && iMinutesTimezone != 0) {
	...
	public void normalize() {
		...
		if (iHours == 24 || hoursTimezone != null
				&& (iHoursTimezone != 0 || iMinutesTimezone != 0)) {
			...
			Calendar cal = new GregorianCalendar(iYear, iMonths - 1, iDays,
		...
		if (fractionalSeconds != null) {
			...
			if (zeroCount == fractionalSeconds.length()) {
				...
				fractionalSeconds = fractionalSeconds.substring(0,
						fractionalSeconds.length() - zeroCount);
			}
		}
	...
	public int compareTo(XMLDateTime otherDT) {
		...
		if (thisDT.hoursTimezone != null
				&& (thisDT.iHoursTimezone != 0 || thisDT.iMinutesTimezone != 0)) {
			// Create a normalized copy of this DateTime object
			thisDT = (XMLDateTime) thisDT.clone();
			thisDT.normalize();
		}

		if (otherDT.hoursTimezone != null
				&& (otherDT.iHoursTimezone != 0 || otherDT.iMinutesTimezone != 0)) {
			// Create a normalized copy of this DateTime object
		...
		if (thisDT.iYear != otherDT.iYear) {
			...
			result = (thisDT.iFractionalSeconds < otherDT.iFractionalSeconds) ? -1
					: 1;
		}
