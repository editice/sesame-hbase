...
public class XMLDatatypeUtil {
	...
	public static boolean isPrimitiveDatatype(URI datatype) {
		return datatype.equals(XMLSchema.DURATION)
	...
	public static boolean isDerivedDatatype(URI datatype) {
		return datatype.equals(XMLSchema.NORMALIZEDSTRING)
	...
	/**
	 * Checks whether the supplied datatype is equal to xsd:decimal or one of
	 * the built-in datatypes that is derived from xsd:decimal.
	 */
	public static boolean isDecimalDatatype(URI datatype) {
		return datatype.equals(XMLSchema.DECIMAL)
				|| isIntegerDatatype(datatype);
	}

	/**
	 * Checks whether the supplied datatype is equal to xsd:integer or one of
	 * the built-in datatypes that is derived from xsd:integer.
	 */
	public static boolean isIntegerDatatype(URI datatype) {
		return datatype.equals(XMLSchema.INTEGER)
				|| datatype.equals(XMLSchema.LONG)
				|| datatype.equals(XMLSchema.INT)
				|| datatype.equals(XMLSchema.SHORT)
				|| datatype.equals(XMLSchema.BYTE)
	...
	public static boolean isFloatingPointDatatype(URI datatype) {
		return datatype.equals(XMLSchema.FLOAT)
				|| datatype.equals(XMLSchema.DOUBLE);
	}
	...
	private static boolean isPrefixStartChar(int c) {
		return ASCIIUtil.isLetter(c) || c >= 0x00C0 && c <= 0x00D6
	...
	private static boolean isNameChar(int c) {
		return isNameStartChar(c) || ASCIIUtil.isNumber(c) || c == '-'
	...
	/**
	 * Normalizes the supplied value according to the normalization rules for
	 ...
	 */
	private static String normalizeIntegerValue(String integer,
	...
	public static String normalizeFloat(String value) {
		return normalizeFPNumber(value, "-16777215.0", "16777215.0", "-149",
				"104");
	}
	...
	public static String normalizeDouble(String value) {
		return normalizeFPNumber(value, "-9007199254740991.0",
				"9007199254740991.0", "-1075", "970");
	}
	 ...
	 *             If the supplied value is not a legal floating point number.
	 */
	private static String normalizeFPNumber(String value, String minMantissa,
			String maxMantissa, String minExponent, String maxExponent) {
		value = collapseWhiteSpace(value);
	 ...
	 *             number or if <tt>NaN</tt> is compared to a floating point
	 *             number other than <tt>NaN</tt>.
	 */
	 ...
	 *         string argument.
	 * @throws NumberFormatException
	 *             If the supplied string is not a valid xsd:decimal or
	 *             xsd:double value.
	 */
	...
	public static URI qnameToURI(QName qname) {
		if (DatatypeConstants.DATETIME.equals(qname)) {
			...
			throw new IllegalArgumentException(
					"QName cannot be mapped to an XML Schema URI: "
							+ qname.toString());
		}
	}
