...
public class GraphImpl extends AbstractCollection<Statement> implements Graph {
	...
	public boolean add(Resource subj, URI pred, Value obj, Resource... contexts) {
		...
		if (contexts.length == 0) {
			...
			for (Resource context : contexts) {
				graphChanged |= add(valueFactory.createStatement(subj, pred,
						obj, context));
			}
		}
	...
	public Iterator<Statement> match(Resource subj, URI pred, Value obj,
			Resource... contexts) {
		OpenRDFUtil.verifyContextNotNull(contexts);
	...
	private static class PatternIterator extends FilterIterator<Statement> {
		...
		public PatternIterator(Iterator<? extends Statement> iter,
