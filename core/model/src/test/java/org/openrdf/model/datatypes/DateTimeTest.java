...
public class DateTimeTest extends TestCase {
	...
	private static final String[] VALID_DATES = { "0001-01-01T00:00:00",
			"0001-01-01T00:00:00.0", "0001-01-01T00:00:00Z",
			"0001-01-01T00:00:00.0Z", "0001-01-01T00:00:00+00:00",
			"0001-01-01T00:00:00.0+00:00", "0001-01-01T00:00:00.0-00:00",
			"0001-01-01T00:00:00.0+14:00", "0001-01-01T00:00:00.0-14:00",
			"-0001-01-01T00:00:00", "1234-12-31T23:59:59",
	...
	private static final String[] INVALID_DATES = {
			...
			"0001-02-30T00:00.00",
			// "2005-02-29T00:00:00",
			"0001-01-01T25:00.00", "0001-01-01T00:61.00",
			"0001-01-01T00:00.61", "0001-01-01T00:00.00+15:00",
			"0001-01-01T00:00.00-15:00", "001-01-01T00:00:00.0",
			"0001-1-01T00:00:00.0", "0001-01-1T00:00:00.0",
			"0001-01-01T0:00:00.0", "0001-01-01T00:0:00.0",
			"0001-01-01T00:00:0.0", "0001/01-01T00:00:00.0",
			"0001-01/01T00:00:00.0", "0001-01-01t00:00:00.0",
			"0001-01-01T00.00:00.0", "0001-01-01T00:00.00.0",
			"0001-01-01T00:00:00:0", "0001-01-01T00:00.00+0:00",
			"0001-01-01T00:00.00+00:0", "0001-jan-01T00:00:00",
			"0001-01-01T00:00:00+00:00Z", "0001-01-01T24:01:00",
			"0001-01-01T24:00:01", "00001-01-01T00:00:00",
			"0001-001-01T00:00:00", "0001-01-001T00:00:00",
			"0001-01-01T000:00:00", "0001-01-01T00:000:00",
			"0001-01-01T00:00:000", "0001-01-01T00:00:000",
			"0001-01-01T00:00:00z", "0001-01-01T00:00:00+05",
			"0001-01-01T00:00:00+0500", "0001-01-01T00:00:00GMT",
			"0001-01-01T00:00:00PST", "0001-01-01T00:00:00GMT+05",
			"0000-01-01T00:00:00", "0000-01-01T00:00:00",
			"-0000-01-01T00:00:00", "+0001-01-01T00:00:00" };
	...
	public static Test suite() {
		...
		for (int i = 0; i < NORMALIZED_DATES.length; i++) {
			suite.addTest(new NormalizedDateTest(NORMALIZED_DATES[i][0],
					NORMALIZED_DATES[i][1]));
		}

		for (int i = 0; i < EQUAL_DATES.length; i++) {
			suite.addTest(new EqualDateTest(EQUAL_DATES[i][0],
					EQUAL_DATES[i][1]));
		}

		for (int i = 0; i < COMPARISON_DATES.length; i++) {
			suite.addTest(new CompareDateTest(COMPARISON_DATES[i][0],
					COMPARISON_DATES[i][1]));
		}
	...
	private static class ValidDateTest extends TestCase {
		...
		protected void runTest() {
			if (!XMLDatatypeUtil.isValidDateTime(dateString)) {
				String errMsg = "string should be valid but is not: "
						+ dateString;
				logError(errMsg);
				fail(errMsg);
			}
	...
	private static class InvalidDateTest extends TestCase {
		...
		protected void runTest() {
			if (XMLDatatypeUtil.isValidDateTime(dateString)) {
				String errMsg = "string should be invalid but is not: "
						+ dateString;
				logError(errMsg);
				fail(errMsg);
			}
	...
	private static class NormalizedDateTest extends TestCase {
		...
		protected void runTest() {
			...
			if (!expected.equals(normalized)) {
				String errMsg = "normalizing " + input + " should produce "
						+ expected + " but was " + normalized;
				logError(errMsg);
				fail(errMsg);
			}
		}
	...
	private static class EqualDateTest extends TestCase {
		...
		protected void runTest() {
			int result = XMLDatatypeUtil.compareDateTime(dateString1,
					dateString2);
			if (result != 0) {
				String errMsg = dateString1 + " and " + dateString2
	...
	private static class CompareDateTest extends TestCase {
		...
		protected void runTest() {
			int result = XMLDatatypeUtil.compareDateTime(dateString1,
					dateString2);
			if (result >= 0) {
				String errMsg = dateString1 + " should be smaller than "
						+ dateString2 + " but is not (result=" + result + ")";
				logError(errMsg);
				fail(errMsg);
			}
		}
