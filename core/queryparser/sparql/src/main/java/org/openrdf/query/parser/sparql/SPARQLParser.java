...
public class SPARQLParser implements QueryParser {
	...
	public ParsedUpdate parseUpdate(String updateStr, String baseURI)
		...
		try {
			...
			ASTUpdateSequence updateSequence = SyntaxTreeBuilder
					.parseUpdateSequence(updateStr);

			List<ASTUpdateContainer> updateOperations = updateSequence
					.getUpdateContainers();
			...
			for (int i = 0; i < updateOperations.size(); i++) {
				...
				if (uc.jjtGetNumChildren() == 0 && i > 0
						&& i < updateOperations.size() - 1) {
					// empty update in the middle of the sequence
					throw new MalformedQueryException(
							"empty update in sequence not allowed");
				}
				...
				// sequences: if
				// the current
				// operation has its own prefix declarations, use those.
				// Otherwise,
				// try and use
				// prefix declarations from a previous operation in this
				// sequence.
				...
				UpdateExprBuilder updateExprBuilder = new UpdateExprBuilder(
						new ValueFactoryImpl());

				ASTUpdate updateNode = uc.getUpdate();
				if (updateNode != null) {
					UpdateExpr updateExpr = (UpdateExpr) updateNode.jjtAccept(
							updateExprBuilder, null);
		...
		while ((line = in.readLine()) != null) {
			if (line.length() > 0) {
				...
				if (queryStr.length() > 0) {
					try {
						ParsedOperation parsedQuery = QueryParserUtil
								.parseOperation(QueryLanguage.SPARQL, queryStr,
										null);
