	...
	private static class NullVisitor extends ASTVisitorBase {
		...
		public Object visit(ASTSelect selectNode, Object data)
			...
			while (iter.hasNext()) {
				...
				if (pe.getValueExpr() instanceof ASTNull) {
					logger
							.warn("Use of NULL values in SeRQL queries has been deprecated");
					iter.remove();
				}
			}
		...
		public Object visit(ASTCompare compareNode, Object data)
			...
			if (leftIsNull && rightIsNull) {
				...
				case EQ:
					logger
							.warn("Use of NULL values in SeRQL queries has been deprecated, use BOUND(...) instead");
				...
				if (valueOperand instanceof ASTVar && operator == EQ
						|| operator == NE) {
