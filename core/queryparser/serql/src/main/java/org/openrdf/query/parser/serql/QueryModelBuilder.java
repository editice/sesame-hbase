...
class QueryModelBuilder extends ASTVisitorBase {
	...
	public static TupleExpr buildQueryModel(ASTQueryContainer node,
			ValueFactory valueFactory) throws MalformedQueryException {
		try {
	...
	public TupleExpr visit(ASTSelectQuery node, Object data)
		...
		if (orderByNode != null) {
			List<OrderElem> orderElemements = (List<OrderElem>) orderByNode
					.jjtAccept(this, null);
			tupleExpr = new Order(tupleExpr, orderElemements);
		}

		// Apply projection
		tupleExpr = (TupleExpr) node.getSelectClause().jjtAccept(this,
				tupleExpr);
	...
	public TupleExpr visit(ASTSelect node, Object data) throws VisitorException {
		...
		for (ASTProjectionElem projElemNode : node.getProjectionElemList()) {
			ValueExpr valueExpr = (ValueExpr) projElemNode.getValueExpr()
					.jjtAccept(this, null);
	...
	public TupleExpr visit(ASTConstructQuery node, Object data)
		...
		if (!constructNode.isWildcard()) {
			TupleExpr constructExpr = (TupleExpr) constructNode.jjtAccept(this,
					null);
			tupleExpr = cb.buildConstructor(tupleExpr, constructExpr,
					constructNode.isDistinct(), constructNode.isReduced());
		} else if (node.hasQueryBody()) {
			tupleExpr = cb.buildConstructor(tupleExpr, constructNode
					.isDistinct(), constructNode.isReduced());
		}
	...
	public Object visit(ASTFrom node, Object data) throws VisitorException {
		...
		if (node.hasContextID()) {
			...
			ValueExpr contextID = (ValueExpr) node.getContextID().jjtAccept(
					this, null);
			...
			if (contextID instanceof Var) {
				...
				throw new IllegalArgumentException(
						"Unexpected contextID result type: "
								+ contextID.getClass());
			}
		}
	...
	public Object visit(ASTWhere node, Object data) throws VisitorException {
		ValueExpr valueExpr = (ValueExpr) node.getCondition().jjtAccept(this,
				null);
	...
	public List<OrderElem> visit(ASTOrderBy node, Object data)
		...
		List<OrderElem> elements = new ArrayList<OrderElem>(orderExprList
				.size());
	...
	public OrderElem visit(ASTOrderExpr node, Object data)
			throws VisitorException {
		ValueExpr valueExpr = (ValueExpr) node.getValueExpr().jjtAccept(this,
				null);
	...
	public Object visit(ASTBasicPathExprTail tailNode, Object data)
		...
		List<Var> objVars = (List<Var>) tailNode.getNode()
				.jjtAccept(this, null);

		Var contextVar = graphPattern.getContextVar();
		StatementPattern.Scope spScope = graphPattern
				.getStatementPatternScope();

		for (Var subjVar : subjVars) {
			for (Var objVar : objVars) {
				StatementPattern sp = new StatementPattern(spScope, subjVar,
						predVar, objVar, contextVar);
	...
	public Var visit(ASTEdge node, Object data) throws VisitorException {
		...
		if (arg instanceof Var) {
			...
			throw new IllegalArgumentException(
					"Unexpected edge argument type: " + arg.getClass());
		}
	}
		...
		for (int i = 0; i < nodeVars.size() - 1; i++) {
			...
			for (int j = i + 1; j < nodeVars.size(); j++) {
				...
				if (!var1.hasValue() || !var2.hasValue()) {
					graphPattern
							.addConstraint(new Not(new SameTerm(var1, var2)));
				}
			}
		}
	...
	public Var visit(ASTNodeElem node, Object data) throws VisitorException {
		...
		if (valueExpr instanceof Var) {
			...
			throw new IllegalArgumentException(
					"Unexpected node element result type: "
							+ valueExpr.getClass());
		}
	}
	...
	public Var visit(ASTReifiedStat node, Object data) throws VisitorException {
		...
		graphPattern.addRequiredTE(new StatementPattern(spScope, idVar,
				rdfType, rdfStatement, contextVar));
		graphPattern.addRequiredTE(new StatementPattern(spScope, idVar,
				rdfSubject, subjVar, contextVar));
		graphPattern.addRequiredTE(new StatementPattern(spScope, idVar,
				rdfPredicate, predVar, contextVar));
		graphPattern.addRequiredTE(new StatementPattern(spScope, idVar,
				rdfObject, objVar, contextVar));
	...
	public LangMatches visit(ASTLangMatches node, Object data)
		...
		ValueExpr range = (ValueExpr) node.getLanguageRange().jjtAccept(this,
				null);
	...
	public SameTerm visit(ASTSameTerm node, Object data)
			throws VisitorException {
		ValueExpr leftArg = (ValueExpr) node.getLeftOperand().jjtAccept(this,
				null);
		ValueExpr rightArg = (ValueExpr) node.getRightOperand().jjtAccept(this,
				null);
	...
	public CompareAny visit(ASTCompareAny node, Object data)
			throws VisitorException {
		ValueExpr valueExpr = (ValueExpr) node.getLeftOperand().jjtAccept(this,
				null);
		TupleExpr tupleExpr = (TupleExpr) node.getRightOperand().jjtAccept(
				this, null);
	...
	public ValueExpr visit(ASTInList node, Object data) throws VisitorException {
		ValueExpr leftArg = (ValueExpr) node.getValueExpr().jjtAccept(this,
				null);
	...
	public ValueConstant visit(ASTLiteral litNode, Object data)
		...
		if (dtNode instanceof ASTURI) {
			...
			throw new IllegalArgumentException("Unexpected datatype type: "
					+ dtNode.getClass());
		}
		...
		if (datatype != null) {
			...
			literal = valueFactory.createLiteral(litNode.getLabel(), litNode
					.getLang());
		} else {
