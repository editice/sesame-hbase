...
public class QueryParserUtil {
	...
	public static QueryParser createParser(QueryLanguage ql)
		...
		throw new UnsupportedQueryLanguageException(
				"No factory available for query language " + ql);
	}
	 ...
	 *            operation against, can be <tt>null</tt> if the operation does
	 *            not contain any relative URIs.
	...
	public static ParsedOperation parseOperation(QueryLanguage ql,
			String operation, String baseURI) throws MalformedQueryException {
		ParsedOperation parsedOperation = null;
		QueryParser parser = createParser(ql);

		if (QueryLanguage.SPARQL.equals(ql)) {
			String strippedOperation = removeSPARQLQueryProlog(operation)
					.toUpperCase();
	 ...
	 * @param baseURI
	 *            The base URI to resolve any relative URIs that are in the
	 *            operation against, can be <tt>null</tt> if the update
	 *            operation does not contain any relative URIs.
	 ...
	 *             If the specified query language is not supported.
	 */
	public static ParsedUpdate parseUpdate(QueryLanguage ql, String update,
	 ...
	 *             If the specified query language is not supported.
	 */
	public static ParsedQuery parseQuery(QueryLanguage ql, String query,
			String baseURI) throws MalformedQueryException,
			UnsupportedQueryLanguageException {
		QueryParser parser = createParser(ql);
	...
	public static ParsedTupleQuery parseTupleQuery(QueryLanguage ql,
			String query, String baseURI) throws MalformedQueryException,
			UnsupportedQueryLanguageException {
		...
		throw new IllegalArgumentException("query is not a tuple query: "
				+ query);
	}
	...
	public static ParsedGraphQuery parseGraphQuery(QueryLanguage ql,
		...
		throw new IllegalArgumentException("query is not a graph query: "
				+ query);
	}
	...
	public static ParsedBooleanQuery parseBooleanQuery(QueryLanguage ql,
		...
		throw new IllegalArgumentException("query is not a boolean query: "
				+ query);
	}
	...
	public static String removeSPARQLQueryProlog(String queryString) {
		...
		while (matcher.find()) {
			normalizedQuery = normalizedQuery.substring(matcher.end()
					- startIndexCorrection, normalizedQuery.length());
